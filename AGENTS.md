...
9. [docs/theory/juggler_fate_almost_all_note.md](docs/theory/juggler_fate_almost_all_note.md) — Paper C: *Fate Contagion and the Juggler Conjecture as an Almost-All Statement* — the paper distilled from items 7–8 (main theorem with \(\lambda^{**}\) unconditional; Tao-type equivalence; hierarchy of hypotheses down to the pressure form; exact map; one frontier statement; §10 conditional on Paper B). Built like Papers A and B (`pandoc -f markdown+tex_math_single_backslash --pdf-engine=xelatex -V geometry:margin=1in`) into `juggler_review/`; the laboratory notes 7–8 remain the source of the proofs and constants.
