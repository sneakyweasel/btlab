...
| Level-2 wave bound \(q^{-1/6}P^{23/24+\varepsilon}\) (Lemma 5.2) | **EXACT — HUMAN PROOF** | targeted third differencing; standalone statement under (C1)--(C4); (ii) from (i) is Claims A–H; coefficient budget \(\lvert q_d\rvert\le 4P^{1/24}\); (C4) is \(h_1,h_2\le P^{1/24}\); (D3) closed under one difference (\(3\to6\)); leftover \(uW\) modes of Theorem 5.3 Step 4 are a good/bad \(h_3\) split, not printed (ii); at most nine decorations |
...
| Length-5 splits (Theorem 6.3) | **EXACT — HUMAN PROOF** | not a census of depth five: four words only; \(OOOO*\) excluded (level-3 kernel); \(OOOE*\) rides Theorem 6.1 at \(N^{1-1/96+\varepsilon}\); \(OOEO*\) uses the shifted-window method of Theorem 4.8 at the \(v\)-level, error \(N^{43/48+\varepsilon}\); \(X\)-modes sit in the Lemma 5.2(i) budget, not (D1); \(k=0\) on \(OOEO*\) is Theorem 4.7 |
...
| Companion audit ledger (`paper_b_audit_ledger.md`; `paper_b_audit`) | **COMPUTATIONALLY VERIFIED** (machine layer) + author re-derivation | exact identities on 360 starts at 60–120 digits; standing estimates on \(P=10^6,10^8,10^{10}\); 114 exponent checks; monomial Theorem 5.3; frozen \(\lvert B\rvert\le6\); \(\rho_0\) ratios \(O(P^{-1/4})\); Lemma 5.2(ii) from (i) written as Claims A–H; not an independent verification of the six-stage proof of (i) |
...
| What the kernel program buys and cannot (Section 8) | prose, cites Paper C | depth-\(\le3\) localization (Theorems 4.11--4.12, Corollary 4.13) is what Paper C uses; Theorem 5.3 is dyadic and is not localized; fixed-depth results improve constants (\(\lambda^{***}\), certificate densities, Tao constants) and cannot reach the termination frontier (per-depth loss budget \(2^{1/19}\)) |
