...
\(\lambda<\lambda^{**}=0.4926\ldots\), whereas
...
and backward-closed, and let \(\lambda<\lambda^{**}=0.4926\ldots\), the
root of \(2^{-\lambda}+\tfrac19(\tfrac38)^{\lambda}+\tfrac29(\tfrac34)^{\lambda}+\tfrac1{27}(\tfrac9{32})^{\lambda}+\tfrac1{81}(\tfrac{27}{128})^{\lambda}+\tfrac1{243}(\tfrac{81}{512})^{\lambda}+\tfrac1{729}(\tfrac{243}{2048})^{\lambda}+\tfrac1{2187}(\tfrac{729}{8192})^{\lambda}=1\).
...
The exponent \(0.4926\) is not the method's ceiling. Section 5.7
...
\(o((\log x)^{\lambda})\); (iii) for some \(e>1-\lambda^{**}=0.5074\ldots\)
...
\((e_i,c_i)=(\tfrac12,1),(\tfrac38,\tfrac19),(\tfrac34,\tfrac29),(\tfrac9{32},\tfrac1{27}),(\tfrac{27}{128},\tfrac1{81}),(\tfrac{81}{512},\tfrac1{243}),(\tfrac{243}{2048},\tfrac1{729}),(\tfrac{729}{8192},\tfrac1{2187})\),
so that
...
it — together with the \(OEOEE\) term \(+\tfrac1{27}g_A(9t/32)\), the
\(V_3\) term \(+\tfrac1{81}g_A(27t/128)\), the \(V_4\) term
\(+\tfrac1{243}g_A(81t/512)\), the \(V_5\) term
\(+\tfrac1{729}g_A(243t/2048)\), and the \(V_6\) term
\(+\tfrac1{2187}g_A(729t/8192)\) of
Section 5.7, has the required form with \(\eta_1=\varepsilon_1\),
\(\eta_2=\varepsilon_4\), \(\eta_3=\varepsilon_5\), and production-note
...
\(+\tfrac1{27}g_A(9t/32)\): this is the \(OEOEE\) truncation
\(0.4801\). Adding \(V_3=OEOEOEE\) at \(+\tfrac1{81}g_A(27t/128)\)
gives the \(V_3\) truncation \(0.4891\). Adding \(V_4=OEOEOEOEE\) at
\(+\tfrac1{243}g_A(81t/512)\) gives the six-term root \(0.4916\).
Adding \(V_5=OEOEOEOEOEE\) at \(+\tfrac1{729}g_A(243t/2048)\) gives
the seven-term root \(0.4924\).
Adding \(V_6=OEOEOEOEOEOEE\) at \(+\tfrac1{2187}g_A(729t/8192)\) gives
Theorem 1's exponent \(\lambda^{**}=0.4926\), and alongside
...
\(e=e(C)-\varepsilon\ge e(19)-\varepsilon=0.527-\varepsilon>0.5074\);
...
The least unconditional depth is \(C=19\) (\(e(19)=0.527>0.5074\)).
...
\(OEOEE\), \(V_3\), \(V_4\), \(V_5\) and \(V_6\) productions, \(\lambda^{**}=0.4926\); the ideal ones
...
\(1-\lambda^{**}=0.5074\) / \(V_5\)-only \(1-\lambda_{V_5}=0.5076\) / \(V_4\)-only \(1-\lambda_{V_4}=0.5084\) / \(V_3\)-only \(1-\lambda_{V_3}=0.5109\) / \(OEOEE\)-only \(1-\lambda_{\mathrm{oeoee}}=0.5199\) /
pairing \(1-\lambda_{\mathrm{pair}}=0.5520\) /
conditional \(1-\lambda^{***}=0.4608\):
fair Chernoff \(19\)/\(20\)/\(18\); one-sided Azuma at
\(q=0.5,0.55,0.60,0.62\): \(19,41,223,1586\) / \(20,44,240,1715\) /
\(18,39,206,1451\);
pressure (biased Chernoff) at the same \(q\): \(19,41,214,1496\) /
