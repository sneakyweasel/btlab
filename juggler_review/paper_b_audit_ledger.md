...
## Two sites still running on E's old constant

Widening the sweep to bare `symbol <= cP^e` flagged `V/S` at two values,
and that one is clean. `V/S = (1/12)lam^(-1/2)P^(-7/48)`, so the
coefficient is `0.107583` at `lam = 0.60` and `0.111359` at `lam = 0.56`;
the paper prints `0.11` at the Step 5a sites and `0.12` at the Step 5b
one, each rounded up, each labelled with its own `S`, and the constant
inventory names the `0.11` as "Step 5a's ratio at `S >= 0.60P^(-5/8)`".
The labels are load-bearing --- `0.11` would not cover the `0.56` end ---
and they are all correct.

**But the paragraph carrying one of those `0.11`s is not.** Step 5a
reads:

> `W = V + E` satisfy `W <= c_7S/2` at `c_7 = 1/232` from
> `P >= 1.6\cdot10^{13}` --- a lower threshold than Step 5b's, because
> `S` is larger here [...] against the interpolant error
> `106P^{-25/24}+0.11P^{-5/6}`.

`106` is `E`'s superseded constant. The paper says so itself, in three
review notes: *"E's `106 -> 170.6`. It was never cosmetic."* And the
threshold printed beside it is exactly the one that constant gives:
...
So the body and A.5 disagree on this row by `1.807`, and the
disagreement is precisely the update. The paragraph is internally
consistent --- its threshold goes with its constant --- which is why
nothing has caught it: every instrument built here compares A.5 with the
certificate, and this is the body against A.5.

**A.6's closing paragraph is the second survivor.** "The remaining slack
is in `E` itself: the middle-band half-width `60`, which enters `106`
linearly through the cap `60(2.6)/0.84`, and the range `[0.35,2.6]` for
`lambda_0`." The formula is right and every number in it is
pre-correction: `60(2.6)/0.84 = 185.71` is the old `u_cap` of `186`,
`[0.35,2.6]` is the old range, `106` is the old constant. With the
current range the same cap reads `60(4.2)/0.84 = 300` exactly --- the
`u_cap` the certificate carries --- and it enters `170.6`.

Neither reaches `P_0`. Step 5a is not the binding row; Step 5b is, at
`3.5858e13`, and `2.9117e13` sits below it either way. What changes is
the contrast the sentence draws: "a lower threshold than Step 5b's" is
true at `1.2315` with the corrected number, where the printed pair
implies `2.2249`.

Tags. EXACT: `V/S = (1/12)lam^(-1/2)P^(-7/48)`, giving `0.107583` at
`0.60` and `0.111359` at `0.56`, so the printed `0.11` and `0.12` are
each the rounded-up value at their own site and the `0.11` would not
serve at `0.56`; `60(4.2)/0.84 = 300` exactly, against the printed
`60(2.6)/0.84 = 185.71`. COMPUTATIONALLY VERIFIED: Step 5a's row first
holds at `1.6117e13` with `E = 106P^(-25/24)+0.11P^(-5/6)` and at
`2.9117e13` with `170.6`, matching the body's `1.6e13` and A.5's
`2.92e13` respectively; the ratio is `1.8066`; the certificate's row is
`2.9117e13`; `P_0` is unmoved at `3.5858e13`. OBSERVATION: `106` appears
five other times in the manuscript and every one of those is a review
note *about* the update; these two are the only live uses left.

Probe: `survivors_of_the_E_constant_update` with `E_UPDATE_SURVIVORS`,
...
## The Ostrowski assembly: Theorem 5.7's display at a length, and 5.8's comparison

*Mathematical target.* `theta_block_envelope` accepts a list of
certified convergents and bounds the ergodic sum over
\((\text{blocks}.\mathrm{map}\ \mathrm{snd}).\mathrm{sum}\) letters by
\(\text{blocks}.\mathrm{length}\cdot2\). Paper A feeds it one specific
list: the Ostrowski decomposition \(L=\sum_jb_jq_j\), a pair repeated
once per digit, so that the two readings become \(L\) and \(s(L)\). That
assembly was the last mechanical step in Theorem 5.7's chain, and
Theorem 5.8's row named it as "Denjoy--Koksma comparison human".

**It is three list lemmas and a bridge.** `OstrowskiBlocks.lean`:

- `length_flatMap_replicate`, `sum_map_snd_flatMap_replicate`,
  `mem_flatMap_replicate` --- a `flatMap` of `replicate`s, which is the
  shape "a pair repeated once per digit" has. Nothing about \(\theta\).
- `thetaPair`, `thetaPair_snd` --- the bridge. The certified pair at
  level \(j\) has second component `thetaDenomFn j`. That single
  `decide` is what lets the numeration and the envelope meet: one side
  speaks in `thetaConvergents`, the other in `thetaDenomFn`, and they
  are the same thirteen integers.
- `ostroBlocks` --- level \(i\) contributes
  `ostroDigit thetaDenomFn L 12 i` copies of the pair at level
  \(12-i\), matching the association in `theta_sum_eq`.
- `ostroBlocks_snd_sum` --- the denominators sum to \(L\). This *is*
  `theta_sum_eq`, once the pairs are rewritten to denominators.
- `ostroBlocks_length` --- the length is the digit sum \(s(L)\).
- `theta_block_envelope_of_length` --- the display, for **every** \(L\),
  with no window hypothesis: the ergodic sum of \(L\) letters is within
  \(2s(L)\) of \(L\,C_*\).
- `theta_block_envelope_window` --- and on the certified window the
  digit cap \(s(L)\le47\) makes that the constant \(94\).

**What Theorem 5.8 gets.** Its row said the Denjoy--Koksma comparison
was human. It is now `theta_block_envelope_window`:
\(|C_L-C_*|\le 94/L\) for every \(L<301994\), uniformly in the starting
phase, with the digit cap supplied structurally by `theta_digitSum_le`
rather than by a scan.

**Three guards fired, and all three were right.**

1. `test_imports_are_one_way`. `OstrowskiBlocks` imports
   `OstrowskiNumeration`, and I had inserted it *before* that module in
   the layer order. The rank comes from `lean_paths.LAYERS`, and I had
   fixed the list but not the path map --- two orderings that must agree
   and no single source. Both moved.
2. `test_the_axiom_check_covers_exactly_the_cited_declarations`. Paper A
   has its own axiom check, and it must ask about every declared
   citation and nothing else. Six new names, `theta_sum_eq` among them
   because the new prose cites it. `AxiomCheckPaperA.lean` now has 207
   lines; `.expected` regenerated by running it, not by editing.
3. `test_index_is_current_and_complete`, twice --- once for the new
   module and once for the reordering.

Human, and now the only thing Theorem 5.7 takes from its human proof:
the reading of \(C_L\) as that ergodic sum, which is Lemma 5.6's
rotation identification.

Tags. COMPUTATIONALLY VERIFIED: `lake build Problems.JugglerPaper` clean
at 3450 jobs; all six new declarations within
`[propext, Classical.choice, Quot.sound]`, and the regenerated Paper A
axiom check still shows exactly the two declared `native_decide`
consumers. `tests/tools` and `tests/research/juggler_sequence` green
apart from `test_localized_kernel`, which belongs to another session's
untracked, in-flight module and is not touched here.
