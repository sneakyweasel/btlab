...
is the minimal-certificate count  \(M_d\)  of this paper, tied to the
survivor count by  \(M_d=2N_{d-1}-N_d\) , and summing that identity
...
bounded.  It is in fact of bounded variation.  The jump amplitudes
\(a_k\)  at  \(\{k\beta\}\)  are summable: computed exactly from two
depths straddling  \(k\beta\), \(\sum_k\lvert a_k\rvert=4.58\)  over
\(k\le3000\)  at two independent anchors agreeing to  \(0.13\%\), with
every amplitude negative and the partial sums settling.  A single-exponent
fit is misleading here because the exponent steepens with the window —
\(-1.13\)  over  \(k\le69\)  against  \(-1.77\)  over  \(100\le k\le3000\)  —
so a short window reads it as nearly  \(1/k\), which is exactly the
