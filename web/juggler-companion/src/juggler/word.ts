/**
 * Word algebra. Display fork of
 * src/research/juggler_sequence/power_words.py and cycle_word.py.
 */
...
export function parseWord(text: string, max = WORD_MAX): string | null {
...
export function followsWord(n: bigint, word: string): boolean {
  let current = n;
...
export function rotateWord(word: string, shift: number): string {
  if (!word) return "";
  const n = word.length;
  const k = ((shift % n) + n) % n;
...
export function parseCycleWord(text: string): string | null {
  return parseWord(text, CYCLE_WORD_MAX);
}
