/**
 * One-step preimages. Display fork of
...
export function evenPreimage(q: number): { lo: number; hi: number } {
  if (!Number.isInteger(q) || q < 0) {
    throw new Error("evenPreimage requires a nonnegative integer");
  }
...
export function oddPreimageIntegers(m: number): number[] {
  if (!Number.isInteger(m) || m < 0) {
    throw new Error("oddPreimageIntegers requires a nonnegative integer");
  }
  const lo2 = m * m;
  const hi2 = (m + 1) * (m + 1);
  let lo = 0;
