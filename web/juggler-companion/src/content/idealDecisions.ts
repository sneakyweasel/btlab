...
export const IDEAL_DECISIONS: readonly IdealDecision[] = [
  ...
  },
  {
    ...
    kind: "theorem",
    focus: "balloon",
    title: "Every CycleMin word is O^{a₁}E ⋯ O^{aₑ}E",
    why: "Lean proves w = assembleOddEvenRuns as with e = #E, a₁ ≥ 2, and aₑ ≤ 1 (cycleMin_has_full_odd_even_run_form). The bead schema is a projection of that run list: middle odd-runs are bunched and extra evens become empty runs. Lemma 3.21b’s leftover use for e ≤ 3 stays the Paper A argument.",
    lemma: "cycleMin_has_full_odd_even_run_form, cycleMin_run_form_first_block, exists_cycleMin_last_odd_run, Lemma 3.21b",
  },
  {
    id: "balloon-seam",
    part: "cycle",
    ...
    why: "The word ends O^a E with a ≤ 1. cycleMin_has_two_seams names the exclusive 2+2 windows OE|n|OO and EE|n|OO. Trailing EE is legal when aₑ = 0 and puts the previous even below (n+1)^4; it is not a sure table link. isolated_OE_not_forced_on_shape: both windows inhabit CycleMinShape. Realized CycleMin occupation of both types is computationally verified, not Lean.",
    lemma: "cycleMin_has_two_seams, cycleMin_ee_seam_prev_lt, isolated_OE_not_forced_on_shape, cycleMin_wrap_is_EO, sureLink_iff, J-cyclemin-last-odd-run",
  },
  {
    id: "balloon-hug",
    part: "cycle",
  ...
  },
  {
    ...
    why: "The Lean candidate schema has a₁ extras 0+, middle odd-runs 0+, aₑ ∈ {0,1}, and extra E 0+. cycleMin_projects_balloonSchema is projection onto forced stations, not an assembleFill reconstruction. cycleMin_sure_letter_inventory names the six sure letters OO+EEEE. Those marks sit between the six sure letters. They are not grey ? beads and they are not forced stations.",
    lemma: "cycleMin_projects_balloonSchema, cycleMin_sure_letter_inventory, assembleFill_eq_assembleOddEvenRuns",
  },
  {
    id: "balloon-links",
    part: "cycle",
    ...
    why: "cycleMin_only_forced_adjacencies / sureLink_iff: the only forced cyclic adjacencies are launch OO and wrap EO. Trailing EE can appear when lastOdds = 0, but that adjacency is not a sure link. No other consecutive pair is always present.",
    lemma: "cycleMin_only_forced_adjacencies, sureLink_iff, cycleMin_launch_is_OO, cycleMin_wrap_is_EO",
  },
  {
    id: "balloon-fill",
    part: "cycle",
    ...
    why: "On a NecklaceFill, #O = 2+a₁+mid+last, #E = 4+extra, length = 6 plus those slots. assembleFill equals assembleOddEvenRuns of toRuns — a bunched projection. A CycleMin-shaped word such as O³EO²EO²EE has runs [3,2,2,0] and is not a fill (leftover_three_valley_not_fill_runs). There is no Lean map CycleMin → NecklaceFill.",
    lemma: "assembleFill_oddCount, assembleFill_evenCount, assembleFill_length, assembleFill_eq_assembleOddEvenRuns, leftover_three_valley_not_fill_runs",
  },
  {
    id: "balloon-parity",
    part: "cycle",
  ...
  },
  {
    ...
    focus: "balloon-fade",
    why: "O⁷EEEE inhabits CycleMinShape and is not a cycle itinerary for any n (CycleMinShape_not_of_CycleMin). O⁶EEEOE and the (1,3) EEE family do the same. leftover_fudge_not_cycleMin and leftover_four_even_slack live on those spellings, not on the ideal circle. The 56-word necklace pin is refuted: OOEEEOOOOOE and OOOEEEOOOOE have last odd-run ≥ 2, so they are outside CycleMinShape and NecklaceFill.admits (necklace_pin_misses_not_CycleMinShape). There is no no_cycleMin_four_even. Last-cluster stays a human proof.",
    lemma: "CycleMinShape_not_of_CycleMin, leftover_fudge_not_cycleMin, leftover_four_even_slack, leftover_slack139, necklace_pin_misses_not_CycleMinShape, J-o7eeee-gap, J-o6eeeoe-gap, J-one-three-eee-gap, J-cyclemin-fudge, J-cyclemin-slack, J-cyclemin-necklace, J-cyclemin-last-cluster",
  },
  {
...
export const HARVESTED_LEMMA_NEEDLES = [
  ...
  "isolated_OE_not_forced_on_shape",
  ...
  "assembleFill_eq_assembleOddEvenRuns",
  "CycleMinShape_not_of_CycleMin",
  "leftover_fudge_not_cycleMin",
  "leftover_four_even_slack",
