...
Theorem 5.3 at exponent \(3/10\), unconditional: two productions, no
hypothesis & Lean \\
The side condition \(e(C)>\tfrac7{10}\) of the unconditional criteria,
at \(C=30\), by rational bounds on \(e\), \(\log 2\) and \(\log_2 3\) &
Lean; the least such \(C\) (\(23\)) and the paper's own threshold
through \(\lambda^{**}\) stay with the audit \\
...
or with nothing else assumed when \(e^{\rm Ch}_q(C)>\tfrac7{10}\) &
...
nothing else assumed when the exponent exceeds \(\tfrac7{10}\) & Lean;
the paper's \(e^{o(d)}\) is quantified as \((\log y)^\varepsilon\) and
its \(o(d)\) as \(\delta d\), the numerical forms are human \\
...
when the exponent exceeds \(\tfrac7{10}\) & Lean; the condition is
...
\(2^{-3/10}+\tfrac29(\tfrac34)^{3/10}>1\) certified by
\(0.81^{10}\le\tfrac18\) and \(0.91^{10}\le\tfrac{27}{64}\)
...
root of \(2^{-\lambda}+\tfrac29(\tfrac34)^\lambda=1\) is about
\(0.325\); what lifts it to \(\lambda^{**}\) is the block-average
family, which needs the two exponential-sum bounds of Proposition 4.4,
and the ladder (5.10), which needs Appendix D. Composed with Theorem 7.2
and Corollary 8.4, whose contagion hypothesis it discharges, this gives
two statements with no hypothesis on the contagion side: a Tao-type rate
...
cylinder bound \(\mathrm H(C,A)\) with \(e(C)>\tfrac7{10}\)
...
least \(C\) with \(e(C)>\tfrac7{10}\) is \(23\), and the paper's own
threshold \(e(C)>1-\lambda^{**}\) at \(C=19\) is a statement about
...
hypothesis at all large scales, with its exponent above \(\tfrac7{10}\),
gives the conjecture with nothing else assumed. The numerical forms are
not formalized.
...
\(B>C\log_2x+1+e\) and \(\tfrac7{10}<e<e^{\rm Ch}_q(C)\), and nothing
...
give Theorem 5.3 at exponent 3/10 unconditionally; the block-average
...
\item
  ...
  SHA-256:
  \texttt{\nolinkurl{8725a2660a6ab279b2b1c23a831053fed07064aae53a6c0eb2f306f2ab01cdcb}}
\item
