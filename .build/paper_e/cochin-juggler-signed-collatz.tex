...
bound in the modulus; this quantitative extension is a written proof
pending independent review and full Lean verification. On the positive
integers, we prove that every \(3n-1\) target prime to three has at
least \(X^{21/25}\) ancestors below \(X\), for all sufficiently large
\(X\). This signed adaptation uses height-corrected inverse-tree
inequalities, a closed root domain, and an exact integer certificate
with 177147 rows. A mean inequality places every positive certificate in
the fixed \(1/50\) grid, for either sign and at every finite residue
level, strictly below exponent \(0.99\) in the at-most-linear range.
Lean formalizations accompany the orbit, period, counting, grid-ceiling,
and modular-return results, including the latter's analytic recurrence
...
This is the living preprint, version 0.6.0. It has not been deposited or
independently refereed. Mathematical priority for the signed adaptation
and the isolated obstruction results remains subject to specialist
review.

Theorems 2.1, 3.2, 4.1, 5.1, and 6.1 and Corollaries 4.2-4.3 have
compiled Lean statements; Appendix B identifies their precise scope.
Theorem 4.1 is unconditional in Lean: first-derivative estimates,
mixed-power cancellation, and the Fourier criterion prove its
simultaneous-box recurrence input. Theorem 4.4 and Appendix C are a
separate quantitative extension with a written proof. They use the
explicitly cited derivative estimate {[}AR24{]}; their complete counting
and witness conclusions are not in the selected Lean audit. The finite
certificate is checked with exact integers both in Lean and by an
independent Python verifier. The numerical search that found its weights
is outside the proof. Kernel checking, agreement between prose and
formal statements, and independent mathematical review are distinct
checks.
...
\textbf{Theorem 4.4 (effective OOE returns; written proof).} For every
\(M,T\ge1\), \[
\begin{split}
...
the large-denominator family of Theorem 4.1 effective. Its written
analytic proof has passed an internal audit, but independent review and
complete quantitative Lean verification remain outstanding.
...
A lower bound \(N(X)\gg X^\kappa\) with \(\kappa<1\) alone cannot force
divergent reciprocal mass. For example the distinct integers
\(\lfloor j^{1/\kappa}\rfloor\) have count at least
...
build tools, and outputs. This edition makes no claim of an existing
external deposit.
...
Example 2.2 & \texttt{\nolinkurl{CollatzPadic.code_three_cleared}},
\texttt{\nolinkurl{code_not_injective}} \\
...
AxiomCheckJugglerCollatzPaper.lean prints the dependencies of the 49
...
This appendix proves Theorem 4.4. Its analytic proof is written
mathematics; the selected 49-declaration audit covers the earlier
qualitative results. The finite Fejer estimate has a separate
kernel-checked proof, but that does not certify the complete
quantitative theorem.

\subsection{C.2 Explicit analytic
input}\label{c.2-explicit-analytic-input}

Write e(x)=exp(2\emph{pi}i*x). We use J. Arias de Reyna {[}AR24{]},
Theorem 11 and Table 1, only at derivative orders k=3 and k=5. In
notation adapted to this proof, if floor(Y)\textgreater k, f has
continuous derivatives through order k on (X,X+Y{]}, and its k-th
derivative lies between positive lambda and Lambda, then, with D=2\^{}k,
\[
...
project. Our phases are smooth for x\textgreater0, and every retained
interval has Y=N\textgreater=6, so floor(Y)\textgreater5\textgreater=k.
The normalization is by the real interval length Y, not by the number of
integer summands.
...
Let H\textgreater=1 be an integer and let integers h\_1,h\_2 satisfy
0\textless max(\textbar h\_1\textbar,\textbar h\_2\textbar)\textless=H.
Consider \[
f_h(x)=\frac{h_1}{2}(1+2Mx)^{9/2}
       +\frac{h_2}{2M}(1+2Mx)^{9/4}. \tag{C.6}
\] On a real interval N\textless x\textless=2N with
N\textgreater=max(H\^{}2,6), \[
...
If h\_1 is nonzero, the fifth derivative is \[
f_h^{(C.5)}(x)=\frac{945}{2}h_1M^5s^{-1/2}
             +\frac{945}{64}h_2M^4s^{-11/4},\qquad s=1+2Mx.
\] The absolute ratio of the second term to the first is at most
H\emph{s\^{}(-9/4)/(32M)\textless=1/32, using
\textbar h\_1\textbar\textgreater=1, N\textgreater=H, and
s\textgreater=2MN. The derivative therefore has the sign of h\_1
...
\] Indeed 945/(4}sqrt(C.5))\textgreater100 and 2835/(4\emph{sqrtthe
parameter bound in Theorem 4.4)\textless600. Substitution into (C.5),
with k=5, gives \[
...
11}(6/N)\^{}(1/16),
11\emph{(3600}H*M\textsuperscript{(9/2)\emph{N\textsuperscript{(-1/2))}(1/30),
and 11}(100\emph{M\^{}(9/2)}N}(9/2))\^{}(-1/16). The scalar inequalities
6\textless2\^{}16 and 3600\textless2\^{}30 suffice for the constant 32.

\subsection{Vanishing high-power
coefficient}\label{vanishing-high-power-coefficient}

If h\_1=0, then h\_2 is nonzero and \[
f_h^{the start bound in Theorem 4.4}(x)=\frac{45}{16}h_2M^2s^{-3/4}.
\] Use \[
\lambda=\tfrac12|h_2|M^{5/4}N^{-3/4},\qquad
\Lambda=2|h_2|M^{5/4}N^{-3/4}.
\] The endpoint comparisons follow from
45/(16\emph{5\^{}(3/4))\textgreater1/2 and 45/(16}2\^{}(3/4))\textless2.
Estimate (C.5), now with k=3, bounds the normalized sum by
22\emph{H\textsuperscript{(1/6)\emph{M\textsuperscript{(5/24)\emph{N\^{}(-1/8).
The three terms before enlargement are at most 11}(4/N)}(1/4),
11}(8\emph{H}M}(5/4)}N\textsuperscript{(-3/4))}(1/6), and
11\emph{((1/2)}M\^{}(5/4)*N\textsuperscript{(9/4))}(-1/4). Each is
bounded by the displayed common expression using H,M,N\textgreater=1.
Since H\^{}2\textless=N, \[
H^{1/6}N^{-1/8}
\le H^{1/30}N^{-7/120}
\le H^{1/30}N^{-1/60}.
...
Suppose 1\textless=H\textless=T\^{}(1/4). Split (0,T{]} into intervals
(N,2N{]} with N=T/2, T/4, \ldots{} while N\textgreater=max(H\^{}2,6).
The remaining initial interval is (0,R{]}, where
R\textless2*max(H\^{}2,6); take R=T if no interval is retained. The
intervals are disjoint as half-open sets, even at noninteger endpoints.
The remainder has floor(R) integer summands. The change from indices
1,\ldots,T to 0,\ldots,T-1 is exactly e(f\_h(C.0))-e(f\_h(T)), of norm
at most 2. Their total contribution is at most
floor(R)+2\textless=2H\textsuperscript{2+14\textless=16H}2, including
when no interval is retained. Moreover, \[
\sum_{j\ge0}(T/2^{j+1})^{59/60}
=\frac{T^{59/60}}{2^{59/60}-1}<2T^{59/60}.
\] Since H\textsuperscript{2\textless=T}(1/2), (C.10) and the initial
remainder imply \[
...
For arbitrary points z\_0,\ldots,z\_(T-1) on the two-dimensional unit
torus, let E\_H bound the normalized sums of every nonzero Fourier mode
with max(\textbar h\_1\textbar,\textbar h\_2\textbar)\textless=H. For
every half-open product of circular intervals B, \[
...
1-\textbar h\textbar/(H+1) for \textbar h\textbar\textless=H and zero
otherwise. For 0\textless delta\textless=1/2,
sin(pi\emph{x)\textgreater=2}x on 0\textless=x\textless=1/2 gives \[
\int_{\|x\|\ge\delta}F_H(x)\,dx
\le\frac1{2(H+1)\delta}.
\] The product kernel K=F\_H tensor F\_H therefore has mass at most
q=1/((H+1)\emph{delta) outside the coordinate delta-neighbourhood of
zero. Expand and contract each interval of B by delta on the circle,
obtaining B\^{}+ and B\^{}-. A contracted interval can be empty and an
expanded one full; both cases are retained. More precisely, write a
proper circular interval as the image of {[}a,a+ell), with
0\textless ell\textless1. Its expansion is the image of
{[}a-delta,a+ell+delta) if ell+2}delta\textless1, and the whole circle
otherwise. Its contraction is the image of {[}a+delta,a+ell-delta) if
ell\textgreater2\emph{delta, and empty otherwise. Empty and full
original intervals are left unchanged. For shifts y with each circular
distance \textbar\textbar y\_i\textbar\textbar\textless delta, these
definitions give z in B implies z-y in B\^{}+, and z-y in B\^{}- implies
z in B. The exceptional shifts have K-mass at most q. Since
0\textless=1\_B}K\textless=1, the following inequalities hold at every
point, including the original interval endpoints: \[
\mathbf1_{B^-}*K-q\le\mathbf1_B\le\mathbf1_{B^+}*K+q.
\] The changes in volume are at most 4\emph{delta. A circular interval's
Fourier coefficient has absolute value at most
...
4}delta+q+(3+2\emph{log(H))\^{}2}E\_H. Take delta=(H+1)\^{}(-1/2) for
H\textgreater=3. For H=1 or 2, the trivial discrepancy bound one already
implies (C.12).
...
\] Its area is 1/(4M). For any x\textgreater=0 and integers
m\textgreater=1 and 0\textless=r\textless m, writing floor(x)=m*q+r'
with 0\textless=r'\textless m gives fract(x/m)=(r'+fract(x))/m.
Consequently floor(x)=r modulo m is equivalent to
r/m\textless=fract(x/m)\textless(r+1)/m, including equality at the left
endpoint. Box membership is therefore exactly u even and v=1 modulo 2M.
For M=1 the second interval is {[}1/2,1): its right endpoint is
excluded; an integer value of x/(2M) has fractional part zero and is
rejected. The first interval likewise excludes its endpoint 1/2. No
absence of boundary hits is assumed. Choose H=floor(T\^{}(1/32)), so
H\textgreater=1, H\textless=T\^{}(1/4), and \[
(H+1)^{-1/2}\le T^{-1/64},\qquad
H^{1/30}T^{-1/60}\le T^{-1/64},\qquad
2\log H\le\frac{\log T}{16}.
\] Equations (C.11) and (C.12) give the first bound in (4.6) before the
final +8. Exactly min(T,1+floor(7/M)) parameters have 1+2Mt\textless16.
Deleting the box hits among these parameters changes the count by at
most eight.

For the second bound in (4.6), put x=log(T)\textgreater=0. The maximum
of \[
(3+x/16)^2e^{-x/128}
\] on this half-line is 256\emph{exp(-13/8), attained at x=208, and is
less than 64. For example, the first four terms of the exponential
series already give exp(13/8)\textgreater4. Absorb 8/T into
8}T\^{}(-1/128) and 5\emph{T\^{}(-1/64) into 5}T\^{}(-1/128). Since
M\textgreater=1 and 5+128*64+8=8205\textless2\^{}14, the second
inequality follows.

At T=2\textsuperscript{(C.2176)M}(C.160), one has
T\textsuperscript{(1/128)=2}17*M\^{}(5/4). Hence the error is at most
T/(8M), while the main term is T/(4M). Thus A\_M(T)\textgreater=
T/(8M)\textgreater0. Select a counted t. Since t\textless T is integral,
1+2Mt\textless2MT, giving the parameter bound in Theorem 4.4 and the
start bound in Theorem 4.4.

The actual orbit is \[
s^2\longmapsto s^3\longmapsto u\longmapsto v.
\] Both initial sources are odd, the third is even by construction, and
the exact identity isqrt(isqrt(s\textsuperscript{9))=floor(s}(9/4))
identifies the exit. The already proved size threshold s\textgreater=16
gives v\textgreater s\^{}2; the first two images are larger still.
Finally s=1 modulo 2M gives n=1 modulo 2M, and the box gives the exit
congruence of Theorem 4.4. This completes the written proof.
