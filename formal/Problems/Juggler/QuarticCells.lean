...
/-- Compatibility name for the shared logarithmic cell capacity. -/
noncomputable abbrev logEta := LogCells.logEta

theorem logEta_pos {v : ℝ} (hv : 1 < v) : 0 < logEta v :=
  LogCells.logEta_pos hv

theorem logEta_antitone {u v : ℝ} (hu : 1 < u) (huv : u ≤ v) :
    logEta v ≤ logEta u :=
  LogCells.logEta_antitone hu huv

theorem logEta_lt_inv {v : ℝ} (hv : 1 < v) :
    logEta v < 1 / (v * Real.log v) :=
  LogCells.logEta_lt_inv hv
