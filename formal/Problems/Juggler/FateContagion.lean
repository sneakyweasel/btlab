...
/-! ### Forward closure and odd generation

The fate classes are also forward-closed.  A class that is closed both
ways and excludes `1` is generated by its odd members: every member
descends along even steps to an odd member `≥ 3` of the class, and the
odd members are exactly the odd preimages of the class's intersection
with the image set `{⌊m^{3/2}⌋ : m odd}`.  Hence such a class is empty
iff it contains no image of an odd integer — the exact form of the
"S-fairness" reformulation in the fate-contagion note. -/

/-- `A` is closed under one forward step. -/
def ForwardClosed (A : ℕ → Prop) : Prop :=
  ∀ n, A n → A (floorPower n)

theorem reachesOne_forwardClosed : ForwardClosed ReachesOne :=
  fun _ h => reachesOne_floorPower h

theorem not_reachesOne_forwardClosed : ForwardClosed fun n => ¬ReachesOne n :=
  fun _ h hn => h (reachesOne_of_iterate (k := 1) rfl hn)

theorem escapes_forwardClosed : ForwardClosed EscapesToInfinity :=
  fun _ h => escapes_floorPower h

theorem mem_iff_floorPower_mem {A : ℕ → Prop} (hF : ForwardClosed A)
    (hB : BackwardClosed A) (n : ℕ) : A n ↔ A (floorPower n) :=
  ⟨hF n, hB n⟩

/-- Odd generation: in a forward-closed class, every member `n ≥ 1` reaches an
odd member of the class through even steps only. -/
theorem exists_odd_ancestor {A : ℕ → Prop} (hF : ForwardClosed A) :
    ∀ n, 1 ≤ n → A n →
      ∃ k, (floorPower^[k] n) % 2 = 1 ∧ A (floorPower^[k] n) ∧
        ∀ j < k, (floorPower^[j] n) % 2 = 0 := by
  intro n
  ...
  | _ n ih =>
    ...
    · have hn2 : 2 ≤ n := by omega
      have hlt := floorPower_even_lt hn2 he
      obtain ⟨k, hk1, hk2, hk3⟩ := ih (floorPower n) hlt (floorPower_pos hn) (hF n hA)
      refine ⟨k + 1, ?_, ?_, ?_⟩
      · rw [Function.iterate_succ_apply]; exact hk1
      ...
      · intro j hj
        ...
        | succ j =>
            rw [Function.iterate_succ_apply]
            exact hk3 j (by omega)
    · exact ⟨0, by simpa using ho, by simpa using hA, fun j hj => absurd hj (Nat.not_lt_zero j)⟩

/-- If the class excludes `1`, the odd ancestor is at least `3`. -/
theorem exists_odd_ancestor_ge_three {A : ℕ → Prop} (hF : ForwardClosed A) (h1 : ¬A 1)
    {n : ℕ} (hn : 1 ≤ n) (hA : A n) :
    ∃ m, m % 2 = 1 ∧ 3 ≤ m ∧ A m ∧ Ancestor n m := by
  obtain ⟨k, hk1, hk2, -⟩ := exists_odd_ancestor hF n hn hA
  refine ⟨floorPower^[k] n, hk1, ?_, hk2, ⟨k, rfl⟩⟩
  have hpos : 1 ≤ floorPower^[k] n := floorPower_iterate_pos hn k
  have hne : floorPower^[k] n ≠ 1 := fun h => h1 (h ▸ hk2)
  omega

/-- A class closed both ways and excluding `1` is nonempty iff it contains the
image `⌊m^{3/2}⌋ = Nat.sqrt (m^3)` of some odd `m ≥ 3`. -/
theorem nonempty_iff_odd_image_mem {A : ℕ → Prop} (hF : ForwardClosed A)
    (hB : BackwardClosed A) (h1 : ¬A 1) :
    (∃ n, 1 ≤ n ∧ A n) ↔ ∃ m, m % 2 = 1 ∧ 3 ≤ m ∧ A ((m ^ 3).sqrt) := by
  constructor
  · rintro ⟨n, hn, hA⟩
    obtain ⟨m, hodd, hm3, hAm, -⟩ := exists_odd_ancestor_ge_three hF h1 hn hA
    refine ⟨m, hodd, hm3, ?_⟩
    have := hF m hAm
    rwa [floorPower_odd_eq hodd] at this
  · rintro ⟨m, hodd, hm3, hAm⟩
    refine ⟨m, by omega, hB m ?_⟩
    rwa [floorPower_odd_eq hodd]

/-- The odd members of a class closed both ways are exactly the odd preimages
of its members: for odd `n`, `n ∈ A ↔ ⌊n^{3/2}⌋ ∈ A`. -/
theorem odd_mem_iff {A : ℕ → Prop} (hF : ForwardClosed A) (hB : BackwardClosed A)
    {n : ℕ} (hodd : n % 2 = 1) : A n ↔ A ((n ^ 3).sqrt) := by
  rw [← floorPower_odd_eq hodd]
  exact mem_iff_floorPower_mem hF hB n
