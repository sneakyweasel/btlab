...
/-!
# Hausdorff size of the infinite-density set at every slope

Infinite density is equivalent to membership in arbitrarily late rescaled
jump intervals. The three-halves bound on the actual crossing weights gives
zero Hausdorff measure at every exponent above two-thirds, for every
irrational `0<β<1`. No lower bound is asserted.
-/
...
omit hβ0 hβ1 hβ in
private theorem top_mem_tail {y : ℝ} (hy : passageGammaDensity β y = ∞)
    (N : ℕ) : y ∈ passageGammaTail β N := by
  classical
  by_contra hn
  have hz (r : ℕ) (hr : r ∉ Finset.range N) :
      (Ioo (passageGammaJumpLeft β (r+1)) (passageGammaJumpRight β (r+1))).indicator
        (fun y => ENNReal.ofReal (1/((-Real.log (1-β))*y))) y = 0 := by
    apply indicator_of_notMem
    intro hyr
    apply hn
    exact mem_iUnion₂.mpr ⟨r, by simpa using hr, hyr⟩
  have hfin : passageGammaDensity β y ≠ ∞ := by
    rw [passageGammaDensity, tsum_eq_sum hz]
    apply ENNReal.sum_ne_top.mpr
    intro r _
    by_cases hr : y ∈ Ioo (passageGammaJumpLeft β (r+1)) (passageGammaJumpRight β (r+1))
    · rw [indicator_of_mem hr]; exact ENNReal.ofReal_ne_top
    · rw [indicator_of_notMem hr]; exact ENNReal.zero_ne_top
  exact hfin hy

/-- The blowup set `passageGammaBlowupSet` is the entire infinite-density
set of the canonical interval-series representative, not just a subset. -/
theorem passageGammaBlowupSet_eq_top :
    passageGammaBlowupSet β = {y | passageGammaDensity β y = ∞} := by
  apply Subset.antisymm (passageGammaBlowup_subset_top hβ0 hβ1 hβ)
  intro y hy
  have ht := top_mem_tail hy
  obtain ⟨r,_,hyr⟩ := mem_iUnion₂.mp (ht 0)
  have hl := (passageGammaJump_mem_support hβ0 hβ1 hβ r).1
  have hu := (passageGammaJump_mem_support hβ0 hβ1 hβ r).2
  rw [passageGammaLaw_support_eq_Icc hβ0 hβ1 hβ] at hl hu
...
theorem passageGammaDensity_top_hausdorff {s : ℝ} (hs : 2/3 < s) :
    Measure.hausdorffMeasure s {y | passageGammaDensity β y = ∞} = 0 := by
  have hs0 : 0 < s := by linarith
  obtain ⟨a,b,ha,hb,hw⟩ := passageWeight_three_halves hβ0 hβ1 hβ
  let v : ℕ → ℝ := fun n => b*((n : ℝ)+1)^(-3/2 : ℝ)
  let J : ℕ → Set ℝ := fun r =>
    Ioo (passageGammaJumpLeft β (r+1)) (passageGammaJumpRight β (r+1))
  have hv (n : ℕ) : 0 < v n := mul_pos hb (Real.rpow_pos_of_pos (by positivity) _)
  have hdiam (n : ℕ) : ediam (J n) ≤ ENNReal.ofReal (v n) := by
    dsimp only [J]
    rw [Real.ediam_Ioo]
    apply ENNReal.ofReal_le_ofReal
    apply (passageGammaJump_length_le hβ0 hβ1 n).trans
  ...
  have hcover (n : ℕ) : {y | passageGammaDensity β y = ∞} ⊆ ⋃ k, J (k+n) := by
    intro y hy
    obtain ⟨r,hr,hyr⟩ := mem_iUnion₂.mp (top_mem_tail hy n)
    exact mem_iUnion.mpr ⟨r-n, by simpa only [Nat.sub_add_cancel hr] using hyr⟩
  have h := Measure.hausdorffMeasure_le_liminf_tsum s
    {y | passageGammaDensity β y = ∞} _ hvzero (fun n k => J (k+n))
...
theorem passageGammaDensity_top_dimH_le :
    dimH {y | passageGammaDensity β y = ∞} ≤ (2/3 : ℝ≥0∞) := by
  apply dimH_le
  intro d hd
  by_contra hn
  have hreal : (2/3 : ℝ) < (d : ℝ) := by
    ...
    simpa using h
  have hz := passageGammaDensity_top_hausdorff hβ0 hβ1 hβ hreal
  rw [hz] at hd
