...
/-!
# Quantitative concentration of the Gamma limiting law

The logarithmic jump-interval representation and the three-halves length
bound control the mass of every measurable set by the cube root of its
Lebesgue measure. No spacing or independence hypothesis is used.
-/
...
/-- The length of a rescaled jump is no greater than the original weight. -/
theorem certificateAmplitudeJump_length_le (r : ℕ) :
    certificateAmplitudeJumpRight (r+1)-certificateAmplitudeJumpLeft (r+1) ≤
      certificateWeight (r+1) := by
  have hp : (1-beta)^(certificatePhase (r+1)) ≤ 1 :=
    Real.rpow_le_one q_pos.le (by linarith [beta_gt_five_eighths])
      (certificatePhase_mem_Ico (r+1)).1
  calc
    _ = (1-beta)^(certificatePhase (r+1))*certificateWeight (r+1) := by
      unfold certificateAmplitudeJumpRight certificateAmplitudeJumpLeft
      ring
    _ ≤ 1*certificateWeight (r+1) :=
      mul_le_mul_of_nonneg_right hp (certificateWeight_nonneg _)
    _ = _ := one_mul _
...
private theorem certificatePassageLaw_le_truncated {s : Set ℝ} (hs : MeasurableSet s)
    (hfin : volume s ≠ ∞) :
    (certificatePassageLaw : Measure ℝ) s ≤
      ENNReal.ofReal ((1/((-Real.log (1-beta))*(1-beta)))*
        ∑' r, min (certificateWeight (r+1)) (volume s).toReal) := by
  have hn (r : ℕ) : 0 ≤ min (certificateWeight (r+1)) (volume s).toReal :=
    le_min (certificateWeight_nonneg _) ENNReal.toReal_nonneg
  have hsumm : Summable (fun r => min (certificateWeight (r+1)) (volume s).toReal) :=
    Summable.of_nonneg_of_le hn (fun _ => min_le_left _ _)
      certificate_jump_weights_hasSum.summable
  rw [certificatePassageLaw_eq_sum_logIntervalMeasure, Measure.sum_apply _ hs]
  calc
    _ ≤ ∑' r, ENNReal.ofReal (1/((-Real.log (1-beta))*(1-beta)))*
        ENNReal.ofReal (min (certificateWeight (r+1)) (volume s).toReal) := by
      apply ENNReal.tsum_le_tsum
      intro r
      apply (logIntervalMeasure_le_min scale_pos q_pos
        (certificateAmplitudeJump_bounds r).1 hs).trans
      gcongr
      rw [ENNReal.ofReal_min, ENNReal.ofReal_toReal hfin, min_comm]
      exact min_le_min (ENNReal.ofReal_le_ofReal (certificateAmplitudeJump_length_le r)) le_rfl
    _ = _ := by
      rw [ENNReal.tsum_mul_left, ← ENNReal.ofReal_tsum_of_nonneg hn hsumm,
        ← ENNReal.ofReal_mul (one_div_pos.mpr (mul_pos scale_pos q_pos)).le]
...
theorem certificatePassageLaw_concentration :
  ...
  let k : ℝ := 1/((-Real.log (1-beta))*(1-beta))
  have hk : 0 < k := one_div_pos.mpr (mul_pos scale_pos q_pos)
  refine ⟨max 1 (k*(2+2*b)), lt_of_lt_of_le zero_lt_one (le_max_left _ _), ?_⟩
  intro s hs hfin
  by_cases hz : volume s = 0
  · rw [certificatePassageLaw_eq_withDensity, withDensity_apply _ hs, hz, ENNReal.toReal_zero]
  ...
  · have he (n : ℕ) : ((n : ℝ)+1)^(-3/2 : ℝ) = (((n : ℝ)+1)^(3/2 : ℝ))⁻¹ := by
    ...
    apply (certificatePassageLaw_le_truncated hs hfin).trans
    apply ENNReal.ofReal_le_ofReal
    calc
      _ ≤ k*((2+2*b)*(volume s).toReal^(1/3 : ℝ)) :=
        mul_le_mul_of_nonneg_left
          (truncated_sum_three_halves_bounds certificate_jump_weights_hasSum.summable
...
private theorem cdf_increment_le {C : ℝ} (hC : 0 < C)
    ...
    ProbabilityTheory.cdf (certificatePassageLaw : Measure ℝ) y -
      ProbabilityTheory.cdf (certificatePassageLaw : Measure ℝ) x ≤ C*(y-x)^(1/3 : ℝ) := by
...
theorem certificatePassageLaw_cdf_holder :
    ∃ C : ℝ, 0 < C ∧ ∀ x y : ℝ,
      |ProbabilityTheory.cdf (certificatePassageLaw : Measure ℝ) y -
        ProbabilityTheory.cdf (certificatePassageLaw : Measure ℝ) x| ≤
          C*|y-x|^(1/3 : ℝ) := by
  ...
  · rw [abs_of_nonneg (sub_nonneg.mpr
    ...
    exact cdf_increment_le hC hc hxy
  · rw [abs_sub_comm, abs_sub_comm y x,
    ...
    exact cdf_increment_le hC hc hyx

private theorem density_superlevel_finite {T : ℝ} (hT : 0 < T) :
    volume {y | ENNReal.ofReal T < certificatePassageDensity y} ≠ ∞ := by
  apply ne_of_lt
  calc
    _ ≤ volume {y | ENNReal.ofReal T ≤ certificatePassageDensity y} :=
      measure_mono fun _ (hy : ENNReal.ofReal T < _) => hy.le
    _ ≤ (∫⁻ y, certificatePassageDensity y)/ENNReal.ofReal T :=
      meas_ge_le_lintegral_div certificatePassageDensity_measurable.aemeasurable
        (ENNReal.ofReal_pos.mpr hT).ne' ENNReal.ofReal_ne_top
    _ < ∞ := by rw [certificatePassageDensity_lintegral]; simp [hT]

private theorem density_superlevel_lower (T : ℝ) :
    ENNReal.ofReal T * volume {y | ENNReal.ofReal T < certificatePassageDensity y} ≤
      (certificatePassageLaw : Measure ℝ) {y | ENNReal.ofReal T < certificatePassageDensity y} := by
  have hs : MeasurableSet {y | ENNReal.ofReal T < certificatePassageDensity y} :=
    measurableSet_lt measurable_const certificatePassageDensity_measurable
  rw [certificatePassageLaw_eq_withDensity, withDensity_apply _ hs, ← setLIntegral_const]
  apply lintegral_mono_ae
...
theorem certificatePassageDensity_weak_three_halves :
    ∃ D : ℝ, 0 < D ∧ ∀ T : ℝ, 0 < T →
      volume {y | ENNReal.ofReal T < certificatePassageDensity y} ≤
        ENNReal.ofReal (D*T^(-3/2 : ℝ)) := by
  obtain ⟨C,hC,hc⟩ := certificatePassageLaw_concentration
  refine ⟨C^(3/2 : ℝ), Real.rpow_pos_of_pos hC _, fun T hT => ?_⟩
  let s := {y | ENNReal.ofReal T < certificatePassageDensity y}
  have hs : MeasurableSet s := measurableSet_lt measurable_const certificatePassageDensity_measurable
  have hfin : volume s ≠ ∞ := density_superlevel_finite hT
  by_cases hz : volume s = 0
  · change volume s ≤ _
    rw [hz]; exact bot_le
  have hm : 0 < (volume s).toReal := ENNReal.toReal_pos hz hfin
  have h := (density_superlevel_lower T).trans (hc s hs hfin)
