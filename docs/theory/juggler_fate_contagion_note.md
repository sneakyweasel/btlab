...
\qquad(\lambda<\lambda^{**}=0.4926\ldots),
\]
...
\[
2^{-\lambda}+\tfrac19\bigl(\tfrac38\bigr)^{\lambda}+\tfrac29\bigl(\tfrac34\bigr)^{\lambda}+\tfrac1{27}\bigl(\tfrac9{32}\bigr)^{\lambda}+\tfrac1{81}\bigl(\tfrac{27}{128}\bigr)^{\lambda}+\tfrac1{243}\bigl(\tfrac{81}{512}\bigr)^{\lambda}+\tfrac1{729}\bigl(\tfrac{243}{2048}\bigr)^{\lambda}+\tfrac1{2187}\bigl(\tfrac{729}{8192}\bigr)^{\lambda}=1 .
\]
...
\(0.4891\ldots\), the \(V_4\) truncation is \(0.4916\ldots\), the \(V_5\) truncation is
\(0.4924\ldots\), and
\(\lambda^{**}=0.4926\ldots\).
...
*Proof.* Set
\(\zeta=2^{-\lambda}+\tfrac19(\tfrac38)^\lambda+\tfrac29(\tfrac34)^\lambda+\tfrac1{27}(\tfrac9{32})^\lambda+\tfrac1{81}(\tfrac{27}{128})^\lambda+\tfrac1{243}(\tfrac{81}{512})^\lambda+\tfrac1{729}(\tfrac{243}{2048})^\lambda+\tfrac1{2187}(\tfrac{729}{8192})^\lambda-1>0\).
...
the \(V_5\) production (Proposition 14) adds \(+\tfrac1{729}g_A(243t/2048)\),
and the \(V_6\) production (Proposition 16) adds \(+\tfrac1{2187}g_A(729t/8192)\). By Lemma 4.1 and the
decay of the errors, choose \(t_1\) with
\(\tfrac{729}{8192}t_1\ge 4\log(m+1)\) such that for all \(t\ge t_1\) the
...
\(t\ge\tfrac{729}{8192}t_1\). For \(t\in[\tfrac{729}{8192}t_1,t_1]\) this is
Lemma 4.1 (\(Kt^\lambda\le c_A\)). The first induction step covers up
to \(t_1/(\tfrac34)=\tfrac43 t_1\), at which
\(\tfrac{729}{8192}t=\tfrac{243}{2048}t_1\). Suppose the bound holds on
\([\tfrac{729}{8192}t_1,T]\) with \(T\ge t_1\) and let
\(t\in(T,\tfrac43 T]\). Then
\(t/2,\ 3t/8,\ 3t/4,\ 9t/32,\ 27t/128,\ 81t/512,\ 243t/2048,\ 729t/8192\in[\tfrac{729}{8192}t_1,T]\), so
\[
...
\((\tfrac43)^NT\) covers all \(t\ge\tfrac{729}{8192}t_1\). Finally
...
on monotone fibers; the remaining depth-two gap \(0.4926\to 0.4927\)
...
The \(0.4926\to 0.4927\) gap is only one of the two levers, and it is
...
depth-two gap \(0.4926\to 0.4927\) is the later \(V_k\) truncations
