...
\(\lambda<\lambda^{**}=0.4924\ldots\), whereas
...
and backward-closed, and let \(\lambda<\lambda^{**}=0.4924\ldots\), the
root of \(2^{-\lambda}+\tfrac19(\tfrac38)^{\lambda}+\tfrac29(\tfrac34)^{\lambda}+\tfrac1{27}(\tfrac9{32})^{\lambda}+\tfrac1{81}(\tfrac{27}{128})^{\lambda}+\tfrac1{243}(\tfrac{81}{512})^{\lambda}+\tfrac1{729}(\tfrac{243}{2048})^{\lambda}=1\).
(The \(V_4\) truncation of the first six terms is \(0.4916\); the \(V_3\) truncation of the first five is \(0.4891\); the \(OEOEE\) truncation of the first four is \(0.4801\); the pairing-only root of the first three is \(0.4480\).)
...
The exponent \(0.4924\) is not the method's ceiling. Section 5.7
...
\(0.4891\) (\(V_3\)), then to \(0.4916\) (\(V_4\)), then to \(\lambda^{**}=0.4924\) (\(V_5=OEOEOEOEOEE\); all audited) and would raise \(0.5392\) to \(0.5665\) alongside
...
\(o((\log x)^{\lambda})\); (iii) for some \(e>1-\lambda^{**}=0.5076\ldots\)
...
\((e_i,c_i)=(\tfrac12,1),(\tfrac38,\tfrac19),(\tfrac34,\tfrac29),(\tfrac9{32},\tfrac1{27}),(\tfrac{27}{128},\tfrac1{81}),(\tfrac{81}{512},\tfrac1{243}),(\tfrac{243}{2048},\tfrac1{729})\),
so that
...
\(+\tfrac1{243}g_A(81t/512)\), and the \(V_5\) term
\(+\tfrac1{729}g_A(243t/2048)\) of
...
that \(\tfrac{243}{2048}t_1\ge 4\log(m+1)\), that \(e^{243t_1/2048}\ge 10^6\),
...
\([\tfrac{243}{2048}t_1,t_1]\). The first induction step of Lemma 5.1 covers
up to \(t_1/(\tfrac34)=\tfrac43 t_1\), at which
\(\tfrac{243}{2048}t=\tfrac{81}{512}t_1\). Lemma 5.1 then gives
\(g_A(t)\ge Kt^\lambda\) for all \(t\ge\tfrac{243}{2048}t_1\), and
...
Theorem 1's exponent \(\lambda^{**}=0.4924\), and alongside
...
\(e=e(C)-\varepsilon\ge e(19)-\varepsilon=0.527-\varepsilon>0.5076\);
...
The least unconditional depth is \(C=19\) (\(e(19)=0.527>0.5076\)).
...
\(OEOEE\), \(V_3\), \(V_4\) and \(V_5\) productions, \(\lambda^{**}=0.4924\); the ideal ones
...
| + \(V_5=OEOEOEOEOEE\) (\(\lambda^{**}\), Theorem 1; elementary, audited) | \(\ldots,(\tfrac1{729},\tfrac{243}{2048})\) | \(0.4924\) |
...
\(1-\lambda^{**}=0.5076\) / \(V_4\)-only \(1-\lambda_{V_4}=0.5084\) / \(V_3\)-only \(1-\lambda_{V_3}=0.5109\) / \(OEOEE\)-only \(1-\lambda_{\mathrm{oeoee}}=0.5199\) /
pairing \(1-\lambda_{\mathrm{pair}}=0.5520\) /
conditional \(1-\lambda^{***}=0.4608\):
fair Chernoff \(19\)/\(20\)/\(18\); one-sided Azuma at
\(q=0.5,0.55,0.60,0.62\): \(19,41,223,1587\) / \(20,44,240,1715\) /
\(18,39,206,1451\);
pressure (biased Chernoff) at the same \(q\): \(19,41,214,1497\) /
