...
\(\lambda<\lambda^{**}=0.4891\ldots\), whereas
...
and backward-closed, and let \(\lambda<\lambda^{**}=0.4891\ldots\), the
root of \(2^{-\lambda}+\tfrac19(\tfrac38)^{\lambda}+\tfrac29(\tfrac34)^{\lambda}+\tfrac1{27}(\tfrac9{32})^{\lambda}+\tfrac1{81}(\tfrac{27}{128})^{\lambda}=1\).
(The \(OEOEE\) truncation of the first four terms is \(0.4801\); the pairing-only root of the first three is \(0.4480\).)
...
The exponent \(0.4891\) is not the method's ceiling. Section 5.7
...
the pairing root \(0.4480\) to \(0.4801\) (\(OEOEE\)) and then to
\(\lambda^{**}=0.4891\) (\(V_3=OEOEOEE\); both audited) and would raise \(0.5392\) to \(0.5665\) alongside
...
\(o((\log x)^{\lambda})\); (iii) for some \(e>1-\lambda^{**}=0.5109\ldots\)
...
\((e_i,c_i)=(\tfrac12,1),(\tfrac38,\tfrac19),(\tfrac34,\tfrac29),(\tfrac9{32},\tfrac1{27}),(\tfrac{27}{128},\tfrac1{81})\),
so that
...
it — together with the \(OEOEE\) term \(+\tfrac1{27}g_A(9t/32)\) and the
\(V_3\) term \(+\tfrac1{81}g_A(27t/128)\) of
...
that \(\tfrac{27}{128}t_1\ge 4\log(m+1)\), that \(e^{27t_1/128}\ge 10^6\),
...
\([\tfrac{27}{128}t_1,t_1]\). The first induction step of Lemma 5.1 covers
up to \(t_1/(\tfrac34)=\tfrac43 t_1\), at which
\(\tfrac{27}{128}t=\tfrac9{32}t_1\). Lemma 5.1 then gives
\(g_A(t)\ge Kt^\lambda\) for all \(t\ge\tfrac{27}{128}t_1\), and
...
gives Theorem 1's exponent \(\lambda^{**}=0.4891\), and alongside
...
Suppose that for some \(e>1-\lambda^{**}=0.5109\ldots\) and all
...
\(e=e(C)-\varepsilon\ge e(19)-\varepsilon=0.527-\varepsilon>0.5109\);
...
The least unconditional depth is \(C=19\) (\(e(19)=0.527>0.5109\)).
...
\(OEOEE\) and \(V_3\) productions, \(\lambda^{**}=0.4891\); the ideal ones
...
| + \(V_3=OEOEOEE\) (\(\lambda^{**}\), Theorem 1; elementary, audited) | \(\ldots,(\tfrac1{81},\tfrac{27}{128})\) | \(0.4891\) |
...
\(1-\lambda^{**}=0.5109\) / pairing \(1-\lambda_{\mathrm{pair}}=0.5520\) /
conditional \(1-\lambda^{***}=0.4608\):
fair Chernoff \(19\)/\(20\)/\(18\); one-sided Azuma at
\(q=0.5,0.55,0.60,0.62\): \(19,42,225,1596\) / \(20,44,240,1715\) /
\(18,39,206,1451\);
