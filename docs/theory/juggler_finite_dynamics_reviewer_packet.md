...
| Length-5 splits (Theorem 6.3) | **EXACT — HUMAN PROOF** | not a census of depth five: four words only; \(OOOO*\) excluded (level-3 kernel); \(OOOE*\) rides Theorem 6.1 at \(N^{1-1/96+\varepsilon}\); \(OOEO*\) uses the shifted-window method of Theorem 4.8 at the \(v\)-level, error \(N^{47/48+\varepsilon}\); \(X\)-modes sit in the Lemma 5.2(i) budget, not (D1); \(k=0\) on \(OOEO*\) is Theorem 4.7 |
