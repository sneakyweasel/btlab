...
Powers: Four-Step Descent and Conditional Extensions for the Juggler
...
Powers\\[0.4em]{\large Four-Step Descent and Conditional Extensions for
the Juggler Map}}
...
\(13/16\). The analytic input is a mixed exponential-sum estimate for
two nested floor powers and a smooth third coordinate. After one
differencing, the remaining floor error is small in the required
frequency range. An exact carry expansion and second-derivative
estimates summed over every gap cell give the error
\(O(N^{23/24}(\log(2N))^3)\) for the four-step count. We also repair the
OE-branch third-letter count using an exact square-root identity, and
give a partition-aware curvature estimate for a basic collision model.
These arguments restore part of an earlier draft without asserting its
general decorated kernel bound. Under two additional finite-chain
correlation hypotheses the five-step certificate density is \(7/8\).
Fair-share densities for every fixed-depth odd-rooted itinerary would
imply density-one finite certificates; no uniformity in depth is needed
for that implication. The general kernel estimate, its short-interval
extension, and the remaining correlation hypotheses are unproved here.
No result asserts universal arrival at \(1\).
...
general \(95/96\) kernel estimate remains open here. Section 7.2 repairs
a basic collision model while stating the additional work needed for the
decorated kernel. The former threshold \(3.6\cdot10^{13}\) is not
certified by this version: the analytic constants are implicit and no
numerical threshold is claimed. The hypotheses for the higher
conditional results remain explicit.
...
not estimate the expanding third-level coordinate \(v^{3/2}\) or the
other formal chains required at length five.
...
\textbf{Theorem 5.3 (conditional five-step density).} Fix
\(0<\delta_0<1/24\). Suppose \(\mathrm H(w;\delta_w)\) holds for each \[
...
\] The two qualitative hypotheses \(\mathrm H_0(w)\) suffice for natural
density \(7/8\).
...
\(1/32\) by Proposition 4.1. Add these counts to Theorem 5.2, absorbing
its logarithmic factor into \(N^{1-\delta_0}\). The total density is
\(13/16+1/32+1/32=7/8\), and Lemma 5.1 exhausts certificates through
length five. \(\square\)
...
5 & \(OOOEE,OOEOE\) & \(7/8\) & Conditional on the two hypotheses in
Theorem 5.3 \\
\end{longtable}
...
5.2 \\
Certificate density \(7/8\) & Conditional: two five-letter hypotheses
unproved \\
...
in the repairs. Those checks support reproducibility; they are not
proofs of asymptotic cancellation.
...
Reaching \(7/8\) by this route requires the two five-letter correlation
hypotheses in Theorem 5.3. Section 7.2 supplies a partition-aware
estimate for a basic collision model, but all decorated phase families
and their weights still need to be controlled before the former kernel
theorem can be restored. A proof of all fixed-depth counts would imply
density-one certificates by Theorem 6.1; a further argument would still
be required to settle universal termination.
...
proofs, the subsequent four-step and collision-model repairs, and its
