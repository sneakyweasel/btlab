...
The printed hug-charge displays against that average are
`hugCharge_sub_rotationAverage_le` and companions; the slack form
`hugCharge_lt_invLog_add_budget` adds the Laplace bound without
claiming the numeric kill.
...
with the general Lean block envelope, and the printed \(C_*\) form
`hugCharge_sub_rotationAverage_extended`. It is
...
`hugCharge_sub_circleMean_le`, `hugCharge_sub_rotationAverage_le`); the constant-cap instance
...
its transcendence input is external to Lean. The printed first display
under that hypothesis is `cycleMin_length_of_rhin`; the calculator
step \(2e^{6.1256}<915\) stays human. The standalone numerical
...
replayable. All publication copies are generated by `tools/build_paper.py A`.
