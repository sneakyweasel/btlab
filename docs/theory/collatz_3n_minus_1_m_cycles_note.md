...
below or is exact arithmetic recorded with its margins. Simons–de Weger 2005 has not yet
been read in full, and the note says so where it matters.
...
states that the floor-free route stops at \(m\ge3\). For \(3n+1\), Simons and de Weger [SdW]
exclude \(m\)-cycles for \(m\le68\) from the verification floor of their day, and Hercher
[H23] reaches \(m\le91\) from \(695\cdot2^{60}\) with the arrangement of the valleys. The
template of [SdW], as [S08] lays it out for generalized maps, has five steps: a floor, a bound
on \(\Lambda\) from the cycle equation, Crandall's lemma for a lower bound on the length,
Rhin's or Laurent–Mignotte–Nesterenko's bound for an upper bound, and lattice reduction where
the two bounds leave a window. Nobody has run it on \(3n-1\), because the first step was
missing. This note runs the first four steps with the constants of this side, and leaves the
fifth as its open list.
...
The corresponding bound on the positive side, Hercher's Corollary 17, carries the constant
\(97/54\). Here it is \(1\): the odd step subtracts, so the run's elements sit at or above
the geometric progression rather than below it.
...
**Proposition 5.** Assume Rhin's effective measure in the form used since Paper A,
\[
|u_1\log2+u_2\log3|\ >\ \exp\bigl(-13.3\,(0.46057+\log H)\bigr),\qquad H=\max(|u_1|,|u_2|),
\]
[R87, Proposition p. 160, (7)]. For an \(m\)-cycle of length \(K\) with an even step,
\[
2^{L_{\min}(K,m)}\ <\ m\,e^{6.1256}K^{13.3}\ <\ 457.42\,m\,K^{13.3}.
\]
Let \(K_3(m)\) be the least integer such that \(2^{L_{\min}(K,m)}\ge914.82\,m\,K^{13.3}\) for
every \(K\ge K_3(m)\). Then every \(m\)-cycle has \(K<K_3(m)\).

*Proof.* Lemma 2 gives \(x_{\min}-1<m/\Lambda\), Rhin with \((u_1,u_2)=(-K,o)\), \(H=K\), gives
\(1/\Lambda<e^{13.3\cdot0.46057}K^{13.3}\), and Lemma 3 gives \(x_{\min}-1>2^{L_{\min}}\).
The function \(K\mapsto L_{\min}(K,m)-13.3\log_2K-\log_2(914.82\,m)\) is convex, negative
at \(K=2\), and its larger root exists; beyond that root it is increasing, which makes
\(K_3(m)\) well defined and the doubled constant covers \(m/\Lambda+1\). \(\square\)
...
The general two-logarithm bound of Laurent–Mignotte–Nesterenko [LMN] would replace \(13.3\)
by an exponent in the thousands at these heights and is not competitive; Rhin's constant
is specific to \(\log2\) and \(\log3\).
...
|---|---|---|---|---|---|
| 1 | \(5.7\cdot10^{-14}\) | 174 | 0 | — | — |
| 2 | \(1.1\cdot10^{-13}\) | 538 | 0 | — | — |
| 5 | \(2.8\cdot10^{-13}\) | 4171 | 0 | — | — |
| 10 | \(5.7\cdot10^{-13}\) | 59731 | 0 | — | — |
| 20 | \(1.1\cdot10^{-12}\) | 8648328 | 0 | — | — |
| 30 | \(1.7\cdot10^{-12}\) | 1120076589 | 0 | — | — |
| 40 | \(2.3\cdot10^{-12}\) | 137210422157 | 0 | — | — |
| 44 | \(2.5\cdot10^{-12}\) | \(7.3\cdot10^{11}\) | 1 | 757698850864 | \(-399.7\) |
| 45 | \(2.6\cdot10^{-12}\) | 1497362821122 | 3 | 757698850864 | \(-236.4\) |
| 47 | \(2.7\cdot10^{-12}\) | 3886787404098 | 9 | 757698850864 | \(-68.3\) |
| 48 | \(2.7\cdot10^{-12}\) | 6259577418842 | 15 | 757698850864 | \(-27.3\) |
| 49 | \(2.8\cdot10^{-12}\) | 10078272110504 | 25 | 757698850864 | \(-1.4\) |
| 50 | \(2.8\cdot10^{-12}\) | 16222496293743 | 41 | 539722056247 | \(+22.4\) |
...
Two comparisons, offered as plausibility checks and not as reproductions: Simons and de
Weger's \(68\) for \(3n+1\) at a floor above \(2.5\cdot2^{60}\) is the number this
transposition gives at \(2^{60}\); Hercher's \(91\) at \(695\cdot2^{60}\) against the plain
template's \(82\) at \(2^{68}\) prices his valley arrangement at about nine values of \(m\),
which is therefore the next refinement to transpose. The floor enters only through the
...
Eliahou-type lattice of admissible periods on this side is not written; Simons–de Weger 2005
has not been read in full, so their sharper constants may move Table 1 upward and their
attribution is taken from Simons 2008 and from Lagarias's bibliography. Lemmas 1–3 are
...
- [SdW] J. L. Simons and B. M. M. de Weger, "Theoretical and computational bounds for
  m-cycles of the 3n+1-problem," *Acta Arith.* 117 (2005), 51–70. Not yet read in full here.
