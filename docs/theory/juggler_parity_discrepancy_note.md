...
which the Collatz stopping time is not constant. The minimal
certificates, counted by length, are the admissible sequences of
A100982 [6], supported on the binary lengths of the powers of three,
A020914 [7]. That support has a name in the Collatz literature which
this paper had not used: A020914 is the set of allowable **dropping
times** of the shortcut map, recorded as such in its OEIS entry since
2009, with the dropping time of an individual start tabulated as
A126241. The length of a minimal certificate is a dropping time, and
the two descriptions are the same set. The list of Lemma 5.1 and the
surviving words of Theorem 6.1 are those objects; no priority is
claimed for them, and the counting identities are recorded and
machine-checked separately in the laboratory repository rather than
proved here.
