...
\(k=0,\dots,7\), below which the measurement floor is reached: a
sawtooth on the orbit. Its Fourier coefficients decay like
\(k^{-0.8}\), which is slower than the  \(k^{-1}\)  a function of
bounded variation would give, so the jumps are observed but the
summability of their amplitudes is not, and  \(\psi\)  is not claimed
to be of bounded variation. No Taylor expansion of  \(\psi\)  exists,
since it is not continuous. What is not
