...
The complete analytic argument for this density is included in
Appendices A--C.
Fair-share densities at every fixed depth would imply density-one
finite certificates; that hypothesis and general decorated estimates
remain open. For the two five-letter words, the starts reaching a
given fifth iterate lie in a window of about  \(n^{5/32}\)  integers
near a start  \(n\); all but a power-saving proportion of these windows
carry their fair share (Theorem 6.3). Localization to a prescribed short
interval remains open. No result asserts universal arrival at  \(1\).

**Keywords:** Juggler map; nested floor powers; parity correlations;
discrepancy; dropping time; finite-step descent.
...
extensions from earlier drafts remain unproved. Theorem 6.3 and
Appendix D prove an averaged substitute for the two five-letter words:
almost every target fibre receives its fair share. The weaker
\(127/128\)  exponent proved here suffices for the five-step count.

That kinship is an identity rather than an analogy, and stating it
precisely is what fixes the contribution of this paper. The shortcut
Collatz map sends  \(x\)  to  \(x/2\)  or  \((3x+1)/2\), so
\(\log x\)  moves by  \(-\log2\)  or  \(+\log(3/2)\); the Juggler
map multiplies  \(\log n\)  by  \(1/2\)  or  \(3/2\), so
\(\log\log n\)  moves by the same two steps. A word therefore
contracts in one problem exactly when it contracts in the other, and
the word counts used here are Collatz word counts. The number of words
of length  \(d\)  with no contracting prefix is A076227 [5], in the
OEIS since 2002 as the number of residue classes modulo  \(2^d\)  on
which the Collatz *dropping time* is not constant; that entry's own
comment names A074473, which the OEIS titles the dropping time. The
minimal certificates, counted by length, are the admissible sequences
of A100982 [6], supported on the binary lengths of the powers of
three, A020914 [7] -- which that entry has recorded since 2009 as the
set of allowable dropping times of the shortcut map, the value for an
individual start being A126241. The length of a minimal certificate is
a dropping time; computational work calls the same quantity the
*glide*, and Hikawa's toolkit [15], which prints it under that name,
computes the survivor counts, the minimal-certificate counts and their
refinement by odd count, all three agreeing with the counts used here.

A word on the term, since two conventions collide here and this paper
had declared neither. Terras [4], whom it follows elsewhere, calls the
first time an orbit falls below its start the *stopping time*; the
OEIS calls that the *dropping time* and reserves *stopping time* for
the number of steps needed to reach  \(1\), a different quantity that
plays no role below. The object is the same under either name, and it
is the one used throughout. The list of Lemma 5.1 and the surviving
words of Theorem 6.1 are those objects; no priority is claimed for
them, and the counting identities are recorded and machine-checked
separately in the laboratory repository rather than proved here.

The two problems part at the arithmetic, and that is the whole of the
present work. For Collatz the word of length  \(d\)  is a function of
\(x\bmod2^d\), and the induced map to words is a bijection [4], so
every word has density exactly  \(2^{-d}\)  and the fair-share counts
are free. For the Juggler map the word is a function of
\(\{n^{3/2}\}\)  and of the nested fractional parts that follow it,
and the corresponding statement is Hypothesis  \(\mathrm{FD}\)  of
Section 6, which is open. Theorems 3.1, 5.2, 5.3 and 5.4 are statements
of that second kind and have no Collatz counterpart. No theorem about
...
| Section 6.1 and Appendix D | Averaged localization on depth-five fibres |
...
These five words are the minimal certificates of length at most five
counted by A100982 [6]. That sequence is supported on the binary
lengths of the powers of three [7], which is why length three carries
no certificate; the enumeration above verifies its first terms rather
than establishing them.

The list, and the statement for every length --- a minimal certificate
of length  \(L\)  exists exactly when a power of three lies in
\([2^{L-1},2^{L})\), its odd count is then forced by  \(L\), and its
last letter is  \(E\)  --- are machine-checked in
`formal/Problems/Juggler/PaperBCertificates.lean` and
`PaperBCertificateLengths.lean`. Length six adds no minimal certificate
and length seven adds exactly three,  \(OOEOOEE\),  \(OOOEOEE\)  and
\(OOOOEEE\); that is a count of words, not a density statement, since
each would need its own mixed-mode estimate. Both facts are checked in
`PaperBFiveStepDensity.lean`.
...
The partition behind Theorem 5.4 --- at depth five the cylinders of the
five words of Lemma 5.1 have sizes  \(16,8,2,1,1\), are pairwise
disjoint and exhaust the certified words, so that
\(1/2+1/4+1/16+1/32+1/32=7/8\)  is cylinder arithmetic rather than an
identity between rational literals --- and the assembly of the count as
a sum of five class counts are machine-checked in
`PaperBFiveStepDensity.lean`; the analytic estimates for those counts
are not.
...
The words this bound counts are those of A076227 [5], so its decay is
a statement about Collatz residue classes as well. There the
fair-share densities that Hypothesis  \(\mathrm{FD}\)  assumes are a
theorem of Terras [4]; that asymmetry, and not the combinatorics, is
what the analytic sections of this paper are for.
...
in the notation of the recursion below,  \(b_{d-1}M_{d-1}\), the
survivors on the barrier that lose their even extension --- and summing
that identity
writes  \(N_d/2^d\)  as a tail of his sequence, so his results do bear
on  \(\psi\) . They do not determine it. His Corollary 12 normalises
\(a_3(r)\)  by its lower cycle bound and proves that the ratio has
\(\liminf\)  equal to  \(1\)  and  \(\limsup\)  equal to
\(\alpha/(\alpha-1)\)  with  \(\alpha=\log_23\) , the lower value
attained exactly on the record minima of  \(\{r\alpha\}\)  and the
upper approached along the record maxima. That is an exact statement
about the extremes of two sparse sets of orders, not an asymptotic for
a general  \(r\) , and it carries no error term at all. The envelope
has width  \(2.7095112914\)  and transfers termwise to the tail, while
the entire oscillation of  \(\psi\)  spans a factor  \(1.06\) : too
...
The increments are tabulated too: T. D. Noe's frequency comment on
A186008 [16], February 2011, lists
\(1/2,1/4,1/16,1/16,3/128,7/256,3/256,\dots\), which are
\(b_{d-1}M_{d-1}/2^{d}\)  here, the certificate density gained at each
carrying length, and A186009 [16] is the sequence of nonzero
minimal-certificate counts. The telescoped form of the recursion,
writing  \(N_d/2^d\)  as the mass of the minimal certificates beyond
depth  \(d\)  plus the survivors at the cutoff, and the doubling
\(N_{d+1}=2N_d\)  exactly at the lengths of A054414 are machine-checked
in `PaperBCertificateRecursion.lean`; that exchanging two adjacent
barrier letters costs exactly the mass on the barrier, the elementary
...
\]
reproduced to  \(2.3\cdot10^{-14}\)  over  \(n\le3000\)  by a
transfer-operator propagation that uses no measured data; with the
normalisation of  \(\psi\)  above,
\(a_n=2\varrho a_1\,\psi(\{n\beta\})\,n^{-3/2}(1+o(1))\),
so the amplitudes decay with the same power as the counts they come
from, and the steepening power law that local differencing had suggested
was the bias of the differencing. The one constant is closed form,
\[
...
excess at  \(\beta\)'s convergent denominators, and the convergence is
uniform across them.  A change of weight would restore a spectral gap,
and none does: at weight  \(r^m\)  the log essential radius is the same
expression whose stationary point is  \(r^{*}\), so it meets  \(\log\varrho\)
there and lies strictly above it at every other  \(r\).  Equivalently a
Foster--Lyapunov drift  \(V(m)=\theta^{m}\)  is pinned at exactly
\(\varrho\)  when  \(\theta=1/r^{*}\)  and is useless otherwise.  That one
is proved rather than measured.

The literature reaches the driven case but stops short of this one.
Oçafrain treats absorbing boundaries that move periodically, and two of
those results bear on the construction above.  For a boundary that moves
at all there is *no* quasi-stationary distribution in the strict sense,
so the phase-indexed family  \(R(\varphi)\)  is forced rather than chosen;
and for a finite state space with a periodic boundary a quasi-ergodic
distribution exists by an extension of Darroch--Seneta, which covers the
truncated rational barrier.  The general moving-boundary theorems assume
a conditional Doeblin condition that is equivalent, in the
time-homogeneous case, to *exponential uniform* convergence to
quasi-stationarity — which this process fails precisely by being
critical.  So the missing combination is driven *and* critical, and it is
now the hypothesis of a named theorem rather than an absence in the
record.

The driving itself is not the obstruction.  For a rational barrier
\(p/q\)  the period map is autonomous, and the constant in the
convergence  \(\mathrm{TV}\sim c/d\)  is flat in  \(q\): across six
convergents of  \(\beta\)  from  \(q=19\)  to  \(q=24727\)  the exponent
stays  \(d^{-1}\)  and  \(c\)  stays  \(20.1\), the last two agreeing to
six digits.  Every barrier in the family is a bounded perturbation of the
same line,  \(\lvert\lceil ts\rceil-ts\rvert<1\)  uniformly in  \(t\)  and
\(s\), which is why  \(q\to\infty\)  costs nothing.  What remains is the
criticality itself.
...
### 6.1 Averaged localization on depth-five fibres

Hypothesis  \(\mathrm{FD}\)  concerns the proportion of starts with a
given word. A finer question is whether the starts sharing a word and a
target also share evenly. Fix one of the two five-letter words
\(OOOEE\)  and  \(OOEOE\). The integers whose fifth iterate along the
word, with each letter applied regardless of parity, equals  \(y\)  form
a window of about  \(\tfrac{32}{27}y^{5/27}\)  consecutive integers near
\(y^{32/27}\), roughly  \(P^{5/32}\)  integers at scale  \(P\). The
starts that actually follow the word and reach  \(y\)  are some of the
odd members of this window. Corollaries 4.10 and 4.12 count the two
words over whole dyadic blocks. They do not say that the individual
windows receive their share, and Section 7 explains why a block count
cannot be rescaled to a short interval. The following theorem proves the
share on almost every window by averaging, without a bound for any fixed
short interval.

For  \(a\in\{OOOEE,OOEOE\}\)  and an integer  \(y\ge1\), let
\(\mathcal F_a(y)\)  be the set of  \(n\)  with
\(\operatorname{word}_5(n)=a\)  and  \(J^5(n)=y\), let
\(\mathcal I_a(y)\)  be the set of integers whose fifth iterate along
\(a\), with each letter applied regardless of parity, equals  \(y\), and
let  \(H_a(y)\)  be the number of odd integers in  \(\mathcal I_a(y)\).
Appendix D.1 shows that  \(\mathcal I_a(y)\)  is an interval, that
\(\mathcal F_a(y)\)  consists of its odd members with word  \(a\), and
that  \(H_a(y)=\tfrac{16}{27}y^{5/27}+O(1)\).

**Theorem 6.3 (averaged localization on depth-five fibres).** For
\(\eta>0\)  let  \(\Pi_\eta(a)\)  be the set of targets whose fibre
misses the share  \(1/16\)  of its odd candidates by at least  \(\eta\):
\[
\Pi_\eta(a)=\Bigl\{y\ge1:\ \Bigl|\#\mathcal F_a(y)
-\frac{H_a(y)}{16}\Bigr|\ge\eta H_a(y)\Bigr\}.
\]
Then, for every  \(y_0\ge1\),
\[
\sum_{\substack{y>y_0\\ y\in\Pi_\eta(OOOEE)}}\frac1y\ll_\eta y_0^{-1/55},
\qquad
\sum_{\substack{y>y_0\\ y\in\Pi_\eta(OOEOE)}}\frac1y\ll_\eta y_0^{-1/41}.
\tag{6.1}
\]
More precisely, the proof shows the dyadic form from which (6.1)
follows by summation: among the targets whose windows begin in
\((P,2P]\), a proportion  \(O_{\eta,\varepsilon}(P^{-1/64+\varepsilon})\)
for  \(OOOEE\), and  \(O_\eta(P^{-1/48})\)  for  \(OOEOE\), lie in
\(\Pi_\eta(a)\). The exponents in (6.1) are rounded down from
\(\tfrac{32}{27}(\tfrac1{64}-\varepsilon)\)  and  \(\tfrac2{81}\).

The set  \(\Pi_\eta(a)\)  contains the targets with  \(H_a(y)=0\). By
(D.2) and an exact computation below the point where (D.2) excludes
them, these are  \(y=4,12\)  for  \(OOOEE\)  and  \(y=5,14,16,20\)  for
\(OOEOE\).

The proof is in Appendix D. It rests on the estimates of Appendix C, on
Lemma 4.4 and Section 4.3, and on two further differenced estimates
proved there. One diagonal curvature constant,  \(243/4096\), belongs to
Lemma D.6 for  \(OOOEE\). The other,  \(-1701/4096\), belongs to a carry
of  \(\lfloor U\rfloor\)  that treats  \(OOEOE\)  without Theorem 4.9's
centering. Theorem 6.3 is a statement about almost every window. It
gives no bound for a prescribed short interval, and Section 7's caution
stands. Like the proofs of Appendices A--C, the proof is an AI-assisted
written argument that has not been independently reviewed; Appendix D.8
records what has been machine-checked.
...
Theorem 6.3 is consistent with this caution: it averages over the
target windows and asserts nothing for a prescribed one.
...
| Word counts, certificate lengths, and the survivor decay | Shared with the Collatz literature: A076227, A100982, A020914; no priority claimed |
...
| Oscillating prefactor  \(\psi\), its jump spectrum and Fourier identity | Verified computation in Section 6; the existence of the limit is not proved |
| Finite combinatorics of Sections 5--6 and the chain of Theorem 6.1 | Machine-checked; modules listed below |
| Restricted mixed sums | Theorems 4.5, 4.9, and 4.11 |
| General decorated kernel and prescribed short-interval extension | Unproved; no numerical threshold certified |
| Fair share on almost every depth-five fibre | Theorem 6.3 and Appendix D; AI-assisted written proof, not independently reviewed |
...
The Lean modules that check statements of this manuscript, module by
module, all of them under `formal/Problems/Juggler/`:

- `PaperBCertificates`: Lemma 5.1, the minimal certificates through
  length five.
- `PaperBCertificateLengths`: a minimal certificate of length  \(L\)
  exists exactly when a power of three lies in  \([2^{L-1},2^{L})\),
  its odd count is then forced and its last letter is  \(E\).
- `PaperBFiveStepDensity`: the five cylinders partition the certified
  words at depth five and the count is the sum of the five class counts
  behind Theorems 5.2--5.4; depth six adds no minimal certificate and
  depth seven adds three.
- `PaperBCertificateRecursion`: the survivors of length  \(d+1\)  and
  the words contracting first at  \(d+1\)  together number  \(2N_d\),
  the telescoped tail form of that identity, and the doubling at the
  free lengths.
- `PaperBSurvivorDecay`:  \(N_d/2^d\le\vartheta(p)^d\), the upper
  bound of Lagarias's Theorem D [14] with its exact exponent, and  \(N_d/2^d\to0\).
- `PaperBChernoff`, `PaperBMarkov`, `PaperBDensity`: the chain of
  Theorem 6.1 with Hypothesis  \(\mathrm{FD}\)  carried as a hypothesis.
- `PaperBThreshold`: the threshold algebra of Remark 6.2, that
  \(q_d\)  saturates the step and  \(d\ge4\)  is the first usable depth.
- `PaperBTilt`: the mean-zero tilt and the closed form of  \(\varrho\).
- `PaperBBarrierStep`: when the barrier does not rise every survivor
  keeps both extensions, and when it rises an extension dies exactly
  for a word on the barrier taking the even letter.
- `PaperBJumpTransposition`: exchanging two adjacent barrier letters
  costs exactly the mass on the barrier, the elementary fact under the
  discontinuities of  \(\psi\).

The declarations audited for each, in the same order, are those whose
axioms are printed by the published artifact,
`formal/AxiomCheckPaperBPublished.lean`:

```
PaperBCertificates          lemma51  lemma51_complete
PaperBCertificateLengths    minimalCert_exists_iff  certWindow_unique
                            minimalCert_concat_even
PaperBFiveStepDensity       five_cylinders_cover  certifiedCount_five_eq
                            no_minimal_certificate_six
                            minimal_certificates_seven
PaperBCertificateRecursion  neverNegCount_add_minimalCertCount
                            minimalCert_tail_eq
                            neverNegCount_succ_of_window_empty
PaperBSurvivorDecay         neverNegCount_div_pow_le_theta
                            neverNegCount_div_pow_tendsto_zero
PaperBChernoff              theta_lt_one  klDiv_pos
PaperBMarkov                sum_choose_mul_pow  chernoff_density
                            tilt_gives_theta
PaperBDensity               density_of_finite_union
                            exceptional_density_zero
PaperBThreshold             breakEven_saturates  first_usable_depth
PaperBTilt                  lamStar_mean_zero  rho_closed_form
PaperBBarrierStep           survives_succ_of_no_rise  dies_iff_on_barrier
PaperBJumpTransposition     stepRise_stepFlat_eq_add_barrierMass
```

Its recorded output shows no axiom beyond Mathlib's three, `propext`,
`Classical.choice` and `Quot.sound`, on any line; `klDiv_nonneg`, cited
in the proof of Theorem 6.1 for contrast, is audited there too. All are
checked by Lean's kernel, with no `native_decide`. This
version claims no Lean formalization of its analytic estimates — the
exponential sums of Sections 3, 4 and 7 and Appendices A--D — nor of
the complete manuscript. Earlier theorem numbers are superseded and
should not be used as citations to unconditional nested estimates.
Companion texts that rely on those estimates require their own review.

Lemma D.3 and the fibre geometry (D.2) of Appendix D are checked in two
further modules, listed with their audits in Appendix D.8. The
square-wave, second-moment and counting steps of Appendix D are not
formalized.
...
termination conclusion. Theorem 6.3 is an averaged substitute for the
two five-letter words, not a bound on a prescribed interval. A proof of all fixed-depth fair-share
...
In Appendices A--D, a subscript on \(\Delta\) denotes the actual
...
## Appendix D. Localization on depth-five fibres

This appendix proves Theorem 6.3. Throughout,
\(a\in\{OOOEE,OOEOE\}\), all sums run over odd integers, and  \(P\)  is
large. The four formal coordinates of  \(a\)  are written
\(R_1,R_2,R_3,R_4\):  \(X,Y,Z,U\)  of Appendix C for  \(OOOEE\), and
\(X,Y,W,U\)  of Section 4.3, in that order, for  \(OOEOE\). For a
nonzero integer vector  \(\nu=(i,j,k,l)\)  put
\[
\phi_\nu(n)=\tfrac12\bigl(iR_1(n)+jR_2(n)+kR_3(n)+lR_4(n)\bigr),
\qquad
D_h(\nu)=\sum_{P<n\le2P}e\bigl(\phi_\nu(n+2h)-\phi_\nu(n)\bigr).
\tag{D.1}
\]
Here  \(k\)  is the frequency of  \(Z\)  for  \(OOOEE\), as in Theorem
4.11, and of  \(W\)  for  \(OOEOE\), as in Theorem 4.9, whose  \(\ell\)
is our  \(l\);  \(l\)  is the frequency of  \(U\)  in both words. For
\(OOEOE\)  the coordinates are listed out of their order of iteration so
that  \(k\)  and  \(l\)  keep these meanings. The coordinates are defined
on  \([P,3P]\), as in Section 4.3 and Appendix C, but  \(D_h(\nu)\)
always runs over the dyadic block  \((P,2P]\). All estimates are uniform
in  \(\nu\)  with  \(\|\nu\|_\infty\le C\), for any fixed  \(C\), and in
the shifts  \(1\le h<H_1=\lfloor P^{1/48}\rfloor\), the first-shift
range of Appendix C.9.

### D.1 The fibres

Write  \(\iota(b)=\lceil b^{2/3}\rceil\), so that
\(\lfloor q^{3/2}\rfloor\ge b\)  if and only if  \(q\ge\iota(b)\). Along
\(OOOEE\)  the fifth iterate is  \(\lfloor\lfloor Z\rfloor^{1/4}\rfloor\).
Along  \(OOEOE\)  it is  \(\lfloor\lfloor W\rfloor^{1/2}\rfloor\). Each is
a composition of nondecreasing maps of  \(n\). Put
\[
E_{OOOEE}(y)=\iota(\iota(\iota(y^4))),\qquad
E_{OOEOE}(y)=\iota\bigl(\iota(\iota(y^2)^2)\bigr).
\]
Then the fifth iterate along  \(a\)  equals  \(y\)  exactly on
\(\mathcal I_a(y)=[E_a(y),E_a(y+1))\), and  \(\mathcal F_a(y)\)  is the
set of odd members of  \(\mathcal I_a(y)\)  with
\(\operatorname{word}_5(n)=a\).

The ceilings propagate as follows. For  \(b\ge1\)  and  \(e\ge0\),
concavity gives
\[
(b+e)^{2/3}\le b^{2/3}+\tfrac23b^{-1/3}e\le b^{2/3}+e,
\]
and each ceiling adds less than  \(1\). For  \(OOOEE\)  the weaker form
suffices: after the three ceilings the errors are below  \(1\),  \(2\)
and  \(3\). For  \(OOEOE\), the square of  \(\iota(y^2)<y^{4/3}+1\)
exceeds  \(y^{8/3}\)  by less than  \(2y^{4/3}+1\). The factor
\(b^{-1/3}\)  of the next power  \(2/3\)  reduces this excess to
\(\tfrac43y^{4/9}+O(1)\), and the last power reduces it to  \(O(1)\). For
\(y\ge1\)  this gives
\[
y^{32/27}\le E_a(y)\le y^{32/27}+4,\qquad
\Bigl|H_a(y)-\tfrac{16}{27}y^{5/27}\Bigr|\le4,
\tag{D.2}
\]
and every  \(n\in\mathcal I_a(y)\)  satisfies
\(y^{32/27}\le n\le y^{32/27}+\tfrac{32}{27}y^{5/27}+6\). For
\(OOOEE\)  the endpoint constant is  \(3\).

### D.2 Square waves

By Lemma 2.2,  \(\lfloor R\rfloor\)  is odd exactly when
\(\{R/2\}\in[1/2,1)\). So for odd  \(n\), the indicator of
\(\operatorname{word}_5(n)=a\)  is a product
\(\prod_{t=1}^4\chi_t(R_t(n)/2)\)  of indicators of half arcs of the
circle. For a degree  \(K\), Vaaler's trigonometric polynomials
\(\chi_t^\pm\)  satisfy  \(\chi_t^-\le\chi_t\le\chi_t^+\)  everywhere, and
\(\gamma_t=\chi_t^+-\chi_t^-\ge0\)  has mean  \(2/(K+1)\). Their
coefficients are bounded, and  \(\sup|\chi_t^+|\le A\)  for an absolute
constant  \(A\ge1\). Telescoping gives
\(|\prod\chi_t-\prod\chi_t^+|\le A^3\sum_t\gamma_t\). The constant
coefficient of  \(\prod\chi_t^+\)  is  \(1/16+O(1/K)\). Hence, for any set
\(Q\)  of consecutive odd integers, with
\(S_\nu(Q)=\sum_{n\in Q}e(\phi_\nu(n))\),
\[
\Bigl|\#\{n\in Q:\operatorname{word}_5(n)=a\}-\tfrac{\#Q}{16}\Bigr|
\le\frac{C_0\,\#Q}{K}+C_1\sum_{0<\|\nu\|_\infty\le K}|S_\nu(Q)|,
\tag{D.3}
\]
where  \(C_0\)  and  \(C_1\)  are absolute constants. The sum includes
the vectors with a single nonzero coordinate that come from the
\(\gamma_t\). We fix  \(K=\lceil40C_0/\eta\rceil\).

### D.3 Sub-block averaging

**Lemma D.3 (bad blocks in a poor fibre).** Let a finite union of blocks
\(Q\)  of  \(L\)  consecutive odd integers carry deviations
\(\mathrm{dev}(Q)\)  with  \(|\mathrm{dev}(Q)|\le L\), let their total
size be at most  \(H\), and let the whole set, together with a remainder
\(p\), have deviation  \(|\sum_Q\mathrm{dev}(Q)+p|\ge\eta H\), where
\(|p|\le\eta H/5\). Then the blocks with
\(|\mathrm{dev}(Q)|\ge(\eta/2)L\)  have total size at least
\(\tfrac3{10}\eta H\).

*Proof.* The block deviations sum in absolute value to at least
\(\tfrac45\eta H\). If  \(b\)  blocks are bad, that sum is at most
\(bL+\tfrac12\eta H\). Hence  \(bL\ge\tfrac3{10}\eta H\).  \(\square\)

Call a block  \(Q\)  of  \(L\)  odd integers bad if
\(|\#\{n\in Q:\operatorname{word}_5(n)=a\}-L/16|\ge(\eta/2)L\). The
blocks used below are those of the single tiling fixed in Appendix D.4.
A window  \(\mathcal I_a(y)\)  with  \(y\in\Pi_\eta(a)\)  is cut into the
blocks of that tiling that lie inside it, where
\(L\le\eta\min H_a/10\), and at most two partial pieces, each with fewer
than  \(L\)  odd integers. The partial pieces make up the remainder
\(p\). By Lemma D.3 at least  \(\tfrac3{10}\eta H_a(y)/L\)  of its blocks
are bad. A single bad block would not suffice for what follows; a
positive proportion of bad blocks is what lets the shifts stay small.

### D.4 The second moment

Let  \(1\le L\le H_1\). By (D.2), every window whose left end lies in
\((P,2P]\)  lies in  \((P,2P+c_2P^{5/32}]\), for an absolute constant
\(c_2\). Let  \(\mathcal R\)  be the set of odd integers in that
interval. Tile all odd integers into blocks of  \(L\)
consecutive ones, starting at an offset  \(t\in[0,L)\), and restrict the
tiling to  \(\mathcal R\). A pair  \(n,n+2h\)  with  \(1\le h<L\)  lies
in one block for exactly  \(L-h\)  of the  \(L\)  offsets. Hence
\[
\frac1L\sum_{t<L}\sum_Q|S_\nu(Q)|^2
=\#\mathcal R+2\,\mathrm{Re}\sum_{1\le h<L}\Bigl(1-\frac hL\Bigr)
\sum_{n,\,n+2h\in\mathcal R}e\bigl(\phi_\nu(n+2h)-\phi_\nu(n)\bigr).
\]
For  \(P<n\le2P\)  the point  \(n+2h\)  lies in  \(\mathcal R\), so each
inner sum differs from  \(D_h(\nu)\)  by fewer than  \(c_2P^{5/32}\)
terms, those with  \(n>2P\). Since  \(LP^{5/32}\le P\),
\[
\frac1L\sum_{t<L}\sum_Q|S_\nu(Q)|^2
\ll P+\sum_{1\le h<L}|D_h(\nu)|.
\tag{D.4}
\]
Choose one offset  \(t\)  for which the sum over the finitely many
\(\nu\)  of (D.3) of  \(\sum_Q|S_\nu(Q)|^2\)  is at most its average over
\(t\). By (D.3) with  \(C_0L/K\le\eta L/40\), a bad block has
\(|S_\nu(Q)|\ge c_\eta L\)  for some  \(\nu\)  with
\(0<\|\nu\|_\infty\le K\), where  \(c_\eta>0\)  depends only on  \(\eta\).
Chebyshev's inequality then bounds the number of bad blocks of this
tiling by
\[
\ll_\eta\frac1{L^2}\Bigl(P+\max_{\nu}\sum_{1\le h<L}|D_h(\nu)|\Bigr).
\tag{D.5}
\]
Only shifts  \(h<L\)  enter. This is why the shifts can stay far below
the window length.

### D.5 The differenced sums of OOOEE

**Proposition D.5.** For  \(a=OOOEE\), every bounded nonzero  \(\nu\)
and every  \(1\le h<H_1\),
\(|D_h(\nu)|\ll_{C,\varepsilon}P^{63/64+\varepsilon}\).

*Proof.* If  \(k\ne0\),  \(D_h(\nu)\)  is the first correlation
\(T_1(h)\)  of Appendix C.9 up to the replacement (C.6). That replacement
is a per-point cost, so it applies at both endpoints. C.9 gives
\(T_1(h)\ll P^{63/64+\varepsilon}\)  for  \(h<H_1\). If  \(k=0\),
(C.4)--(C.6) reduce the phase to  \(iX/2+jY/2+lm^{9/8}/2\).

- If  \(j\ne0\), C.2 bounds this correlation by
  \(P^{7/8}(1+\sqrt h)\), since  \(h\le P^{1/12}\).
- If  \(j=l=0\), then  \(i\ne0\). The phase  \((i/2)\Delta_{2h}X\)  has a
  monotone derivative of size about  \(hP^{-1/2}\), below  \(1/4\)  on the
  odd lattice, and the Kusmin--Landau inequality gives
  \(O(P^{1/2}/h+1)\).
- The case  \(j=0\), \(l\ne0\)  is Lemma D.6.  \(\square\)

### D.6 The diagonal of the fifth coordinate

**Lemma D.6.** Let  \(a=OOOEE\)  and  \(j=k=0\neq l\), with  \(i\)  and
\(l\)  bounded, and  \(1\le h<H_1\). Then
\(|D_h(\nu)|\ll P^{15/16}\log P\), where the phase is taken after the
replacement (C.6), whose cost  \(O(|l|P^{7/16})\)  is charged at each
endpoint.

*Proof.* Put  \(\theta=\{X\}\),
\(B(x)=\tfrac{9l}{16}x^{3/16}\)  and  \(N_B=\lfloor B\rfloor\). By (C.8),
at each endpoint
\[
\tfrac l2m^{9/8}=\tfrac l2n^{27/16}-B\theta+O(|l|P^{-21/16}).
\]
Lemma 4.7 at cutoff  \(T=\lfloor P^{1/16}\rfloor\)  gives
\(e(\phi(n))=\sum_{|r|\le T}a_r(\beta)e(f_r(n))+O(E_T(X(n))+P^{-21/16})\),
where  \(f_r=\tfrac l2x^{27/16}+(\tfrac i2+r-N_B)X\). The truncated series
is uniformly bounded by (4.13). So the product of the expansions at
\(n+2h\)  and  \(n\)  has error  \(O(E_T(X(n))+E_T(X(n+2h)))\), and (4.3),
applied inside  \([P,3P]\), bounds its sum by  \(P^{15/16}\log P\).

Put  \(N_1=N_B(x+2h)\)  and  \(N_2=N_B(x)\). Since
\(B(x+2h)-B(x)=o(1)\)  with the sign of  \(l\), we have
\(N_1-N_2\in\{0,\operatorname{sign}l\}\). Both  \(N_1\)  and  \(N_2\)  are
constant on each of the  \(O(1+P^{3/16})\)  intervals of a common
partition, and on each such interval both residuals  \(\beta\)  are
monotone. By (4.13) the product weight
\(a_r(\beta(n+2h))\overline{a_{r'}(\beta(n))}\)  has supremum plus
variation  \(O(1/((1+|r|)(1+|r'|)))\).

The pair  \((r,r')\)  has phase
\(F=\tfrac l2\Delta_{2h}(x^{27/16})+c_1X(x+2h)-c_2X(x)\), where
\(c_1=\tfrac i2+r-N_1\)  and  \(c_2=\tfrac i2+r'-N_2\). Their difference
\(\omega=c_1-c_2\)  is an integer, and
\[
F''=\omega X''(x+2h)+c_2\bigl(X''(x+2h)-X''(x)\bigr)
+\tfrac l2\Delta_{2h}\bigl((x^{27/16})''\bigr).
\]
If  \(\omega\ne0\), the first term has size about  \(|\omega|P^{-1/2}\)
and the others are  \(O(hP^{-21/16})\), because  \(|c_2|\ll P^{3/16}\).
The second-derivative estimate on each interval, summed with the
weights, gives  \(O(T^{1/2}P^{3/4}\log T+P^{7/16}\log^2T)\).

If  \(\omega=0\), then
\(F''=\Delta_{2h}((l/2)(x^{27/16})'')+c\,\Delta_{2h}X''\)  with  \(c\)  frozen.
Writing  \(N_1=B(x)+O(1)\), the leading part is
\[
F''=\tfrac{243}{4096}\,l\,h\,x^{-21/16}
\Bigl(1+O\bigl((1+T)P^{-3/16}\bigr)\Bigr),
\tag{D.6}
\]
from  \(2h\bigl[\tfrac12\cdot\tfrac{27}{16}\cdot\tfrac{11}{16}\cdot(-\tfrac5{16})
+\tfrac9{16}\cdot\tfrac38\bigr]=\tfrac{243}{4096}h\). It is one-signed,
and its size is comparable to  \(hP^{-21/16}\)  across the block. The
estimate gives  \(h^{1/2}P^{11/32}+(1+P^{3/16})h^{-1/2}P^{21/32}\ll
P^{27/32}\), and the diagonal weights sum to  \(O(1)\).  \(\square\)

With the cutoff  \(T=\lfloor P^{1/8}\rfloor\)  the same proof gives
\(P^{7/8}\log P\), but (D.6) is then certified to keep its sign only from
much larger  \(P\)  (Appendix D.8). The cutoff  \(P^{1/16}\)  costs
nothing here, because Proposition D.5 is limited by Appendix C.9.

### D.7 The differenced sums of OOEOE

**Proposition D.7.** For  \(a=OOEOE\), every bounded nonzero  \(\nu\)
and every  \(1\le h<H_1\),
\(|D_h(\nu)|\ll_C P^{15/16}\log^2P\).

*Proof.* Theorem 4.9 centers the coefficient of  \(\{U\}\)  at its floor
on intervals of length  \(P^{7/16}\). For the differenced sum at  \(j=0\)
that fails. A pair of modes  \((r,r')\)  with  \(r\ne r'\)  keeps a phase
\((r-r')x^{9/8}\), whose curvature is about  \(P^{-7/8}\), and on an
interval of length  \(P^{7/16}\)  the second-derivative test then saves
nothing. We keep the coefficient smooth and treat the gap of
\(\lfloor U\rfloor\)  as a carry, as Lemma 4.4 treats the gap of
\(\lfloor X\rfloor\).

By (4.18),
\(\tfrac k2W=\tfrac k2n^{27/16}-C\theta-B\xi+O(P^{-9/16})\), where
\(\xi=\{U\}\), and  \(B=\tfrac{3k}4x^{9/16}\)  and
\(C=\tfrac{9k}{16}x^{3/16}\)  are the coefficients  \(B(x)\)  and
\(C(x)\)  of the proof of Theorem 4.9. Replacing  \(B(n+2h)\)  by
\(B(n)\)  costs  \(khP^{-7/16}\)  per point. The two  \(\xi\)-terms then
combine to  \(-B(n)(\Delta_{2h}U-g)\), where
\(g=\lfloor U(n+2h)\rfloor-\lfloor U(n)\rfloor\).

Write  \(\Delta_{2h}U=V+\zeta_\theta\), with  \(V=\Delta_{2h}(x^{9/8})\)  and
\(\zeta_\theta=-\tfrac34[\theta(n+2h)(n+2h)^{-3/8}-\theta(n)n^{-3/8}]
+O(P^{-9/8})\). Because  \(\tfrac34Bx^{-3/8}=C\), the term
\(-B\zeta_\theta\)  cancels the two  \(C\theta\)  terms up to
\(O(khP^{-13/16})\)  per point. This is (4.19) at each endpoint.

Put  \(G=\lfloor V\rfloor\)  and  \(z=V-G\), which is monotone on each of
the  \(O(1+hP^{1/8})\)  intervals where  \(G\)  is constant. Then
\(g=G+\kappa\)  with  \(\kappa=\mathbf 1[\{U(n)\}\ge1-z]\), except where
\(\|U(n+2h)\|\ll P^{-3/8}\). By (4.17) on the shifted block, that happens
at  \(O(P^{7/8})\)  points. The analogue of (4.8) writes the exponential
exactly as
\((1-z)e(F_{G,0})+ze(F_{G,1})\)  plus sawtooth terms
\(b(U(n))-b(U(n)+z)\), where
\(F_{G,\epsilon}=\tfrac k2\Delta_{2h}(x^{27/16})-B(V-G-\epsilon)
+\tfrac i2\Delta_{2h}X+\tfrac l2V\)  plus the  \(Y\)-part. Lemma 4.3 at
cutoff  \(R_U\)  expands the sawtooth terms into modes  \(e(sU(n))\)  and
\(e(s(U(n)+V))\)  with weights  \(1/|s|\). Since  \(R_U|\zeta_\theta|\le1\),
we have  \(E_R(U(n)+z)\le2E_R(U(n+2h))\), so (4.17) bounds the total
error by  \(P\log R_U/R_U+P^{7/8}\).

*The case  \(j=0\).* The leading terms of  \(\tfrac k2\Delta_{2h}(x^{27/16})\)
and  \(BV\)  are both  \(\tfrac{27}{16}khx^{11/16}\), and they cancel. With
\(G+\epsilon\)  frozen,
\[
F_{G,\epsilon}''=-\tfrac{1701}{4096}\,k\,h\,x^{-21/16}
\bigl(1+O(P^{-1/8})\bigr).
\tag{D.7}
\]
A carry mode adds curvature about  \(sP^{-7/8}\), which dominates. At
\(R_U=P^{1/8}\), the second-derivative estimate with the monotone
weights  \(z\)  and  \(1-z\)  gives
\(P^{11/32}h^{1/2}+P^{25/32}h^{1/2}\)  for the zero modes and
\(R_U^{1/2}P^{9/16}+hP^{9/16}\)  for the carry modes. The remaining costs
are:

- the truncation,  \(P^{7/8}\log P\);
- the mismatches,  \(P^{7/8}\);
- the  \(\theta\)-noise in the modes,  \(R_UP^{5/8}\);
- the  \(l\)-noise,  \(P^{5/8}\);
- the replacements,  \(khP^{9/16}\),  \(khP^{3/16}\)  and  \(kP^{7/16}\).

*The case  \(j\ne0\).* Combine the exact interpolation of Lemma 4.4 for
the gap of  \(\lfloor X\rfloor\)  with the one above for
\(\lfloor U\rfloor\). Their product is exact, and products of monotone
weights have bounded variation on the common cells.

- The zero mode keeps Lemma 4.4's curvature  \(uhP^{-3/4}\). The
  \(U\)-part changes it by relative  \(O(P^{-9/16})\).
- A  \(U\)-mode  \(s\le R_U=P^{1/16}\)  adds  \(sP^{-7/8}\), relative at
  most  \(P^{-1/16}\).
- An  \(X\)-mode dominates everything by Lemma 4.4's ratio  \(P^{-1/8}\).

This is Lemma 4.4's proof with one more carry, valid for
\(h\le P^{1/12}\). It gives  \(P^{7/8}(1+h^{1/2})+hP^{3/4}+P^{5/6}\), and
the  \(U\)-truncation adds  \(P^{15/16}\log P\).

*The case  \(k=0\).* If  \(j\ne0\), after the  \(l\)-noise the phase is
that of Lemma 4.4 with its  \(k\)  equal to  \(l\), because
\(V=\Delta_{2h}(x^{9/8})\). If  \(j=0\), the phase is
\((i/2)\Delta_{2h}X+(l/2)V\)  plus the noise. Its derivative is monotone, of
size about  \(hP^{-1/2}\)  or  \(hP^{-7/8}\), and Kusmin--Landau gives
\(O(P^{7/8}/h)\).  \(\square\)

### D.8 Proof of Theorem 6.3, and what has been checked

Take  \(L=H_1=\lfloor P^{1/48}\rfloor\)  odd integers per block. This is
the first-shift range of Appendix C.9, on which Proposition D.5 is
available. By (D.2) it is below  \(\eta\min H_a/10\)  for large
\(P\). Propositions D.5 and D.7 bound  \(|D_h(\nu)|\)  by  \(P^{1-\sigma}\)
for  \(1\le h<L\), with  \(\sigma=\tfrac1{64}-\varepsilon\)  for
\(OOOEE\)  and  \(\sigma=\tfrac1{16}-\varepsilon\)  for  \(OOEOE\). By
(D.5) the bad blocks of the tiling fixed in Appendix D.4 number
\(\ll P/L^2+P^{1-\sigma}/L\).

Assign each target to the dyadic block containing the left end of its
window. By (D.2) the window then lies in  \(\mathcal R\), the range tiled
in Appendix D.4. Windows are disjoint, and by Lemma D.3 a window of a
target in  \(\Pi_\eta(a)\)  contains at least  \(\tfrac3{10}\eta H_a/L\)
bad blocks. So these targets number
\[
\ll_\eta\frac P{H_a}\bigl(L^{-1}+P^{-\sigma}\bigr)
\ll P^{27/32}\,P^{-\min(1/48,\sigma)}.
\]
The targets themselves have size about  \(P^{27/32}\), and about
\(P^{27/32}\)  of them have windows beginning in  \((P,2P]\). This is the
dyadic form stated after Theorem 6.3. For  \(OOEOE\)  its exponent is set
by  \(L\), since  \(\min(1/48,1/16-\varepsilon)=1/48\). Summing  \(1/y\)
over the dyadic blocks with  \(P\gg y_0^{32/27}\)  gives (6.1) with
exponents  \(\tfrac{32}{27}(\tfrac1{64}-\varepsilon)>\tfrac1{55}\)  and
\(\tfrac{32}{27}\cdot\tfrac1{48}=\tfrac2{81}>\tfrac1{41}\). The finitely
many small targets, including those with  \(H_a(y)=0\), are absorbed in
the implied constant.  \(\square\)

**What is machine-checked.** The following are kernel-checked in Lean 4,
with no axiom beyond  `propext`,  `Classical.choice`  and  `Quot.sound`:

- Lemma D.3, as `SubBlockAveraging.poor_fibre_bad_blocks`;
- the fibre geometry (D.2) and the identification of
  \(\mathcal F_a(y)\)  with the actual fibres, in
  `DepthFiveFibreGeometry`.

Their audits are the two files

- `formal/AxiomCheckSubBlockAveraging.lean`;
- `formal/AxiomCheckDepthFiveWeighted.lean`.

The second also audits `FateDepthFiveWeighted`. That module derives,
from the finiteness of the reciprocal sums in (6.1) at
\(\eta=1/1000\), the two depth-five production inequalities, at
coefficient  \(1/28\), of a conditional contagion assembly in the
laboratory's Lean library. No published
companion text uses these productions; the companion notes on fate
contagion still record them as unavailable. The square-wave,
second-moment and counting steps of D.2, D.4 and D.8 are not formalized.

Numerical checks, none of them a proof of cancellation:

- The constants of (D.6) and (D.7) were derived exactly with SymPy.
- With every charged correction included, Arb interval arithmetic
  certifies the sign of (D.7) for  \(x\ge10^8\), and of (D.6) for
  \(x\ge10^{11}\)  at the cutoff  \(P^{1/16}\), for frequency bound
  \(1\)  and shifts  \(h\le x^{1/48}\). For frequency bound  \(100\)
  the thresholds are  \(10^{11}\)  and  \(10^{16}\). At the cutoff
  \(P^{1/8}\), (D.6) is certified only from  \(10^{18}\). The proof uses
  frequencies up to  \(K\), which grows as  \(\eta\)  decreases; for them
  the sign is known only asymptotically, so these checks do not bound the
  effective range of Theorem 6.3.
- An exact-fraction ledger checks every listed cost against its claimed
  bound.
- A pointwise evaluation on the actual nested-floor coordinates, at
  \(P=10^8\)  and  \(10^{10}\), matches the  \(j=0\)  carry decomposition
  of Proposition D.7 within its charged error orders, for three frequency
  vectors and shifts  \(1\),  \(30\)  and  \(300\). It does not test the
  cases  \(j\ne0\)  or  \(k=0\).

The exponential-sum estimates of this appendix are not formalized.
Lemma D.6 and Proposition D.7 have each had two independent AI
adversarial passes, and the averaging and counting steps one.
A further AI review of this appendix's text found no failing step. It
found two gaps, the range of the second moment and the frequency labels
of  \(OOEOE\), both repaired in this version. Independent mathematical
...
This is version 1.2.0 of Paper B, of 24 September 2026. It adds
Section 6.1 and Appendix D: Theorem 6.3, the fair share on almost every
depth-five fibre, with its proof. The earlier numbered results and their
proofs are unchanged. Version 1.1.2 clarified Section 6's conjectural
limiting-profile discussion, and this version retains that, the corrected
Terras attribution and arXiv reference [13]. It is a preprint, it has
...
5. OEIS Foundation Inc., "Number of surviving Collatz residues mod
 \(2^n\) ," *The On-Line Encyclopedia of Integer Sequences*, A076227,
contributed by Labos Elemer, 2002.
[Sequence record](https://oeis.org/A076227), accessed 19 September 2026.
6. OEIS Foundation Inc., "Number of admissible sequences of order n;
related to 3x+1 problem and Wagon's constant," *The On-Line
Encyclopedia of Integer Sequences*, A100982, contributed by
S. R. Finch.
[Sequence record](https://oeis.org/A100982), accessed 19 September 2026.
7. OEIS Foundation Inc., "Number of digits in the base-2 representation
...
15. K. Hikawa, *Collatz Parity Vector Toolkit*, version 1.0, software,
4 July 2026, MIT license.
[Repository](https://github.com/hikawa94/Collatz-Parity-Vector-Toolkit);
[doi:10.5281/zenodo.21186540](https://doi.org/10.5281/zenodo.21186540).
Run for this revision: its survivor counts, minimal-certificate counts
and odd-count refinement agree with those used here, and it prints the
minimal certificate length as the glide.
16. T. D. Noe, "Irregular triangle of the pattern of conjectured dropping
times in the Collatz iteration," A186008, and "Length of Collatz dropping
time patterns in A186008," A186009, *The On-Line Encyclopedia of Integer
Sequences*, contributed 9 February 2011.
[A186008](https://oeis.org/A186008), [A186009](https://oeis.org/A186009),
