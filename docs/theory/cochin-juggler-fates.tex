...
three-halves power. We prove that every nonempty set \(A\) of positive
integers closed under taking preimages satisfies
...
written argument. The earlier two-production route remains an elementary
Lean baseline at \(100/203\).

For a fixed verified target \([1,N_0]\), universal termination is
equivalent to an eventual-entry statement: all but \(O(y(\log y)^{-e})\)
odd starts in \((y,2y]\) enter that target, for some \(e>13/50\); the
machine-checked threshold, which does not use Paper B, is \(3/8\). The
rate itself remains unproved. Parity-cylinder, live-pressure and
scale-averaged pressure bounds are sufficient arithmetic inputs; none is
established here. Earlier finite-production and localized-discrepancy
arguments are retained with their original scope, and the numerical
experiments remain observations. Neither universal termination nor
exclusion of every nontrivial cycle or unbounded orbit is established.
...
explicit exponent, by a downward recursion over five actual productions:
E, OE and OOEE (Sections 5.8--5.9 and Appendix E), which are
machine-checked and give the exponent \(5/8\), and the two five-letter
words OOOEE and OOEOE, which enter through Paper B's averaged fair-share
theorem and raise it to \(37/50\) (Section 5.10). Applied to the fate
classes: if a single start fails to reach \(1\), the failures have
logarithmic count \(\gg(\log x)^\lambda\) and, on infinitely many dyadic
blocks, natural density \(\gg(\log y)^{\lambda-1}\), for every
\(0<\lambda\le37/50\). This excludes no fate; it fixes the quantitative
shape of the trichotomy.
...
\([1,N_0]\), for some \(e>13/50\) (Theorem 3). The threshold is the
complement of the contagion exponent. The Collatz preimage lower bound
of Krasikov--Lagarias {[}7{]}, discussed with its target restriction in
Section 1.3, does not by itself yield this implication.
...
nonempty and backward-closed, and let \(0<\lambda\le37/50\). There are
\(c>0\) and \(x_0\), depending on \(A\) and \(\lambda\), with \[
...
density \(\gg(\log y)^{\lambda-1}\). (Theorems 5.19 and 5.20 and their
dyadic consequences.)

For \(\lambda\le5/8\) its log-mass bound is kernel-checked in Lean with
no hypothesis (Theorem 5.19), and Appendix E gives a written argument
for its analytic input. The range \(5/8<\lambda\le37/50\) (Theorem 5.20)
...
dyadic clause is written. The two earlier routes are retained with their
original exponents, as follows. The route of Sections 4--5 and Appendix
D --- the block average of Proposition 4.4, the monotone sweep and the
...
is \(0.4480\).) The two-production route of Section 5.8 uses no
exponential sum: the fibers whose parity share is far from \(\tfrac12\)
have finite total logarithmic mass, so the pointwise coefficient
\(\tfrac29\) of the \(OE\) family may be replaced by the averaged one,
and the recursion reaches every \(\lambda<\lambda_{\mathrm{ideal}}\),
where
\(2^{-\lambda_{\mathrm{ideal}}}+\tfrac13(3/4)^{\lambda_{\mathrm{ideal}}}=1\)
and \(\lambda_{\mathrm{ideal}}\approx0.4927\). That route is formalized
in Lean 4 with no hypothesis for every
\(\lambda\le100/203=0.49261\ldots\), which exceeds \(\lambda^{**}\); the
range \(100/203<\lambda<\lambda_{\mathrm{ideal}}\) is one choice of a
parameter \(\eta_0\) away and is not separately certified. The formal
infinite series of the finite productions has the same limiting root
\(\lambda_{\mathrm{ideal}}\). The ideal characteristic model in
...
for some \(0<\lambda\le37/50\), the starts \(n\le x\) whose orbit never
enters \([1,N_0]\) have logarithmic count \(o((\log x)^{\lambda})\);
(iii) for some \(e>13/50\) (\(e>3/8\) using only the machine-checked
Theorem 5.19) and all large \(y\),
\(\#\{n\ \text{odd}\in(y,2y]:\ n\notin R\}\le y(\log y)^{-e}\).
...
\item
  ...
  \(q=0.55\), \(C\ge28\) (\(C\ge34\)) crosses the threshold;
\item
...
needs among its open questions --- the earlier two-production exponent
improved to \(\lambda^{***}\approx0.5392\), with rate threshold
\(0.4608\) and least depth constant \(C\ge 18\). This conditional
comparison is now weaker than Theorems 5.19 and 5.20. Nothing in
Sections 2--12 depends on Appendix C.
...
\(0<\lambda\le37/50\): machine-checked up to \(5/8\), and beyond it with
the review boundary of Paper B's Theorem 6.3.
...
Diophantine, not algebraic, and their equidistribution is proved only
through the words of length five (Paper B, {[}12{]}).
...
from the parity equidistribution of nested floor powers; its Theorem 6.3
gives the fair share on almost every depth-five fiber, the input of
...
exactly the thirty-seven modules of its historical barrel and builds
with \texttt{lake\ build\ Problems.JugglerFatePaper}, without
\texttt{\nolinkurl{sorry}} and without
\texttt{\nolinkurl{native_decide}};
\texttt{\nolinkurl{formal/AxiomCheckPaperC.lean}} prints the axiom
dependencies of every name in that barrel and
...
axioms (Appendix A). \emph{Human proof}: the analytic and probabilistic
counting. \emph{Verified computation}: exact integer computations (the
descent floor is Paper A's; the closure of \([1,260]\) under the two
productions up to \(10^9\) is computed here). \emph{Observation}:
numerical experiments that check hypotheses and constants; they prove
nothing and are labelled wherever they appear.
...
Fiber parity (Lemma 4.2), goodness unpacked & Lean, through Lemma
4.1' \\
Thin fibers (Lemma 4.3): the count on \((u,2u]\) and the log-mass of the
bad \(m>U\) & Lean \\
Block average (Proposition 4.4), asymptotic & human proof; the exact
layer, the slow sum \(O(m')\), the two-sided count of the block and the
deduction of (4.1) and of the asymptotic form from the two remaining
exponential-sum bounds are Lean, those two bounds are not \\
Share law and its three corollaries (Lemma 4.5, Corollary 4.6) & human
proof; the phase expansion with its cubic remainder, the range of the
quadratic phase with its threshold \([-\tfrac56,\tfrac16]\), and the
integral \(\tfrac{25}{108}\) are Lean, the equidistribution and the
measure identifications are not \\
Production inequality (5.2) with the \(V\)-ladder terms of Section 5.7 &
human proof; the \(E\)-family and the \(OE\)-fiber family are Lean with
explicit errors, the block-average family needs the two exponential-sum
bounds and the ladder needs Appendix D \\
Theorem 5.3 at exponent \(13/40\), unconditional: two productions at the
pointwise coefficient, no hypothesis & Lean; superseded in exponent by
the averaged route below \\
Block lock (Lemma 5.14): a fiber's parity share leaves \(\tfrac12\) only
by locking onto a rational step of small denominator & Lean \\
Fiber lock (Lemma 5.15), by Dirichlet approximation & Lean \\
Resonance count, the arc count behind Theorem 5.16 & Lean \\
Poor-fiber tail (Theorem 5.16) and the share on an arbitrary set
(Corollary 5.17) & Lean \\
Theorem 5.18, contagion by two productions at the averaged coefficient:
Theorem 5.3 for every \(0<\lambda\le100/203\) with no hypothesis, and
Theorem 7.2 and Corollary 8.4 with their contagion hypothesis discharged
at \(e>103/203\) & Lean; the supremum \(\lambda_{\mathrm{ideal}}\) and
the range \(100/203<\lambda<\lambda_{\mathrm{ideal}}\) are the choice of
\(\eta_0\), human \\
Fate contagion at the failure set, log-mass form (Corollary 5.5(2),
first clause) & Lean, unconditionally at exponent \(100/203\) and, by
the pointwise route, at \(13/40\): the failure set is backward-closed by
Lemma 2.1 and nonempty by assumption, so Theorem 5.18 applies to it \\
Corollary 5.4 (natural density, infinitely often): the dyadic
pigeonhole, from the shell bound of Theorem 5.18 & Lean, for every
\(0<\lambda\le100/203\) with no hypothesis and with the constant
\(K/12\); the range \(100/203<\lambda<\lambda_{\mathrm{ideal}}\) is
human, as for Theorem 5.18 \\
Corollary 5.5 at the three fate classes: the reach-one class, the
failures, the basin of a cycle state and the divergent starts, each in
log-mass form and in dyadic-block form & Lean, at exponent \(100/203\),
by instantiating Theorem 5.18 and Corollary 5.4 through Lemma 2.1 \\
Production words \(V_k=(OE)^{k-1}OEE\) are prefix-free (Section 5.7,
Appendix D) & Lean, for the whole family and not only the six; the
disjointness of source sets it feeds needs Appendix D's analytic layer,
which is human \\
The side condition \(e(C)>\tfrac{27}{40}\) of the unconditional
criteria, at \(C=30\), by rational bounds on \(e\), \(\log 2\) and
\(\log_2 3\) & Lean; the least such \(C\) (\(23\)) and the paper's own
thresholds \(13/50\) and \(3/8\) stay with the audit \\
Cube fibers full or alternating (Lemma 4.7) & Lean \\
Recursion lemma (Lemma 5.1) & Lean \\
Seed (Lemma 5.2) & Lean \\
Contagion (Theorem 5.3), given the production inequality (5.2), for
every \(\lambda\le 0.49\) & Lean; \(\zeta(0.49)>0\) by exact rational
bounds, the root \(\lambda^{**}\) is human \\
Least failure is \(OO\)-type; first-letter trichotomy (Proposition
6.3(i), Section 6.2) & Lean \\
First-letter identity (6.1) & Lean, the exact three-piece decomposition
and the uniqueness of the odd preimage; the normalization to
\(\varphi_A,\varphi^{\rm fib}_A,\psi_A\) and the boundary error are
human \\
Tao-type rate implies the conjecture (Theorem 7.2), with the contagion
bound of Theorem 5.3 as a hypothesis & Lean \\
Almost-all equivalence (Theorem 7.3), with the contagion bound of
Theorem 5.3 as a hypothesis & Lean \\
Corollary 8.4, the conjecture from a cylinder bound, with the contagion
bound of Theorem 5.3 as a hypothesis & Lean \\
...
framework \\
One-sided form (Theorem 9.1, exact: at every \(y\) the odd failures of
\((y,2y]\) number at most
\((xa_q^{d-1}N+(x-1)\,\mathrm{err}\,(d-1)(2x)^{d-1})/x^{p_Cd}\) for
every tilt \(x\ge1\), by exponential moments) & Lean, without the
martingale; the absorption of the error into the rate and the displayed
asymptotic form are human \\
The conjecture from the one-sided hypothesis \(\mathrm H_q(C,A)\)
(Theorem 9.1's consequence), with the contagion bound as a hypothesis,
or with nothing else assumed when \(e^{\rm Ch}_q(C)>\tfrac{27}{40}\) &
Lean; the exponent is the Chernoff one of Proposition 9.3 with
\(A>C(1+\log_2x)+1+e\), the paper's numerical forms are human. The
hypothesis \(\mathrm H_q(C,A)\) itself is doubtful: see the barrier word
in Section 9.1 \\
The conjecture from the pressure hypothesis \(\mathrm P_\theta(C)\) and
from the no-momentum hypothesis \(\mathrm M_{\theta,q}(C)\) (Section
9.2's consequences), with the contagion bound as a hypothesis, or with
nothing else assumed when the exponent exceeds \(\tfrac{27}{40}\) &
Lean; the paper's \(e^{o(d)}\) is quantified as \((\log y)^\varepsilon\)
and its \(o(d)\) as \(\delta d\), the numerical forms are human \\
Theorem 9.1 with exceptional atoms (Section 10(d), first paragraph): the
share bound may fail on bad atoms of total mass \(y(\log y)^{-B}\) at
each depth, and the conjecture still follows, with nothing else assumed
when the exponent exceeds \(\tfrac{27}{40}\) & Lean; the condition is
\(B>C\log_2x+1+e\) in place of the paper's \(B>e_q(C)\), sufficient and
not sharp \\
The bias energy of the \(L(y)\)-bad words supplies the exceptional atoms
(Section 10(d)):
\(\sum_{w\ \text{bad}}D(w)^2\le(q-\tfrac12)^2y^2(\log y)^{-2B}/2^t\) at
the depths below \(\lceil CL(y)\rceil\), at all large scales, gives the
conjecture with nothing else assumed & Lean, by Cauchy--Schwarz on the
bad atoms that violate the share bound; the exact enumeration of Section
11 finds that energy at its worst-case value from depth \(16\)--\(20\)
at every computable scale, so the hypothesis has no numerical support \\
Collapsed-component bias (Section 10(d), after the measurement): the
next-letter bias of the starts whose iterate lands in a window of values
is an alternating sum of fiber sizes, bounded by the variation of the
fiber profile, the even branch smooths it, \((v+1)(M_v-m_v)+2M_v\) per
value, and a sandwich on the profile propagates through even steps
without amplification & Lean; the variation bound on a fiber profile and
the parity of the odd-preimage mass are human \\
Azuma and exponential-moment arguments (Sections 8--10, except Lemma
8.2, Theorem 8.3, Theorem 9.1, Theorem 9.2 and Proposition 9.3 in their
exact forms) & human proof \\
Cylinder-splitting identity for the first-letter bias (Section 10(d),
second equality) & Lean; the Parseval form in Walsh sums, and the
exceptional-atom estimate it is meant to supply, are human \\
Exact landing windows of the nested productions ((D.1), (D.2)) & Lean;
the smooth comparison (D.3), the multiplicities and the production
inequality they feed are human \\
Localized triple discrepancy (Appendix C) & hypothesis, conditional \\
Theorem 5.19, the log-mass bound at 5/8, and Theorems 7.2 and 9.4 and
the corollaries of Theorem 9.2 and Proposition 9.3 at the threshold 3/8
& Lean, with no contagion-side hypothesis:
\texttt{\nolinkurl{FateOOEEWeighted}} proves the actual OOEE production
from a count form of its poor-fiber tail
(\texttt{\nolinkurl{OOEEResonanceTail}}), and
\texttt{\nolinkurl{FatePressureOOEE}} gives the corollaries; Appendix E
is the written argument; the dyadic clause of Theorem 1 and Corollary
7.1 are human proofs \\
Theorem 5.20, the log-mass bound at 37/50, and Theorem 7.2 and the
corollaries of Theorem 9.2 and Proposition 9.3 at the threshold 13/50 &
human proof for its one analytic input, Theorem 6.3 of Paper B {[}12{]},
which is AI-assisted and not independently reviewed; Lean from the
finiteness of the two reciprocal sums it bounds
(\texttt{\nolinkurl{FateDepthFiveWeighted}},
\texttt{\nolinkurl{FateDepthFiveAssembly}}); Theorem 9.4 at 13/50, the
dyadic clause and Corollary 7.1 are human proofs \\
OOEE mixed modes on the actual short source interval & Lean, in
OOEEMixedModes; the poor-fiber tail and the production built on it are
Lean in \texttt{\nolinkurl{OOEEResonanceTail}} and
\texttt{\nolinkurl{FateOOEEWeighted}} \\
...
Lean: \texttt{\nolinkurl{FiberParity.fiber_parity_good}} in
\texttt{\nolinkurl{formal/Problems/Juggler/FateFiberParity.lean}}, with
goodness unpacked as \(22m^{-1/3}\le\alpha_m\le 1-22m^{-1/3}\) and
\(\alpha_m\le\tfrac12-2m^{-1/3}\) or \(\alpha_m\ge\tfrac12+2m^{-1/3}\)
(\texttt{\nolinkurl{FiberParity.Good}}). The step of \(x(n)=n\sqrt n/2\)
over two units is \((u^2+uv+v^2)/(u+v)\) with \(u=\sqrt{n+2}\),
\(v=\sqrt n\) (\texttt{\nolinkurl{xval_step}}), which lies in
\([\tfrac32v,\tfrac32u]\) by \((2u+v)(u-v)\ge 0\) and
\((u+2v)(u-v)\ge 0\) and is nondecreasing because the upper bound at
\(n\) is the lower bound at \(n+2\); no integral is needed. The upper
step is taken at \(n_{j+1}\in\Phi(m)\), so \(B_m=\tfrac32(m+1)^{2/3}\)
and \(\eta_m\le m^{-1/3}\) by Bernoulli on \((1+1/m)^{2/3}\); the fiber
has at least \(\tfrac23m^{1/3}-1\) members by Bernoulli on
\((1+1/m)^{4/3}\). Both cases go through
\texttt{\nolinkurl{Sweep.sweep_monotone_cell}} and
\texttt{\nolinkurl{sweep_monotone_ceil}} of
\texttt{\nolinkurl{FateSweepMonotone}}, with
\(\lceil 2z_j\rceil\equiv\lfloor 2x_j\rfloor\) (mod \(2\)) in Case 2.
...
Lean: \texttt{\nolinkurl{FiberParity.bad_count_le}} and
\texttt{\nolinkurl{FiberParity.bad_logMass_le}} in
\texttt{\nolinkurl{formal/Problems/Juggler/FateThinFibers.lean}}, with
badness the negation of \texttt{\nolinkurl{FiberParity.Good}} and the
log-mass bound stated for every finite range \((U,N]\). The count is the
paper's: the increments of \(\varphi\) lie in
\([(m+1)^{-1/3},m^{-1/3}]\) by Bernoulli both ways; a bad \(m>u\) has
\(\{\varphi(m)+22u^{-1/3}\}<44u^{-1/3}\) or
...
\(w/d+1\) points per integer window (\texttt{\nolinkurl{arc_count_le}}),
and at most \(\varphi(2u)-\varphi(u)+2\) windows are met. The formal
count uses \(d=(2u)^{-1/3}\), sharper than the \((3u)^{-1/3}\) above, so
...
\texttt{\nolinkurl{formal/Problems/Juggler/FateBlockAverage.lean}}. The
block is \texttt{\nolinkurl{BlockAverage.oddBlock}}, exactly the odd
\(n\) with \(m'^2\le\lfloor n^{3/4}\rfloor<(m'+1)^2\) by the landing
window of Appendix D.1 (\texttt{\nolinkurl{mem_oddBlock}});
\texttt{\nolinkurl{U_card_eq}} is the sentence that \(U(m')\) is the
disjoint union of the even-image parts of the \(\Phi(m)\) over the even
\(m\) of the block, with \texttt{\nolinkurl{oddBlock_card_eq}} the same
for the whole block; and \texttt{\nolinkurl{four_card_U}} is the
expansion \(4|U(m')|=M+S_1+S_2+S_{12}\) that the proof starts from. The
slow sum needs no analysis: it is the alternating sum
\(\sum_m(-1)^m|\Phi(m)|\) over the block
(\texttt{\nolinkurl{slowSum_eq_fibers}}), consecutive fibers differ by
at most two members (\texttt{\nolinkurl{oeFiber_card_succ_diff}}, from
\(\tfrac23m^{1/3}-1\le|\Phi(m)|\le\tfrac23(m+1)^{1/3}+1\)), and the
block pairs off, so \(|S_1|\le 2m'+\tfrac23(m'+1)^{2/3}+1\)
(\texttt{\nolinkurl{slowSum_abs_le}}). The count \(M\) is pinned
two-sidedly by the same fiber bounds
(\texttt{\nolinkurl{oddBlock_card_le}},
\texttt{\nolinkurl{oddBlock_card_ge}}), whence
\(|M/4-m'^{5/3}/3|\le m'+1\)
(\texttt{\nolinkurl{oddBlock_quarter_close}}). Given bounds on the fast
sum and the product sum only, (4.1) is
\texttt{\nolinkurl{block_average_two_bounds}}, in the paper's shape
\texttt{\nolinkurl{block_average_bound_two}} with \(C_B=C/2+2\), and the
asymptotic form is \texttt{\nolinkurl{block_average_asymptotic}} with
the explicit error \(B/2+2(m'+1)\). Those two bounds are hypotheses
there, not theorems: Vaaler's approximation, the second-derivative test
and Kusmin--Landau are not formalized, so this proposition stays a human
proof and the table says so.
...
Lean: the exact layer, in
\texttt{\nolinkurl{formal/Problems/Juggler/FateShareLaw.lean}}. The
expansion of \(x_j\) about \(n_1\) with the remainder
\(|E_j|\le\tfrac14(j-1)^3n_1^{-3/2}\) is
\texttt{\nolinkurl{ShareLaw.xval_expansion}}; its Taylor step is, after
\(v=\sqrt{1+u}\), the polynomial inequality
\(0\le1+\tfrac32(v^2-1)+\tfrac38(v^2-1)^2-v^3\le(v^2-1)^3/16\)
(\texttt{\nolinkurl{ShareLaw.taylor_three_halves}}), so no derivative is
taken, and on a fiber the remainder is at most \(\tfrac2{27}(m+1)/m^2\)
(\texttt{\nolinkurl{ShareLaw.xval_expansion_fiber}}). The range of
\(\varphi(s)=\beta s+\tfrac13s^2\) on \([0,1]\) in the three cases of
(2) is \texttt{\nolinkurl{ShareLaw.phiRange}}, attained
(\texttt{\nolinkurl{ShareLaw.exists_phi_sub_eq}}) and never exceeded
(\texttt{\nolinkurl{ShareLaw.phi_sub_le}}), and it is at most
\(\tfrac12\) exactly for \(\beta\in[-\tfrac56,\tfrac16]\)
(\texttt{\nolinkurl{ShareLaw.phiRange_le_half_iff}}). The integral of
(3), \(\int\max(0,\tfrac12-\text{range})\,d\beta=\tfrac{25}{108}\) as
\(\tfrac1{72}+\tfrac{11}{108}+\tfrac{11}{108}+\tfrac1{72}\), is
\texttt{\nolinkurl{ShareLaw.integral_extremeMeasure}}, with the
integrand vanishing outside \([-\tfrac56,\tfrac16]\)
(\texttt{\nolinkurl{ShareLaw.extremeMeasure_eq_zero}}). Not formalized:
the reduction of \(x_j\) modulo \(1\) to
...
\(\theta\)-measure with \(\max(0,\tfrac12-\text{range})\) in (2) and
(3).
...
Lean: \texttt{\nolinkurl{contagion_of_production_inequality}} and
\texttt{\nolinkurl{logMass_contagion_of_production}} in
\texttt{\nolinkurl{formal/Problems/Juggler/FateContagionBound.lean}},
with the inequality (5.2) --- the eight productions \((e_i,c_i)\) above,
with errors \(\eta_i,\eta_0\ge 0\) tending to \(0\) --- as a hypothesis
and for every \(0<\lambda\le 0.49\): the seed is Lemma 5.2
(\texttt{\nolinkurl{gA_seed}}), the induction is Lemma 5.1, and
\(\zeta(0.49)=0.00168>0\) is \texttt{\nolinkurl{zeta_pos_49}}, eight
exact rational lower bounds \(r_i\le e_i^{49/100}\) checked as
\(r_i^{100}\le e_i^{49}\); \(\zeta\) is antitone in \(\lambda\)
(\texttt{\nolinkurl{zeta_antitone}}), so \(\lambda\le 0.49\) suffices.
The root \(\lambda^{**}\) and the range \(0.49<\lambda<\lambda^{**}\)
are not formalized, and neither is (5.2) itself. The hypothesis
\(\eta_0\ge 0\) is not needed.
...
for \(t\ge40\) with
\(\eta_0(t)=2e^{-t/2}+\tfrac49e^{-3t/4}+136e^{-t/8}\)
(\texttt{\nolinkurl{Production.production_two}}, from
\texttt{\nolinkurl{Production.family_E}} on Lemma 3.1 and
\texttt{\nolinkurl{Production.family_OE}} on Lemmas 4.2 and 4.3), and
Lemma 5.1 on these two productions, with
\(2^{-13/40}+\tfrac29(\tfrac34)^{13/40}>1\) certified by
\(0.798^{40}\le2^{-13}\) and \(0.91^{40}\le(\tfrac34)^{13}\)
(\texttt{\nolinkurl{Production.zeta2_pos}}), gives Theorem 5.3 for every
\(0<\lambda\le\tfrac{13}{40}\) with no hypothesis
(\texttt{\nolinkurl{Production.contagion_elementary}},
\texttt{\nolinkurl{Production.logMass_contagion_elementary}}), and
Corollary 5.5(2) at that exponent
(\texttt{\nolinkurl{Production.failures_logMass_ge}}), in
\texttt{\nolinkurl{formal/Problems/Juggler/FateProduction.lean}}. The
root of \(2^{-\lambda}+\tfrac29(\tfrac34)^\lambda=1\) is
\(0.32612\ldots\), which \(\tfrac{13}{40}\) reaches to within \(0.4\%\);
what lifts it to \(\lambda^{**}\) is the block-average family, which
needs the two exponential-sum bounds of Proposition 4.4, and the ladder
(5.10), which needs Appendix D. Composed with Theorem 7.2 and Corollary
8.4, whose contagion hypothesis it discharges, this gives two statements
...
(\texttt{\nolinkurl{Production.conjecture_of_tao_rate}}), and so does a
cylinder bound \(\mathrm H(C,A)\) with \(e(C)>\tfrac{27}{40}\)
(\texttt{\nolinkurl{Production.conjecture_of_cylinder_bound}}). The
paper's conditional forms need \(e>1-\lambda^{**}\approx0.51\) and the
...
at \(e>103/203\)
(\texttt{\nolinkurl{Production.conjecture_of_tao_rate_averaged}},
\texttt{\nolinkurl{Production.conjecture_of_cylinder_averaged}}).
...
Lean: Corollary 5.4 is
\texttt{\nolinkurl{Density.exists_block_of_shell}} in
\texttt{\nolinkurl{formal/Problems/Juggler/FateDyadicDensity.lean}}, the
pigeonhole from a shell bound \(K(\log X)^\lambda\le\ell_A(\sqrt X,X]\)
with the constant \(K/12\): the shell is covered by the blocks
\((X/2^{j+1},X/2^j]\), \(j\le\log_2X\), at most \(3\log X\) of them, a
block's log-mass is at most \(2\,\#(A\cap(y/2,y])/y\), and
\(\log X<2\log y\) on the shell;
\texttt{\nolinkurl{Density.natDensity_averaged}} is the corollary for
every \(0<\lambda\le100/203\) with no hypothesis, on Theorem 5.18.
Corollary 5.5 is the instantiation at the fate classes through Lemma
2.1: \texttt{\nolinkurl{Density.reachesOne_logMass_averaged}} and
\texttt{\nolinkurl{Density.reachesOne_natDensity_averaged}} for \(R\);
\texttt{\nolinkurl{Production.failures_logMass_averaged}} and
\texttt{\nolinkurl{Density.failures_natDensity_averaged}} for the
failures; \texttt{\nolinkurl{Density.basin_logMass_averaged}},
\texttt{\nolinkurl{Density.basin_natDensity_averaged}},
\texttt{\nolinkurl{Density.escapes_logMass_averaged}} and
\texttt{\nolinkurl{Density.escapes_natDensity_averaged}} for the basin
of a cycle state and for the divergent starts. What stays human is the
range \(100/203<\lambda<\lambda_{\mathrm{ideal}}\), as for Theorem 5.18.
...
\(0.448\to 0.4927\) was a dynamical averaging problem for the low-even
set \(P=\{m:G_m/H_m\le 0.40\}\); Section 5.8 closes it without averaging
over \(A\), because \(P\) has finite total logarithmic mass (Theorem
5.16), so that the pointwise \(\tfrac13\) of the pairing is not needed.
The nominal exponent \(\rho_w\) gives a useful scale diagnostic for
candidate productions, but Model calculation 5.13 shows that it does not
collapse the actual nested fiber to one monomial interval. The finite
\(V_k\) constructions below use their own exact nested fibers; their
limiting transfer-matrix value \(0.4927\) is a model ceiling, not a
proved infinite-family exponent.
...
\emph{A Collatz identity.} With \(\rho_w=(\tfrac32)^b2^{-(|w|-b)}\) for
a word with \(b\) odd letters, (5.7) simplifies to
...
and has Kraft mass one, by the survivor-decay estimate of Paper B. Every
terminal multiplier is below one, and the certificate \(E\) has fair
weight \(1/2\) and multiplier \(1/2\). Its total multiplier moment is
therefore at most \(1/4+1/2=3/4\), not one. For an unbounded family,
preserving the second identity requires a separate limiting argument,
such as vanishing multiplier-weighted survivor mass. The root \(1\) of
Proposition 5.12 is Kraft equality read one exponential level up; the
depth-two model, which lacks the ascending word \(OO\), has the root
...
through these run models. Row \(r=4\) would also meet the parked \(K_3\)
kernel, but the model does not prove that this is the only obstruction.

\textbf{The finite productions of the first route to Theorem 1.} Put
...
\subsection{5.8 The averaged coefficient: two productions without
exponential
sums}\label{the-averaged-coefficient-two-productions-without-exponential-sums}
...
the rest, where the pointwise floor of Lemma 4.2 applies, and the ladder
of Section 5.7 recovers part of what the pointwise floor loses. The
split exists only to serve the block average. This section takes the
\(OE\) family over all of \(A\cap(x^{3/8},x^{3/4}]\) at once --- the
\(E\)-images are even and the \(OE\)-images odd, which is the only
disjointness Section 5.1 uses --- and replaces the pointwise coefficient
by an averaged one, at the price of discarding the fibers whose parity
share is far from \(\tfrac12\). Those fibers have finite total
logarithmic mass, so the price is nothing. No exponential sum, Vaaler
approximation or second-derivative test appears below; the tools are
Dirichlet approximation and counting, and the chain is formalized end to
end. Full proofs are in the laboratory note {[}14{]}; the statements are
given here with their constants.

\emph{Notation.} For \(m\ge10^6\) let \(\Phi(m)\) be the \(OE\) fiber of
...
\(\eta_m\le m^{-1/3}\) (Lean:
\texttt{\nolinkurl{FiberParity.xval_step}}); put \(\alpha_m=\{A_m\}\).
By Lemmas 3.2 and 4.2, \(\eta_mH_m\le0.6903\) for \(m\ge10^6\).

\textbf{Lemma 5.14 (block lock).} Let \(m\ge10^6\) and let \(p/q\) be in
lowest terms with \(q\ge1\). Then \[
\Bigl|\sigma_m-\tfrac12\Bigr|\ \le\
4\,|q\alpha_m-p|\ +\ \frac{5}{2q}\ +\ \frac{3.77\,q}{H_m}.
\]

\emph{Proof.} Cut the fiber into consecutive blocks of \(q\) members.
Inside a block the points \(\{x_j\}\) form a translate of the grid
\(\tfrac1q\mathbb Z\) perturbed by at most
\(\varepsilon=|q\alpha_m-p|+q\eta_m\), because \(\gcd(p,q)=1\) makes the
\(q\) points \(\{y+ip/q\}\) a full grid; a \(\tfrac1q\)-grid meets a
half-circle within \(\tfrac12\) of evenly, and at most
\(2\varepsilon q+1\) of its points lie within \(\varepsilon\) of a given
point, so each block's even count is within \(4\varepsilon q+\tfrac52\)
of \(q/2\); the remainder block contributes at most \(q\). Summing,
\(|G_m-\tfrac12H_m|\le4\varepsilon
H_m+5H_m/(2q)+q\), and \(1+4\eta_mH_m\le3.77\). \(\square\)

\textbf{Lemma 5.15 (fiber lock).} Let \(\eta_0\in(0,\tfrac12]\) and
...
\emph{Proof.} Put \(\theta=\eta_0/16\) and
\(H_0=\lfloor\theta H_m\rfloor\ge
...
\textbf{Theorem 5.16 (poor-fiber tail).} Let \(\eta_0\in(0,\tfrac12]\),
...
\emph{Proof.} On \((u,2u]\), \(H_m\ge\tfrac23u^{1/3}-1\) (Lemma 3.2),
and \(u\ge(1950/\eta_0^2)^3\) makes Lemma 5.15 available with
...
sequence \(q\cdot\tfrac32m^{2/3}\), whose steps and total increase scale
by \(q\), bounds the resonant \(m\) by
...
\(1.4u^{-1/3}\) and \(2.1u^{-1/3}\) over \(u\in[10^5,10^8]\) {[}14{]}.
What is used below is only that the tail vanishes: the poor set has
finite total logarithmic mass, so it can support no set at every scale.

\textbf{Corollary 5.17 (the share on an arbitrary set).} For
...
\emph{Proof.} Off \(P_{\eta_0}\) the bound holds termwise; on
\(S\cap P_{\eta_0}\) use \(\sigma_m\ge0\) and discard at most \(1/(2m)\)
per term, which Theorem 5.16 sums to at most
\(1050\,V^{-1/3}/\eta_0^2\). \(\square\)

\textbf{Theorem 5.18 (contagion by two productions).} Let \(A\) be
nonempty and backward-closed, \(0<\eta_0\le\tfrac14\), and \(\lambda>0\)
with \[
...
\(\lambda_{\mathrm{ideal}}=0.49265798\ldots\): given such a \(\lambda\),
some \(\eta_0\) has \(\zeta(\lambda,\eta_0)>0\).

\emph{Proof.} For \(m\in A\cap(x^{3/8},x^{3/4}-1]\) the fibers
\(\Phi(m)\) lie in \((\sqrt x,x]\), are pairwise disjoint and disjoint
from the \(E\)-images, each \(n\in\Phi(m)\) has \(1/n>(m+1)^{-4/3}\),
and \(H_m(m+1)^{-4/3}\ge\tfrac2{3m}(1-O(m^{-1/3}))\) by Lemma 3.2.
Corollary 5.17 with \(V=x^{3/8}\), whose error \(V^{-1/3}=x^{-1/8}\) has
the shape Lemma 4.3 already contributes, gives the \(OE\) family
log-mass at least
\(\tfrac23(\tfrac12-\eta_0)(1-O(x^{-1/8}))\,g_A(3t/4)-700x^{-1/8}/\eta_0^2\),
...
\] with \(\varepsilon_7,\varepsilon_8\to0\) for fixed \(\eta_0\). Lemma
5.1 with the seed of Lemma 5.2 finishes as in Theorem 5.3. \(\square\)

Corollaries 5.4 and 5.5 use Theorem 5.3 only through its conclusion, so
by Theorem 5.18 each holds for every
\(\lambda<\lambda_{\mathrm{ideal}}\); so do Corollary 7.1 and Theorems
7.2 and 7.3 at the matching thresholds. Sections 5.9 and 5.10 raise the
exponent further. Both corollaries are Lean at \(100/203\) (Section
5.5).

Lean: the whole chain. \texttt{\nolinkurl{BlockLock.block_lock}} in
\texttt{\nolinkurl{formal/Problems/Juggler/FateBlockLock.lean}} is Lemma
5.14 before division by \(H_m\), a statement about an arbitrary real
sequence with steps in \([a,a+\eta]\);
\texttt{\nolinkurl{FiberParity.fiber_lock}} in
\texttt{\nolinkurl{FateFiberLock.lean}} is Lemma 5.15, through
Dirichlet's theorem in lowest terms
(\texttt{\nolinkurl{exists_coprime_approx}}) and the fiber instantiation
\texttt{\nolinkurl{fiber_block_lock}};
\texttt{\nolinkurl{FiberParity.resonance_count_le'}} in
\texttt{\nolinkurl{FateResonanceCount.lean}} is the arc count;
\texttt{\nolinkurl{FiberParity.poor_count_le'}} and
\texttt{\nolinkurl{poor_logMass_le}} in
\texttt{\nolinkurl{FatePoorTail.lean}} are the two bounds of Theorem
5.16, and \texttt{\nolinkurl{nonpoor_fiber_logMass_ge}} the per-fiber
bound behind Corollary 5.17. In
\texttt{\nolinkurl{FatePoorProduction.lean}},
\texttt{\nolinkurl{Production.production_two_averaged}} is the
production inequality above, with error \(2e^{-t/8}\) on the \(OE\)
coefficient and \(2e^{-t/2}+\tfrac23e^{-3t/4}+1400e^{-t/8}/\eta_0^2\)
additive, under the one new hypothesis
\(3(1280/\eta_0^2+1)\le e^{t/8}\), which is the block condition of Lemma
5.15 at \(U=\lfloor e^{3t/8}\rfloor\) and the whole cost of averaging;
\texttt{\nolinkurl{Production.contagion_averaged}} is Theorem 5.18 with
\(\lambda\) and \(\eta_0\) free and \(\zeta(\lambda,\eta_0)>0\) as its
hypothesis. The exponent is a separate arithmetic certificate:
\texttt{\nolinkurl{Production.zeta2avg_pos}} checks
\(0.710737\le2^{-100/203}\) and \(216967/250000\le(\tfrac34)^{100/203}\)
as \(203\)rd powers in exact integer arithmetic, so that
\(\zeta(100/203,10^{-5})>2.05\cdot10^{-5}\), and
\texttt{\nolinkurl{zeta2avg_antitone}} extends it to every
\(\lambda\le100/203\). Hence
\texttt{\nolinkurl{Production.logMass_contagion_averaged}}: for every
nonempty backward-closed \(A\) and every \(0<\lambda\le100/203\), with
no hypothesis, \(\sum_{n\in A,\,n\le x}1/n\ge K(\log x)^\lambda\) for
\(x\ge x_0\), above \(\lambda^{**}=0.4925715\ldots\) of the first route;
and with it \texttt{\nolinkurl{failures_logMass_averaged}} (Corollary
5.5(2) at \(100/203\)),
\texttt{\nolinkurl{conjecture_of_tao_rate_averaged}} (Theorem 7.2 at
\(e>103/203\)) and \texttt{\nolinkurl{conjecture_of_cylinder_averaged}}
(Corollary 8.4 at \(e(C)>103/203\)). The certificate is re-derived in
exact rational arithmetic by
\texttt{\nolinkurl{oe_rest_average.exponent_certificate}}, with the
smaller denominators enumerated: \(67/136\) also lies in
\((\lambda^{**},\lambda_{\mathrm{ideal}})\) but leaves a tenth of the
margin, and \(33/67\) falls short of \(\lambda^{**}\). The break-even is
\(\eta_0=4.69\cdot10^{-5}\) at \(100/203\) and \(8.60\cdot10^{-5}\) at
\(\lambda^{**}\); the range \(100/203<\lambda<\lambda_{\mathrm{ideal}}\)
is the choice of \(\eta_0\) in Theorem 5.18 and is not separately
certified.

What Section 5.8 changed is the critical path of Theorem 1: Proposition
4.4 with its two exponential-sum bounds, the ladder of Section 5.7 with
Appendix D, and the statements of Lemmas 4.1, 4.1' and 4.2 are no longer
needed for the exponent; one step of Lemma 4.2's proof, the step
interval, is. What does not change: the supremum is approached and not
attained, \(\eta_0\) being fixed before \(x\), as in Theorem 5.3; the
constants are large (at \(\eta_0=4.3\cdot10^{-5}\),
\(u_0\approx1.2\cdot10^{36}\), \(x_0\approx10^{96}\),
\(t_1\approx221\)); and no standing question moves --- the exponent is
below \(1\), the rate of Theorem 7.2 is not proved, no cycle is excluded
and no orbit is shown to reach \(1\).

\subsection{5.9 Three actual productions and exponent
5/8}\label{three-actual-productions-and-exponent-58}
...
\textbf{Theorem 5.19 (three-production contagion).} Every nonempty
...
\(S_E(t),S_{OE}(t),S_{OOEE}(t)\) be the corresponding weighted masses of
starts in \(A\), below \(\lfloor e^t\rfloor\), with the indicated actual
initial words. The three families are disjoint. Exact even-fiber
conservation gives, for \(t\ge8\), \[
F(t/2-4)\le S_E(t).
...
has \(n^3<(m+1)^4\); an OOEE predecessor has \(n^9<64(m+1)^{16}\). Since
\(m+1\le2m\), the target cutoffs \(m\le e^{3t/4-4}\) and
\(m\le e^{9t/16-4}\) place their sources below \(e^t\). Distinct target
fibers are disjoint, and backward closure puts their sources in \(A\).

For completeness, the weighted OE coefficient follows from
\(H_m=(2/3)m^{1/3}+O(1)\), source weight \(2m^{-4/3}(1+O(1/m))\), and
target weight \(2m^{-1}(1+O(1/m))\). The actual fiber ratio is
\((2/3)\sigma_m+O(m^{-1/3})\). Outside a poor set of finite reciprocal
mass it exceeds \(33/100\), after a finite initial segment is removed.
The removed target weight is bounded independently of \(A\) and the
cutoff. The OOEE coefficient is Appendix E's arbitrary-target
consequence with \(\eta=1/900\), since \(1/9-1/900=11/100\). This proves
(5.12), including its uniform additive loss. The weighted OE step also
has a separate Lean proof.

Disjointness now yields \[
F(t)\ge F(t/2-4)+\frac{33}{100}F(3t/4-4)
                  +\frac{11}{100}F(9t/16-4)-2C.
\] Let \(G(t)=F(t-16)-5C\). For each \(r\in\{1/2,3/4,9/16\}\),
\(rt-16\le r(t-16)-4\). Monotonicity, and the coefficient sum \(36/25\),
therefore give eventually \[
G(t)\ge G(t/2)+\frac{33}{100}G(3t/4)
                  +\frac{11}{100}G(9t/16).
\tag{5.13}
\] The constant remainder is \(5C(36/25-1)-2C=C/5\ge0\). Section 5.8
implies \(F(t)\to\infty\), so \(G\) has a positive seed on a
sufficiently late compact interval. Exact rational comparisons, obtained
by taking eighth powers, give \[
2^{-5/8}\ge\frac{648}{1000},\quad
(3/4)^{5/8}\ge\frac{835}{1000},\quad
(9/16)^{5/8}\ge\frac{697}{1000}.
\] Their weighted sum is at least \(50011/50000>1\). The recursion lemma
with zero errors gives \(G(t)\ge Kt^{5/8}\). Evaluate at \(t=\log X\)
and use the weight comparison to obtain (5.11). \(\square\)

\textbf{Dyadic consequence.} There is \(c>0\) such that infinitely many
dyadic intervals \((y,2y]\) contain at least \(cy(\log y)^{-3/8}\)
members of \(A\). Otherwise their reciprocal masses are eventually at
most \(c(\log y)^{-3/8}\). Summing over \(y=2^k\) up to \(X\) bounds the
total by a constant times \(c(\log X)^{5/8}\), plus a fixed initial
mass. Choosing \(c\) below the constant in (5.11) is a contradiction.
Smaller positive exponents follow by weakening (5.11). This proves all
clauses of Theorem 1; the dyadic consequence at this new exponent is
written, not separately audited in Lean.

\textbf{Formal boundary.} \texttt{\nolinkurl{FateOOEEAssembly}} proves
disjointness, the even cutoff, the shifted recursion, the rational
certificate and the contagion and Tao implications.
\texttt{\nolinkurl{FateOEWeighted}} proves the OE input, including a
uniform error at most 6 when converting its finite weighted sums to
twice reciprocal mass. \texttt{\nolinkurl{FateOOEEWeighted}} proves the
OOEE input, \texttt{\nolinkurl{OOEEProductionBound}}, for every
backward-closed class, from the summable count-poor tail of
\texttt{\nolinkurl{OOEEResonanceTail}}; the mixed-mode cancellation is
\texttt{\nolinkurl{OOEEMixedModes}}. So
\texttt{\nolinkurl{FateOOEEWeighted.logMass_growth}} is (5.11), and
\texttt{\nolinkurl{FateOOEEWeighted.conjecture_of_tao_rate}} and
\texttt{\nolinkurl{FateOOEEWeighted.pressure_average_conjecture}} are
Theorems 7.2 and 9.4 at the threshold \(3/8\), with no contagion-side
hypothesis. These supplementary modules have their own audits, among
them \texttt{\nolinkurl{formal/AxiomCheckOOEEWeighted.lean}} and
\texttt{\nolinkurl{formal/AxiomCheckOOEECountPoorTail.lean}}, with only
\texttt{\nolinkurl{propext}}, \texttt{\nolinkurl{Classical.choice}} and
\texttt{\nolinkurl{Quot.sound}}; the historical Paper C barrel and its
473 recorded reports retain their earlier scope.

\subsection{5.10 Five actual productions and exponent
37/50}\label{five-actual-productions-and-exponent-3750}
...
Appendix B record the localized route as closed. Paper B {[}12, Theorem
6.3{]} proves an averaged substitute: almost every fiber receives its
fair share of both words. This section feeds that theorem into the
recursion of Section 5.9.

\textbf{Theorem 5.20 (five-production contagion).} Every nonempty
backward-closed set \(A\) of positive integers satisfies \[
\sum_{n\le X,\ n\in A}\frac1n\ge K(\log X)^{37/50}
\qquad(X\ge X_0)
\tag{5.14}
\] for some \(K>0,X_0\). The same conclusion holds for each realized
fate class. The proof has one input that is not formalized, {[}12,
Theorem 6.3{]}, an AI-assisted written proof that has not been
independently reviewed; the rest is kernel-checked.

\emph{Proof.} For \(a\in\{OOOEE,OOEOE\}\) and an integer \(y\ge1\), let
...
\(|\#\mathcal F_a(y)-H_a(y)/16|\ge H_a(y)/1000\). By {[}12, Theorem
6.3{]} with \(\eta=1/1000\), the poor targets of each word have a finite
reciprocal sum.
...
backward closure, carry reciprocal mass at least
\(\tfrac1{28}\cdot\tfrac1y\). The windows of distinct targets are
disjoint, the poor targets have bounded reciprocal mass, and a fixed
shift \(b\ge0\) of the target cutoff places every source below \(e^t\).
Converting reciprocal mass to the weight \(w\) as in Section 5.9 gives
constants \(b,C,T\) with \[
...
\(rt-s\le r(t-s)-4\), and \(\tfrac{27}{32}t-s\le\tfrac{27}{32}(t-s)-b\).
...
\textbf{Dyadic consequence.} As in Section 5.9, infinitely many dyadic
intervals \((y,2y]\) contain at least \(cy(\log y)^{-13/50}\) members of
\(A\), for some \(c>0\). This proves all clauses of Theorem 1; the
dyadic consequence at \(37/50\) is written, not separately audited in
Lean.

\textbf{Formal boundary.}
\texttt{\nolinkurl{FateDepthFiveWeighted.depth_five_productions}} proves
(5.15) for every backward-closed class from the finiteness of the two
reciprocal sums at \(\eta=1/1000\). It uses the fiber geometry of
\texttt{\nolinkurl{DepthFiveFibreGeometry}}: the exact windows, the
bounds above, and the identification of \(\mathcal F_a(y)\) with the
actual fibers. \texttt{\nolinkurl{FateDepthFiveAssembly}} proves the
five-way disjointness, the shifted recursion, the certificate
\texttt{\nolinkurl{certificate_37_50}}, and, given (5.15), Theorem 7.2
and the corollaries of Theorem 9.2 and Proposition 9.3 at the threshold
\(13/50\). Composed,
\texttt{\nolinkurl{FateDepthFiveWeighted.logMass_growth_of_tails}} is
Theorem 5.20 with the finiteness of the two reciprocal sums as its only
hypothesis. The audits
\texttt{\nolinkurl{formal/AxiomCheckDepthFiveWeighted.lean}} and
\texttt{\nolinkurl{formal/AxiomCheckDepthFiveAssembly.lean}} show only
\texttt{\nolinkurl{propext}}, \texttt{\nolinkurl{Classical.choice}} and
\texttt{\nolinkurl{Quot.sound}}. Paper B's Theorem 6.3 supplies the
...
exact layer of (6.1) is \texttt{\nolinkurl{first_letter_split}} there:
for any weight and any finite index set, the weighted mass of a two-way
closed class is the sum of its even, \(OE\)-type and \(OO\)-type pieces,
each indexed as the paper indexes it, by the image lying in the class;
\texttt{\nolinkurl{shellLogMass_split}} is the log-mass form on a shell
\((y,x]\). The free term's \(n(m)\) is well defined by
\texttt{\nolinkurl{floorPower_odd_injective}} (\texttt{\nolinkurl{J}} is
strictly increasing on the odd integers,
\texttt{\nolinkurl{floorPower_odd_lt}}), and
\texttt{\nolinkurl{sum_image_ooPiece}} rewrites the third piece as the
paper's sum over the odd images. What is not formalized is the
normalization: the densities \(\varphi_A\), \(\varphi^{\rm fib}_A\),
\(\psi_A\), the identification of each piece with its term of (6.1), and
the boundary error \(O(e^{-t/4}/t)\).
...
\(\gg(\log x)^\lambda\) for every \(0<\lambda\le37/50\) (Theorems 5.19
and 5.20). In the present argument the two estimates do not meet:
finance bounds the seed, contagion the growth from the seed, and no
inequality bounds a basin from above. Paper B {[}12{]} controls the
descending branches of (6.1) --- the fairness of the landing
distributions of \(E\), \(OE\) and, with its depth-4 and depth-5
theorems, of deeper contracting words --- on dyadic blocks, and for the
two depth-five words on almost every fiber (its Theorem 6.3, used in
Section 5.10); Appendix C uses one statement of that type on sub-dyadic
...
\(p_C=(1-1/C)/\log_23\), and \(e(C)/C\to1-H(\log2/\log3)=0.050044\) bits
as \(C\to\infty\), the exponent of Lagarias's Theorem D {[}3{]} that
...
(1) every \(n\ge 1\) reaches \(1\); (2) for some \(0<\lambda\le37/50\),
\(\sum_{n\le x,\ n\notin R}1/n=o((\log x)^{\lambda})\); (3) for some
\(0<\lambda\le37/50\), the starts \(n\le x\) whose orbit never enters
\([1,N_0]\) have logarithmic count \(o((\log x)^{\lambda})\).
...
backward-closed; if it were nonempty, Theorem 5.20 would contradict (2).
\(\square\)

\textbf{Theorem 7.2 (a Tao-type bound with rate implies the
conjecture).} Suppose that for some \(e>13/50\) and all sufficiently
large \(y\), \[
\#\{n\ \text{odd},\ y<n\le 2y:\ n\notin R\}\ \le\ \frac{y}{(\log y)^{e}} .
\] Then \(R=\mathbb N\).

\emph{Proof.} By decreasing \(e\) if necessary, assume \(13/50<e<1\);
the hypothesis is preserved for large \(y\). Suppose \(F\ne\emptyset\).
By Theorem 6.1 every \(n\in F\) lies in the \(E\)-tree of an odd member
of \(F\). For an odd \(n_0\in F\) the level-\(j\) set \(S_j(n_0)\) of
its \(E\)-tree has log-mass at most
...
\end{aligned}
\] By Theorem 5.20 the left side is \(\ge K(\log x)^{\lambda}\) for
every \(\lambda\le37/50\) and all large \(x\). Choosing
\(\lambda\in(1-e,37/50)\) gives a contradiction. With Theorem 5.19 in
place of Theorem 5.20 the same proof needs \(e>3/8\), and its contagion
input is machine-checked. \(\square\)

\textbf{Theorem 7.3 (equivalence).} Every positive integer reaches \(1\)
if and only if there is \(e>13/50\) such that
...
Lean: \texttt{\nolinkurl{tao_rate_iff_conjecture}} in
\texttt{\nolinkurl{formal/Problems/Juggler/FateContagionBound.lean}},
with a contagion bound as a hypothesis, as for Theorem 7.2; and
\texttt{\nolinkurl{conjecture_of_cylinder_bound_of_production}},
Corollary 8.4 with the earlier Theorem 5.3 replaced by the production
inequality (5.2), so that the chain from \(\mathrm H(C,A)\) to the
conjecture has (5.2) as its only analytic input, for any
\(\lambda\le 0.49\) with \(1-\lambda<e(C)\) (\(C\ge 19\) still suffices:
\(e(19)=0.527>0.51\)). With the contagion hypothesis discharged by
Theorem 5.18,
\texttt{\nolinkurl{Production.conjecture_of_tao_rate_averaged}} is
Theorem 7.2 at \(e>103/203\) and
\texttt{\nolinkurl{Production.conjecture_of_cylinder_averaged}} is
Corollary 8.4 at \(e(C)>103/203\), with no hypothesis on the contagion
side (Section 5.8). At the current thresholds,
\texttt{\nolinkurl{FateOOEEWeighted.conjecture_of_tao_rate}} is Theorem
7.2 at \(e>3/8\) with no contagion-side hypothesis, and
\texttt{\nolinkurl{FateDepthFiveAssembly.conjecture_of_tao_rate}} is
Theorem 7.2 at \(e>13/50\) given the depth-five productions (5.15) for
the failure set, which
\texttt{\nolinkurl{FateDepthFiveWeighted.depth_five_productions}}
derives from the finiteness of the two reciprocal sums of Section 5.10.
...
\(0.4608\). Any improvement of the contagion exponent lowers the rate
required of the almost-all statement, and \(\lambda\to 1\) would make
any positive rate suffice --- but that improvement requires all descent
certificates and is circular here (Section 5.6).
...
\emph{The analogue of Theorem 1 fails for Collatz, by exhibit.} For the
shortcut map \(n\mapsto n/2\), \(n\mapsto(3n+1)/2\), the set
\(A=\{3\cdot2^k:k\ge0\}\) is backward-closed: a multiple of three has no
odd preimage, since \(2\cdot3\cdot2^k-1\equiv2\pmod3\), so the fiber of
\(3\cdot2^k\) is \(\{3\cdot2^{k+1}\}\); the same holds for the standard
map. \(A\) is infinite, its counting function is
...
Section 5.1), whereas the Collatz fiber of \(m\) returns \(\tfrac1m\) in
the mean over \(m\) and exactly \(\tfrac1{2m}\) on every multiple of
three. Theorem 1 quantifies over every backward-closed set, so the worst
...
Numerically \(e(14)\approx0.299\), \(e(16)\approx0.389\),
\(e(18)\approx0.480\), \(e(19)\approx0.527\), \(e(20)\approx0.574\),
...
Lean: \texttt{\nolinkurl{cylinder_bound_implies_conjecture}} in
\texttt{\nolinkurl{formal/Problems/Juggler/FateCylinderCorollary.lean}}:
if \(\mathrm H(C,A)\) holds at all large scales
(\texttt{\nolinkurl{CylinderBound}}, the displayed bound on every
\(O\)-rooted \(L(y)\)-bad cylinder of depth \(d(y)\)) with \(C\ge 5\)
and \(A>C+e(C)\), and the contagion bound of Theorem 5.3 holds for the
failure set with an exponent \(\lambda\) satisfying \(1-\lambda<e(C)\),
then every positive integer reaches \(1\). The step from the explicit
bound of Theorem 8.3 to the rate \(y(\log y)^{-e}\) of Theorem 7.2, for
any \(e<e(C)\), is \texttt{\nolinkurl{oddFailures_eventually_le}}:
\(\Lambda\ge\log y/\log N_0\) turns the first term into
\((\log N_0)^{e(C)}y(\log y)^{-e(C)}\), \(\Lambda\le 2\log y/\log N_0\)
turns the second into \(2(2/\log N_0)^Cy(\log y)^{C-A}\), and both
exponent gaps are positive. The numerical thresholds \(C\ge14\) and
\(C\ge16\) are statements about \(e(C)\) at \(13/50\) and \(3/8\) and
stay with the audit.

\subsection{8.4 Constants}\label{constants}

The least integer depth constant in this sufficient criterion is
\(C=14\) (\(e(14)\approx0.299>13/50\)), and \(C=16\)
(\(e(16)\approx0.389>3/8\)) using only Theorem 5.19; the earlier route
\(\lambda^{**}\) needed \(C=19\). With the certified floor
\(N_0=3.5\cdot 10^8\), the table below keeps \(C=20\) as a conservative
a-fortiori display:
...
Paper B controls depth \(4\) (all words) and the two words of depth
\(5\) on dyadic blocks with power savings, relative errors about
\(y^{-1/24}\), \(y^{-1/48}\) and \(y^{-1/128}\); the hypothesis asks for
the bad cylinders at depth \(d(y)\to\infty\) with relative error
\(o(1)\) --- weaker in rate than a power saving, unbounded in depth.

Lean: a certified instance of the side condition, in
\texttt{\nolinkurl{formal/Problems/Juggler/FateCertified.lean}}. The
constants above are an audit script; for one concrete \(C\) the script
can be removed from the chain entirely. A logarithm is certified by an
integer power comparison --- if \(E\le e\) and \(x^n\le E^m\) then
\(n\log x\le m\) (\texttt{\nolinkurl{Certified.log_le_of_pow_le}}, and
three siblings) --- so with Mathlib's \(2.718<e<2.719\), six comparisons
(\(2^{10}\le2.718^{7}\), \(2.719^{2}\le2^{3}\), \(2^{84}\le3^{53}\),
\(3^{94}\le2^{149}\), \(2.719^{19}\le(3049/2500)^{100}\),
\((50/39)^{4}\le2.718\)) give \(\log 2\le\tfrac7{10}\) and
\(\tfrac{84}{53}\le\log_23\le\tfrac{149}{94}\), hence
\(p_{30}\in[0.6098,0.61]\), hence \(D(p_{30}\|\tfrac12)\ge0.018312\) and
\(e(30)>\tfrac{39}{50}\)
(\texttt{\nolinkurl{Certified.chernoffExponent_thirty_gt}}), with
\(e(30)<\tfrac{13}{10}\) in the other direction. So
\texttt{\nolinkurl{Certified.cylinder_bound_thirty}} is Corollary 8.4 at
\(C=30\) with every constant a numeral: a cylinder bound
\(\mathrm H(30,A)\) at all large scales with \(A\ge32\), above a
certified floor, gives the conjecture, and nothing in its proof reads a
table. \texttt{\nolinkurl{Certified.pressure_thirty}} and
\texttt{\nolinkurl{Certified.one_sided_thirty}} do the same for Theorem
9.2's and Theorem 9.1's criteria (the latter at the fair share
\(q=\tfrac12\), where the one-sided exponent is the Chernoff one,
\texttt{\nolinkurl{Certified.oneSidedExponent_half}}, and \(A\ge52\)
suffices by \texttt{\nolinkurl{Certified.logb_tilt_thirty_le}}). The
least \(C\) with \(e(C)>\tfrac{27}{40}\) is \(23\), unchanged by the
drop from \(\tfrac7{10}\): \(e(22)=0.668392\) misses the lower threshold
too, and \(e(23)=0.716046\) clears both. The paper's own thresholds
...
Lean: exact form, in
\texttt{\nolinkurl{formal/Problems/Juggler/FateOneSided.lean}}, by
exponential moments and without the martingale, carrying out the remark
after Proposition 9.3. \texttt{\nolinkurl{OneSided.badMass}} is the
tilted mass \(\sum_{|w|=t,\ w\ L\text{-bad}}\#[w]\,x^{o(w)}\) of the bad
cylinders of depth \(t\); bad words are prefix-closed
(\texttt{\nolinkurl{OneSided.LBad_of_LBad_append}}) and a cylinder
splits into its two children
(\texttt{\nolinkurl{OneSided.cylinder_split}}), so \(\mathrm H_q\) on
the bad parents gives the affine recursion
\(\mathrm{badMass}(t+1)\le a_q\,\mathrm{badMass}(t)+(x-1)\,\mathrm{err}\,(2x)^t\),
\(a_q=1+(x-1)q\), for \(1\le t<d\)
(\texttt{\nolinkurl{OneSided.badMass_succ_le}}, under
\texttt{\nolinkurl{OneSided.OneSidedShare}}, the hypothesis with its
additive error \(\mathrm{err}\) left symbolic), which unrolls to
\(xa_q^{t}N+(x-1)\,\mathrm{err}\,t\,(2x)^t\)
(\texttt{\nolinkurl{OneSided.badMass_le}}). Lemma 8.1 and the Markov
tilt put every odd failure in a bad cylinder of depth \(d\) with at
least \(p_Cd\) odd letters
(\texttt{\nolinkurl{OneSided.oddFailures_card_le_badMass}}), so
\texttt{\nolinkurl{OneSided.one_sided_bound}} is the bound
\((xa_q^{d-1}N+(x-1)\,\mathrm{err}\,(d-1)(2x)^{d-1})/x^{p_Cd}\) at every
\(y\ge1\), every \(d\ge\max(1,CL(y))\) and every tilt \(x\ge1\), with no
\(\varepsilon\); at the re-centring tilt \(x=p_C(1-q)/(q(1-p_C))\) the
main term is \((x/a_q)\,N\,e^{-dD(p_C\|q)}\)
(\texttt{\nolinkurl{OneSided.one_sided_bound_kl}}, on
\texttt{\nolinkurl{OneSided.tilt_pow_ratio}} and
\texttt{\nolinkurl{tilt_exponent_eq_kl}}), the exponent
\(e^{\rm Ch}_q(C)\) of Proposition 9.3. The error term carries
\((2x)^{d-1}/x^{p_Cd}\) where the Markov step above carries \(2^d\): the
tilt is paid on the error too, so its absorption into \(y(\log y)^{-r}\)
needs \(A>C(1+(1-p_C)\log_2x)+r\) in place of \(A>C+r\). That
absorption, the substitution \(d=\lceil CL(y)\rceil\) and the displayed
form with \(\varepsilon\) are not formalized.

So no lower bound on odd shares and no vanishing error are needed: if no
\(L(y)\)-bad cylinder of depth below \(28\log_2(\log 2y/\log N_0)\)
sends more than \(55\%\) of its members to an odd state, every positive
integer reaches \(1\) (depth \(34\log_2(\log 2y/\log N_0)\) using only
Theorem 5.19).

\textbf{The barrier word (observation; this hypothesis is doubtful).}
Unlike \(\mathrm H(C,A)\), which is asked at the single depth \(d(y)\),
\(\mathrm H_q(C,A)\) is asked at \emph{every} depth below \(d(y)\), and
the shallow depths carry an obstruction of the same shape as the
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/problems/juggler_absorbed_cylinder.md}{absorbed-cylinder
one}. Consider the \emph{barrier word} \(w=OE^{t-1}\): one odd letter,
then even letters, with \(t\) maximal subject to badness. Its prefix
walk is \(u_s=\log_23-s\), so \(w\) is \(L(y)\)-bad exactly while
\(t<L(y)+\log_23\), and its depth is \(O(L(y))\), far inside the window.
On \([w]\) the map is the plain composite
\(G=\lfloor\cdot^{1/2}\rfloor^{t-1}\lfloor n^{3/2}\rfloor\), which is
monotone, and its value sits in \((N_0,N_0^2)\) at every scale --- one
more even letter would cross the floor, one fewer leaves the value below
its square. So the image \(G((y,2y])\) is an interval of about
...
\(G(y+1)=G(2y)=4593\), an odd number: \emph{every} member of
\([OE^{13}]\) has next letter \(O\), and an explicit \(20000\)-digit
witness is given in
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/problems/juggler_barrier_collapse.md}{the
dossier}. Then \(\#[wO]=\#[w]\), so \(\mathrm H_q(C,A)\) requires
\((1-q)\#[w]\le y(\log y)^{-A}\), that is \(\#[w]\le10^{-88}y\) at
\(A=C=19\), against a fair share \(2^{-14}=6.1\cdot10^{-5}\): a deficit
of \(84\) orders of magnitude, growing without bound with \(y\). What is
not established is a lower bound on \(\#[w]\) --- a bounded-depth
nested-floor parity count, not formalized and not proved here --- so
this is not a refutation. But \(\mathrm H_q(C,A)\) should not be
presumed to hold, and Theorem 9.1 and its corollaries are stated
conditionally for that reason. The pressure and no-momentum forms of
Section 9.2 are untouched: they constrain an average over the live mass,
not each cylinder.

Lean: the consequence, in
\texttt{\nolinkurl{formal/Problems/Juggler/FateOneSidedCorollary.lean}},
in the pattern of Corollary 8.4.
\texttt{\nolinkurl{OneSided.OneSidedBound}} is \(\mathrm H_q(C,A)\) at a
scale, \texttt{\nolinkurl{OneSided.oneSidedExponent}} the exponent
\(e^{\rm Ch}_q(C)=CD(p_C\|q)/\ln2\) of Proposition 9.3, and
\texttt{\nolinkurl{OneSided.oddFailures_le_of_one_sided}} the absorption
of the exact bound above into \(y(\log y)^{-e}\) for all large \(y\),
every \(e<e^{\rm Ch}_q(C)\) and \(A>C(1+\log_2x)+1+e\) at the
re-centring tilt \(x\), on Gibbs' inequality \(D(p_C\|q)\ge0\)
(\texttt{\nolinkurl{OneSided.klDiv_nonneg}}) and
\(e^{-dD}\le\Lambda^{-e^{\rm Ch}_q(C)}\),
\((2x)^{d-1}\le\Lambda^{C(1+\log_2x)}\) for \(d=\lceil CL(y)\rceil\)
(\texttt{\nolinkurl{OneSided.exp_le_rpow_scale}},
\texttt{\nolinkurl{OneSided.pow_le_rpow_scale}}). Then Theorem 7.2:
\texttt{\nolinkurl{OneSided.implies_conjecture_of_contagion}} keeps the
contagion bound as a hypothesis at an exponent \(\lambda\) with
\(1-\lambda<e\),
\texttt{\nolinkurl{OneSided.one_sided_implies_conjecture}} discharges it
through the unconditional Theorem 5.3 at exponent \(\tfrac{13}{40}\), so
that \(\mathrm H_q(C,A)\) at all large scales with
\(e^{\rm Ch}_q(C)>\tfrac{27}{40}\) gives the conjecture with nothing
else assumed, and
\texttt{\nolinkurl{OneSided.exact_share_implies_conjecture}} is the
remark above with no error term, where \(A\) disappears. The condition
on \(A\) is sufficient, not the paper's \(A>C+e_q(C)\): the exact form
...
\(C\ge 14\) implies the conjecture using Theorem 1, and with \(C\ge 16\)
using only Theorem 5.19.
...
\texttt{\nolinkurl{formal/Problems/Juggler/FatePressureCorollary.lean}},
in the pattern of Corollary 8.4. A failure never enters the floor, so
the odd failures of \((y,2y]\) are live starts of \(\{1,\dots,2y\}\) at
every depth (\texttt{\nolinkurl{Pressure.oddFailures_subset_live}});
\texttt{\nolinkurl{Pressure.PressureBound}} is \(\mathrm P_\theta(C)\)
at a scale with \(e^{o(d)}\) quantified as \((\log y)^\varepsilon\), and
\texttt{\nolinkurl{Pressure.NoMomentumBound}} is
...
form (\texttt{\nolinkurl{Pressure.momentumExponent}}); the shared
absorption \texttt{\nolinkurl{Pressure.absorb}} turns that into
\(y(\log y)^{-e}\) for every \(e<CD/\ln2-\varepsilon\) and all large
\(y\) (\texttt{\nolinkurl{Pressure.oddFailures_le_of_pressure}},
\texttt{\nolinkurl{Pressure.oddFailures_le_of_noMomentum}}). Then
Theorem 7.2, with the contagion bound as a hypothesis
(\texttt{\nolinkurl{Pressure.pressure_conj_of_contagion}},
\texttt{\nolinkurl{Pressure.noMomentum_conj_of_contagion}}) or
discharged through the unconditional Theorem 5.3 at exponent
\(\tfrac{13}{40}\)
(\texttt{\nolinkurl{Pressure.pressure_implies_conjecture}},
\texttt{\nolinkurl{Pressure.noMomentum_implies_conjecture}}): either
hypothesis at all large scales, with its exponent above
\(\tfrac{27}{40}\), gives the conjecture with nothing else assumed. The
numerical forms are not formalized.
...
Corollaries 4.10 and 4.12) and every split to any fixed depth, or to
depth \(s_0(y)\) for any \(s_0=o(\log\log y)\), is irrelevant to the
reduction.
...
\texttt{\nolinkurl{formal/Problems/Juggler/FateOneSidedAtoms.lean}}.
\texttt{\nolinkurl{OneSided.OneSidedShareExc}} allows at each depth an
exceptional set of words of total cylinder mass at most \(\mathrm{exc}\)
on which the share bound may fail; an exceptional bad atom sends at most
its whole mass to its odd child, which costs \((x-a_q)\#[w]x^{o(w)}\) in
the tilted mass, so the recursion of Theorem 9.1's Lean proof reads
\(\mathrm{badMass}(t+1)\le a_q\,\mathrm{badMass}(t)+(x-1)\bigl(\mathrm{err}\,(2x)^t+(1-q)\,\mathrm{exc}\,x^t\bigr)\)
(\texttt{\nolinkurl{OneSided.badMass_succ_le_exc}}), with the exact
bound \texttt{\nolinkurl{OneSided.one_sided_bound_exc}} and its
re-centred form \texttt{\nolinkurl{OneSided.one_sided_bound_kl_exc}}.
With \(\mathrm{exc}=y(\log y)^{-B}\) the absorption
(\texttt{\nolinkurl{OneSided.oddFailures_le_of_exc}}) needs
\(B>C\log_2x+1+e\), against \(A>C(1+\log_2x)+1+e\) for the error: the
exceptional atoms are not doubled at each depth.
\texttt{\nolinkurl{OneSided.exc_implies_conjecture}} runs this weakest
one-sided form to the conjecture with nothing else assumed when
\(e^{\rm Ch}_q(C)>\tfrac{27}{40}\), and
\texttt{\nolinkurl{OneSided.exc_conj_of_contagion}} with the contagion
bound as a hypothesis. The condition on \(B\) is sufficient and not the
...
\texttt{\nolinkurl{CylinderEnergy.sum_bias_sq}} in
\texttt{\nolinkurl{formal/Problems/Juggler/FateCylinderEnergy.lean}},
for an arbitrary finite set of starts; it rests on the cylinder
splitting \(\#[w]=\#[wE]+\#[wO]\)
(\texttt{\nolinkurl{CylinderEnergy.wordCount_split}}) and is otherwise
the algebra of \((b-\tfrac{a+b}2)^2=\tfrac{a^2+b^2}2-\tfrac{(a+b)^2}4\).
The Parseval equality with the Walsh sums is not formalized; no Walsh
...
Lean: the supply, in
\texttt{\nolinkurl{formal/Problems/Juggler/FateEnergyAtoms.lean}}, for
the energy of the \(L(y)\)-bad words.
\texttt{\nolinkurl{Energy.badEnergy}} is \(\sum_{w\ \text{bad}}D(w)^2\)
over the odd starts of \((y,2y]\), and
\texttt{\nolinkurl{Energy.biasEnergy}} the sum over all words, equal to
\(\tfrac12\mathcal C_{t+1}-\tfrac14\mathcal C_t\)
(\texttt{\nolinkurl{Energy.biasEnergy_eq}}, on
\texttt{\nolinkurl{CylinderEnergy.sum_bias_sq}}). A bad atom violating
\(\#[wO]\le q\#[w]\) for \(q>\tfrac12\) has \(D(w)>(q-\tfrac12)\#[w]\),
so the squared masses of the bad violators sum to at most the bad energy
over \((q-\tfrac12)^2\), and Cauchy--Schwarz over at most \(2^t\) atoms
gives
\(\bigl(\sum_{\text{bad violators}}\#[w]\bigr)^2\le2^t\,\mathrm{badEnergy}/(q-\tfrac12)^2\)
(\texttt{\nolinkurl{Energy.mass_violators_le}}). Hence
\(\mathrm{badEnergy}_t\le(q-\tfrac12)^2\,\mathrm{exc}^2/2^t\) at every
depth \(1\le t<d\) is the one-sided hypothesis with exceptional atoms of
mass \(\mathrm{exc}\) and no error term
(\texttt{\nolinkurl{Energy.oneSidedShareExc_of_energy}}), and
\texttt{\nolinkurl{Energy.energy_implies_conjecture}} runs it to the
conjecture: the bad energy bound with \(\mathrm{exc}=y(\log y)^{-B}\) at
the depths below \(\lceil CL(y)\rceil\) at all large scales
(\texttt{\nolinkurl{Energy.EnergyBound}}), \(\tfrac12<q<p_C\),
\(B>C\log_2x+1+e\) and \(\tfrac{27}{40}<e<e^{\rm Ch}_q(C)\), and nothing
else. The restriction to bad words is forced: the unrestricted energy
(\texttt{\nolinkurl{Energy.badEnergy_le_biasEnergy}}) is dominated by
absorbed orbits, whose cylinders are fully biased. The exact enumeration
of Section 11 then finds the bad energy itself at its worst-case value
\(\tfrac14\sum_{w\ \text{bad}}\#[w]^2\) from depth \(16\)--\(20\) at
every computable scale, because bad words that have dipped to a bounded
value collapse onto single deterministic futures; the reduction stands,
the hypothesis has no numerical support, and the per-cylinder forms it
feeds inherit the warning.

Lean: the collapsed component, in
\texttt{\nolinkurl{formal/Problems/Juggler/FateCollapse.lean}}, the
exact layer of the observation that the collapsed mass, taken as a
whole, is nearly fair. For a finite set of starts,
\texttt{\nolinkurl{Collapse.fiber}} counts the starts whose \(t\)-th
iterate is a given value, and the next-letter bias of the starts whose
iterate lands in a window \([a,b]\) of values
(\texttt{\nolinkurl{Collapse.windowBias}}) is the alternating sum
\(-\sum_{v=a}^{b}(-1)^v\,\mathrm{fiber}(v)\)
(\texttt{\nolinkurl{Collapse.windowBias_eq_sum}}), hence at most the
total variation of the fiber profile over the window plus one fiber
(\texttt{\nolinkurl{Collapse.abs_windowBias_le}}, by summation by
parts). One step of the map sums fibers over preimages, the even numbers
of \([v^2,(v+1)^2)\) plus at most one odd preimage
(\texttt{\nolinkurl{Collapse.fiber_succ_eq}}, on Lemma 3.1's even block
in fiber form), and the even branch smooths: with the depth-\(t\)
profile between \(m_v\) and \(M_v\) on the double block
\([v^2,(v+2)^2)\), consecutive block sums differ by at most
\((v+1)(M_v-m_v)+2M_v\) (\texttt{\nolinkurl{Collapse.blockSum_sub_le}}).
Hence \texttt{\nolinkurl{Collapse.collapse_bias_le}}: the bias of a
collapsed window at depth \(t+1\) is at most
\(\sum_v\bigl((v+1)(M_v-m_v)+2M_v\bigr)\) plus twice the mass arriving
through odd preimages plus one fiber. With a smooth profile, \(M_v-m_v\)
of order \(\bar f/v\) on a block of size \(\bar fv\), the first term is
\(O(\bar f)\) per value against a window mass of \(\bar fv\) per value:
the collapsed mass entering through the even branch is fair up to a
relative \(O(1/v)\), and the excess share of the moment criterion sits
in the odd branch and in the high walkers. Runs of even letters are
self-smoothing: a sandwich \(m\le\mathrm{fiber}(t,\cdot)\le M\) on
\([v^2,(v+1)^2)\) with odd branch at most \(P\) gives
\(vm\le\mathrm{fiber}(t+1,v)\le(v+1)M+P\)
(\texttt{\nolinkurl{Collapse.fiber_succ_sandwich}}), so after two even
steps the bias of a window at depth \(t+2\) is bounded by the
depth-\(t\) sandwich with the relative oscillation not amplified and a
loss of a relative \(O(1/w)\) at the final scale
(\texttt{\nolinkurl{Collapse.collapse_bias_two_step}}); iterated, the
relative oscillation at the end of an even run is the one at its start
plus \(O(1/w)\) at the last scale. What the even run cannot repair is
the oscillation it starts with, the profile after the preceding odd run:
its first even step is the share law of Section 4, and its block average
is Proposition 4.4. Not here: any bound on the variation of a fiber
profile beyond that propagation, and the parity of the odd-preimage
mass, which is the share law at bounded scale.
...
\subsection{9.5 Scale-averaged pressure is
sufficient}\label{scale-averaged-pressure-is-sufficient}

A cumulative pressure estimate suffices even when individual scales have
large spikes. Fix \(C>1\), \(\theta>0\), \(0<q<1\), and \(\eta\ge0\).
Put \(a=1-q+qe^\theta\), \(r=C(\theta p_C-\log a)/\log2\), and at
\(y=2^k\) define \[
...
\textbf{Theorem 9.4 (scale-average criterion).} Suppose, for every
\(\varepsilon>0\), \[
\sum_{k=k_0}^{K}\rho_k\ll_\varepsilon K^{1+\eta+\varepsilon}.
\] If \(r-\eta>13/50\), Theorem 5.20 implies universal termination; with
the machine-checked Theorem 5.19 in its place, \(r-\eta>3/8\) suffices,
and with the elementary Lean baseline, \(r-\eta>103/203\). The
cumulative arithmetic bound itself remains open.

\emph{Proof.} A nonterminating start remains live and has \(o_d>p_Cd\).
Exponential Markov therefore bounds its odd reciprocal mass in the
\(k\)-th block by \(O(\rho_k(k+1)^{-r})\). Summing the cumulative
hypothesis on dyadic groups of indices gives \[
...
\(O((\log X)^\beta\log\log X)\). Choose \(\max(1+\eta-r,0)<\beta<37/50\)
and use Theorem 5.20 to contradict a nonempty failure class. For the
other two thresholds replace 37/50 by 5/8 and by 100/203. \(\square\)

The baseline implication, with the actual stopped Juggler weights, is
\texttt{\nolinkurl{ScaleAverage.pressure_average_conjecture}} in
\texttt{\nolinkurl{FateScaleAverage.lean}}; its fourteen-theorem audit
is separate from the historical Paper C audit. The formal corollary
\texttt{\nolinkurl{ScaleAverage.pressure_average_conjecture_of_ooee}}
proves the 3/8 implication with \texttt{\nolinkurl{OOEEProductionBound}}
explicit, and
\texttt{\nolinkurl{FateOOEEWeighted.pressure_average_conjecture}}
discharges that premise, so the 3/8 implication is kernel-checked with
no contagion-side hypothesis. The reusable proof accepts any supplied
contagion exponent; the 13/50 implication, through Theorem 5.20, is
written. No estimate of the actual cumulative pressure is asserted.
...
\(c=1/3\), and \(1-\lambda_{\mathrm{ideal}}=0.5073\) for \(c=1/4\).
These values describe the upper comparison, not a proved exact decay
rate of a nonempty failure set.
...
At \(C=14\), the least constant for Theorem 1's threshold, this requires
the saving exponent to lose less than a factor \(2^{1/14}=1.051\) per
depth; at \(C=28\), the one-sided constant at \(q=0.55\),
\(2^{1/28}=1.025\). Weyl differencing loses a factor \(2^{c}\) with
\(c\ge1\) per differencing (Paper B's \(OOOEE\) estimate goes
...
enough for the all-\(E\) word to reach the floor, not for the Chernoff
tail, which needs \(C\ge 5\) units. The proposition is narrow by design:
it concerns methods whose only available error estimate deteriorates
exponentially in the depth and are used through the per-cylinder count
of Theorem 8.3. A method that organizes its errors differently, that
proves a polylogarithmic relative saving uniformly in the depth (as
...
\emph{Exact cylinder energies (observation).} An exact enumeration of
every odd start of \((y,2y]\) to depth \(30\) at
\(y=10^4,10^5,10^6,10^7\)
(\texttt{\nolinkurl{research.juggler_sequence.cylinder_energy_measure}},
no sampling) records the bias energy \(\sum_wD(w)^2\) over all words,
over the \(L(y)\)-bad words and over the live starts, against the
fair-coin value \(N/4\) and the worst case \(\tfrac14\sum_w\#[w]^2\).
Over all words the worst case is reached by depth \(16\)--\(22\) at
every scale, the absorbed-cylinder obstruction of Section 8.3 in
numbers. Over the \(L(y)\)-bad words it is reached by depth
\(16\)--\(20\) as well, on cylinders of up to \(11939\) members at
\(10^7\): a bad word whose walk has dipped near \(-L(y)\) has collapsed
onto a bounded value above the floor, and its cylinder's later letters
are the deterministic orbit of that value. The mass of bad atoms
violating \(\#[wO]\le0.55\,\#[w]\) falls with the scale in the dense
window (\(0.47\), \(0.26\), \(0.12\), \(0.05\) of the bad mass at depth
\(10\)) and is a constant \(0.42\)--\(0.49\) at depth \(20\). These
finite observations do not test the hypotheses of Sections 8--10 at
their own scales; they show that their per-cylinder forms live, at every
computable scale, in a regime where cylinders are parity-constant.

\emph{The excess of the bad cylinders (observation).} The same
enumeration measures the quantity \(\mathrm H_q(C,A)\) bounds, namely
\(\max_{w\ \text{bad}}(\#[wO]-q\#[w])\), at every depth of the
hypothesis's own window \(1\le t<d(y)\)
(\texttt{\nolinkurl{research.juggler_sequence.bad_cylinder_excess}}). At
these scales the additive error is inert --- \(y(\log y)^{-A}\) is below
\(10^{-38}\) at \(y=10^7\) for \(A\ge32\) --- so the honest comparison
is with a fair coin on the same cylinder sizes, whose largest excess is
\(\sqrt{2\log W\cdot n_{\max}}/2\). The largest \(A\) the measured
excess permits over the window at \(C=19\) is \(2.62,2.84,2.61,1.59\) at
...
cylinders at depth \(4\) having odd shares \(0.53,0.53,0.52\) at
\(10^4\) and \(0.5001,0.4994,0.5008\) at \(10^7\), because the number of
distinct iterates a cylinder sits over widens with \(y\). What does not
move with \(y\) is the top of the window: the distinct iterates per bad
cylinder at \(t=d(y)-1\) are \(1.14,1.02,1.01,1.00\), so there the
cylinders are parity-constant at every scale measured, and the
hypothesis reduces to whether the mass of the largest such cylinder ---
a fraction of order \(10^{-3}\) here, with no visible decay --- falls
below \((\log y)^{-A}\). Three decades of \(y\) cannot see a
\((\log y)^{-19}\); these observations neither establish nor refute the
hypotheses of Sections 8--10. The backward computation of the barrier
word, recorded with Theorem 9.1 above and in
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/problems/juggler_barrier_collapse.md}{its
dossier}
(\texttt{\nolinkurl{research.juggler_sequence.barrier_collapse}}),
reaches \(y=10^{20000}\) and is what turns that measurement into a
statement about \(\mathrm H_q(C,A)\) at the scales where it is asserted.
...
end to end. The two five-letter words enter through Paper B's Theorem
6.3, the only input that is not formalized, and one that has not been
independently reviewed. The two-production exponent \(100/203\) remains
the elementary Lean baseline.
...
universal termination by Theorem 7.2; the machine-checked form of that
implication needs \(e>3/8\). No such arithmetic rate is proved.
Universal termination alone gives no uniform stopping-time bound here:
Theorem 7.3 concerns eventual entry.

Live pressure and its scale-averaged form are sufficient routes to the
missing rate. The no-momentum hypothesis supplies a sufficient condition
for the pressure bound. The earlier localized Appendix C and
ideal-production models retain their stated hypotheses; they do not
limit the new exponent or establish further production inequalities. No
nontrivial cycle or unbounded orbit is excluded.
...
\texttt{\nolinkurl{formal/Problems/JugglerFatePaper.lean}} imports
exactly the thirty-seven modules named here and builds with
...
Lean floor \(N_0=260\), in
\texttt{\nolinkurl{Problems/Juggler/TerminationFloor257.lean}} &
...
Shared counting of Lemmas 4.1, 4.1' and 4.3 (separated sequences in a
window), in \texttt{\nolinkurl{Problems/Juggler/FateWindowCount.lean}} &
\texttt{\nolinkurl{WindowCount.StepGe}},
\texttt{\nolinkurl{WindowCount.StepLe}},
\texttt{\nolinkurl{WindowCount.span_ge}},
\texttt{\nolinkurl{WindowCount.span_le}},
\texttt{\nolinkurl{WindowCount.mono_of_stepGe}},
\texttt{\nolinkurl{WindowCount.window_card_le}},
\texttt{\nolinkurl{WindowCount.window_card_le_nat}} \\
Proposition 4.4, the exact layer and the deduction from the
exponential-sum bounds (the bounds are hypotheses), in
\texttt{\nolinkurl{Problems/Juggler/FateBlockAverage.lean}} &
\texttt{\nolinkurl{BlockAverage.blockE}},
\texttt{\nolinkurl{BlockAverage.oddBlock}},
\texttt{\nolinkurl{BlockAverage.mem_oddBlock}},
\texttt{\nolinkurl{BlockAverage.mem_oeFiber_iff_cell34}},
\texttt{\nolinkurl{BlockAverage.U}},
\texttt{\nolinkurl{BlockAverage.oddBlock_card_eq}},
\texttt{\nolinkurl{BlockAverage.U_card_eq}},
\texttt{\nolinkurl{BlockAverage.psi}},
\texttt{\nolinkurl{BlockAverage.slowSum}},
\texttt{\nolinkurl{BlockAverage.fastSum}},
\texttt{\nolinkurl{BlockAverage.productSum}},
\texttt{\nolinkurl{BlockAverage.four_card_U}},
\texttt{\nolinkurl{BlockAverage.block_average_of_bounds}},
\texttt{\nolinkurl{BlockAverage.block_average_bound}},
\texttt{\nolinkurl{BlockAverage.psi_succ}},
\texttt{\nolinkurl{BlockAverage.abs_psi}},
\texttt{\nolinkurl{BlockAverage.oddBlock_filter_eq}},
\texttt{\nolinkurl{BlockAverage.slowSum_eq_fibers}},
\texttt{\nolinkurl{BlockAverage.oeFiber_card_succ_diff}},
\texttt{\nolinkurl{BlockAverage.abs_alt_sum_le}},
\texttt{\nolinkurl{BlockAverage.slowSum_abs_le}},
\texttt{\nolinkurl{BlockAverage.card_Ico_block}},
\texttt{\nolinkurl{BlockAverage.oddBlock_card_le}},
\texttt{\nolinkurl{BlockAverage.oddBlock_card_ge}},
\texttt{\nolinkurl{BlockAverage.oddBlock_quarter_close}},
\texttt{\nolinkurl{BlockAverage.block_average_two_bounds}},
\texttt{\nolinkurl{BlockAverage.block_average_asymptotic}},
\texttt{\nolinkurl{BlockAverage.block_average_bound_two}} \\
Section 5.1 without its exponential sums, and Theorem 5.3 at exponent
13/40 unconditionally, in
\texttt{\nolinkurl{Problems/Juggler/FateProduction.lean}} &
\texttt{\nolinkurl{Production.sqrt_floor_exp}},
\texttt{\nolinkurl{Production.floor_exp_ge_half}},
\texttt{\nolinkurl{Production.family_E}},
\texttt{\nolinkurl{Production.good_fiber_logMass_ge}},
\texttt{\nolinkurl{Production.family_OE}},
\texttt{\nolinkurl{Production.errE}},
\texttt{\nolinkurl{Production.errOE}},
\texttt{\nolinkurl{Production.errAdd}},
\texttt{\nolinkurl{Production.two_le_exp}},
\texttt{\nolinkurl{Production.one_div_floor_exp_le}},
\texttt{\nolinkurl{Production.sum_Ioc_le_sum_Ioc_pred}},
\texttt{\nolinkurl{Production.halfLogMass_split}},
\texttt{\nolinkurl{Production.production_two}},
\texttt{\nolinkurl{Production.rate2}},
\texttt{\nolinkurl{Production.coef2}},
\texttt{\nolinkurl{Production.err2}},
\texttt{\nolinkurl{Production.rate2_ge}},
\texttt{\nolinkurl{Production.rate2_le}},
\texttt{\nolinkurl{Production.rate2_pos}},
\texttt{\nolinkurl{Production.coef2_ge}},
\texttt{\nolinkurl{Production.err2_nonneg}},
\texttt{\nolinkurl{Production.err2_le}},
\texttt{\nolinkurl{Production.production_two_sum}},
\texttt{\nolinkurl{Production.zeta2_pos}},
\texttt{\nolinkurl{Production.zeta2_antitone}},
\texttt{\nolinkurl{Production.errors_le}},
\texttt{\nolinkurl{Production.errors_vanish}},
\texttt{\nolinkurl{Production.contagion_elementary}},
\texttt{\nolinkurl{Production.logMass_contagion_elementary}},
\texttt{\nolinkurl{Production.failures_logMass_ge}},
\texttt{\nolinkurl{Production.conjecture_of_tao_rate}},
\texttt{\nolinkurl{Production.conjecture_of_cylinder_bound}} \\
Corollary 5.5(2), the log-mass clause at exponent 13/40, in
\texttt{\nolinkurl{Problems/Juggler/FateProduction.lean}} &
\texttt{\nolinkurl{Production.failures_logMass_ge}} \\
Section 5.7 and Appendix D, the production words and their
prefix-freeness, in
\texttt{\nolinkurl{Problems/Juggler/FateProductionWords.lean}} &
\texttt{\nolinkurl{FateProductionWords.Vword}},
\texttt{\nolinkurl{FateProductionWords.Vword_length}},
\texttt{\nolinkurl{FateProductionWords.Vword_oddCount}},
\texttt{\nolinkurl{FateProductionWords.Vword_eq_odd_cons}},
\texttt{\nolinkurl{FateProductionWords.Vword_prefix_iff}},
\texttt{\nolinkurl{FateProductionWords.Vword_six_prefixFree}},
\texttt{\nolinkurl{FateProductionWords.Vword_five}} \\
Section 4.3, the exact layer of the share law (the expansion, the range,
the integral), in
\texttt{\nolinkurl{Problems/Juggler/FateShareLaw.lean}} &
\texttt{\nolinkurl{ShareLaw.taylor_three_halves}},
\texttt{\nolinkurl{ShareLaw.xval_expansion}},
\texttt{\nolinkurl{ShareLaw.xval_expansion_fiber}},
\texttt{\nolinkurl{ShareLaw.phi}},
\texttt{\nolinkurl{ShareLaw.phiRange}},
\texttt{\nolinkurl{ShareLaw.phi_ge_min}},
\texttt{\nolinkurl{ShareLaw.phi_le_chord}},
\texttt{\nolinkurl{ShareLaw.phi_sub_le}},
\texttt{\nolinkurl{ShareLaw.exists_phi_sub_eq}},
\texttt{\nolinkurl{ShareLaw.phiRange_le_half_iff}},
\texttt{\nolinkurl{ShareLaw.extremeMeasure}},
\texttt{\nolinkurl{ShareLaw.extremeMeasure_eq_zero}},
\texttt{\nolinkurl{ShareLaw.extremeMeasure_piece1}},
\texttt{\nolinkurl{ShareLaw.extremeMeasure_piece2}},
\texttt{\nolinkurl{ShareLaw.extremeMeasure_piece3}},
\texttt{\nolinkurl{ShareLaw.extremeMeasure_piece4}},
\texttt{\nolinkurl{ShareLaw.integral_extremeMeasure}} \\
Section 10(d), the cylinder-splitting identity, in
\texttt{\nolinkurl{Problems/Juggler/FateCylinderEnergy.lean}} &
\texttt{\nolinkurl{CylinderEnergy.itinerary_succ_append}},
\texttt{\nolinkurl{CylinderEnergy.wordCount}},
\texttt{\nolinkurl{CylinderEnergy.wordCount_split}},
\texttt{\nolinkurl{CylinderEnergy.energy}},
\texttt{\nolinkurl{CylinderEnergy.bias}},
\texttt{\nolinkurl{CylinderEnergy.energy_succ}},
\texttt{\nolinkurl{CylinderEnergy.sum_bias_sq}} \\
Appendix D.1, the exact landing windows (D.1) and (D.2), in
\texttt{\nolinkurl{Problems/Juggler/FateLandingWindow.lean}} &
\texttt{\nolinkurl{LandingWindow.cell34}},
\texttt{\nolinkurl{LandingWindow.cell34_eq_floorPower_two}},
\texttt{\nolinkurl{LandingWindow.le_cell34_iff}},
\texttt{\nolinkurl{LandingWindow.cell34_lt_iff}},
\texttt{\nolinkurl{LandingWindow.windowStart}},
\texttt{\nolinkurl{LandingWindow.windowStart_exists}},
\texttt{\nolinkurl{LandingWindow.windowStart_le_iff}},
\texttt{\nolinkurl{LandingWindow.lt_windowStart_iff}},
\texttt{\nolinkurl{LandingWindow.exact_endpoints}},
\texttt{\nolinkurl{LandingWindow.setOf_cell34_mem_Ico}},
\texttt{\nolinkurl{LandingWindow.exact_endpoints_iterate}},
\texttt{\nolinkurl{LandingWindow.setOf_iterate_mem_Ico}} \\
Lemma 4.1 (sweep), in
...
Section 6.2, Proposition 6.3(i) and the exact layer of (6.1), in
...
\texttt{\nolinkurl{first_letter_split}},
\texttt{\nolinkurl{shellLogMass}},
\texttt{\nolinkurl{shellLogMass_split}},
\texttt{\nolinkurl{floorPower_odd_lt}},
\texttt{\nolinkurl{floorPower_odd_injective}},
\texttt{\nolinkurl{ooPiece}}, \texttt{\nolinkurl{sum_image_ooPiece}},
\texttt{\nolinkurl{card_image_ooPiece}} \\
...
Corollary 8.4 (the conjecture from a cylinder bound), in
\texttt{\nolinkurl{Problems/Juggler/FateCylinderCorollary.lean}} &
\texttt{\nolinkurl{CylinderBound}}, \texttt{\nolinkurl{one_le_depth}},
\texttt{\nolinkurl{chernoffExponent_nonneg}},
\texttt{\nolinkurl{oddFailures_eventually_le}},
\texttt{\nolinkurl{cylinder_bound_implies_conjecture}} \\
Shared numerics of Lemmas 4.2, 4.3 and 8.2 (roots by powering, Bernoulli
at a point), in \texttt{\nolinkurl{Problems/Juggler/FateNumerics.lean}}
& \texttt{\nolinkurl{Numerics.rpow_le_iff_pow}},
\texttt{\nolinkurl{Numerics.le_rpow_iff_pow}},
\texttt{\nolinkurl{Numerics.rpow_lt_iff_pow}},
\texttt{\nolinkurl{Numerics.lt_rpow_iff_pow}},
\texttt{\nolinkurl{Numerics.bernoulli_ge}},
\texttt{\nolinkurl{Numerics.bernoulli_le}} \\
Lemma 4.2 (fiber parity), in
\texttt{\nolinkurl{Problems/Juggler/FateFiberParity.lean}} &
\texttt{\nolinkurl{FiberParity.xval}},
\texttt{\nolinkurl{FiberParity.two_xval}},
\texttt{\nolinkurl{FiberParity.floor_two_xval}},
\texttt{\nolinkurl{FiberParity.cell_xval_even_iff}},
\texttt{\nolinkurl{FiberParity.xval_step}},
\texttt{\nolinkurl{FiberParity.xval_step_ge}},
\texttt{\nolinkurl{FiberParity.xval_step_le}},
\texttt{\nolinkurl{FiberParity.xval_step_mono}},
\texttt{\nolinkurl{FiberParity.oeFiber}},
\texttt{\nolinkurl{FiberParity.mem_oeFiber}},
\texttt{\nolinkurl{FiberParity.oeFiber_eq_image}},
\texttt{\nolinkurl{FiberParity.oeFiber_card}},
\texttt{\nolinkurl{FiberParity.evenImageCount}},
\texttt{\nolinkurl{FiberParity.fiber_ge_rpow}},
\texttt{\nolinkurl{FiberParity.fiber_lt_rpow}},
\texttt{\nolinkurl{FiberParity.rpow_four_thirds_succ_ge}},
\texttt{\nolinkurl{FiberParity.rpow_four_thirds_succ_le}},
\texttt{\nolinkurl{FiberParity.rpow_two_thirds_succ_le}},
\texttt{\nolinkurl{FiberParity.oeFiber_card_ge}},
\texttt{\nolinkurl{FiberParity.oeFiber_card_le}},
\texttt{\nolinkurl{FiberParity.Am}},
\texttt{\nolinkurl{FiberParity.alpha}},
\texttt{\nolinkurl{FiberParity.eps}},
\texttt{\nolinkurl{FiberParity.Good}},
\texttt{\nolinkurl{FiberParity.eps_le}},
\texttt{\nolinkurl{FiberParity.step_ge}},
\texttt{\nolinkurl{FiberParity.step_le}},
\texttt{\nolinkurl{FiberParity.fiber_parity_good}} \\
Lemma 4.3 (thin fibers), in
\texttt{\nolinkurl{Problems/Juggler/FateThinFibers.lean}} &
\texttt{\nolinkurl{FiberParity.span_ge_of_step}},
\texttt{\nolinkurl{FiberParity.arc_count_le}},
\texttt{\nolinkurl{FiberParity.Am_step_le}},
\texttt{\nolinkurl{FiberParity.Am_step_ge}},
\texttt{\nolinkurl{FiberParity.eps_antitone}},
\texttt{\nolinkurl{FiberParity.bad_mem_arc}},
\texttt{\nolinkurl{FiberParity.two_rpow_third_le}},
\texttt{\nolinkurl{FiberParity.two_rpow_two_thirds_le}},
\texttt{\nolinkurl{FiberParity.rpow_two_thirds_ge}},
\texttt{\nolinkurl{FiberParity.eps_div_eps_double}},
\texttt{\nolinkurl{FiberParity.Am_double_sub_le}},
\texttt{\nolinkurl{FiberParity.bad_count_le}},
\texttt{\nolinkurl{FiberParity.bad_block_logMass_le}},
\texttt{\nolinkurl{FiberParity.bad_sum_dyadic_le}},
\texttt{\nolinkurl{FiberParity.eps_pow_two_mul}},
\texttt{\nolinkurl{FiberParity.two_rpow_neg_third_le}},
\texttt{\nolinkurl{FiberParity.bad_logMass_le}} \\
Theorem 5.3 given (5.2), Theorem 7.3, Corollary 8.4 through (5.2), in
\texttt{\nolinkurl{Problems/Juggler/FateContagionBound.lean}} &
\texttt{\nolinkurl{productionRate}},
\texttt{\nolinkurl{productionCoeff}},
\texttt{\nolinkurl{productionRate_pos}},
\texttt{\nolinkurl{productionRate_ge}},
\texttt{\nolinkurl{productionRate_le}},
\texttt{\nolinkurl{productionRate_le_one}},
\texttt{\nolinkurl{productionCoeff_ge}},
\texttt{\nolinkurl{productionCoeff_nonneg}}, \texttt{\nolinkurl{zeta}},
\texttt{\nolinkurl{zeta_antitone}},
\texttt{\nolinkurl{le_rpow_div_of_pow_le}},
\texttt{\nolinkurl{zeta_pos_49}}, \texttt{\nolinkurl{seedConst}},
\texttt{\nolinkurl{gA}}, \texttt{\nolinkurl{gA_seed}},
\texttt{\nolinkurl{logMass_ge_gA}},
\texttt{\nolinkurl{contagion_of_production_inequality}},
\texttt{\nolinkurl{logMass_contagion_of_production}},
\texttt{\nolinkurl{oddFailures_eq_empty}},
\texttt{\nolinkurl{tao_rate_iff_conjecture}},
\texttt{\nolinkurl{conjecture_of_cylinder_bound_of_production}} \\
Proposition 4.4 (its two exponential-sum bounds, the fast sum and the
product sum; the exact layer, the slow sum, the block count and the
deduction of (4.1) and of the asymptotic form from those two bounds are
Lean), the share law 4.5 and Corollary 4.6 (the phase expansion, the
range of the quadratic phase and the integral 25/108 are Lean, the
equidistribution and the measure identifications are not), the
production inequality (5.2) (its \texttt{\nolinkurl{E}}-family and
\texttt{\nolinkurl{OE}}-fiber family are Lean with explicit errors and
give Theorem 5.3 at exponent 13/40 unconditionally, and the averaged
\texttt{\nolinkurl{OE}} family of Section 5.8 gives it at 100/203; the
block-average family and the ladder are not Lean and are no longer
needed for the exponent), Corollaries 5.4 and 5.5 in the range
\(100/203<\lambda\le37/50\) (at \(100/203\) both are Lean, in
\texttt{\nolinkurl{Problems/Juggler/FateDyadicDensity.lean}}), Sections
8--10 except Lemma 8.2, the explicit form of Theorem 8.3, Corollary 8.4,
the exact form of Theorem 9.2 and Proposition 9.3, Appendix C; Theorem
5.3 in the range \(100/203<\lambda<\lambda_{\mathrm{ideal}}\), and
Theorems 7.2, 7.3 and Corollary 8.4 with the contagion bound as a
hypothesis outside the exponents of Sections 5.8--5.10 & human proofs \\
Theorem 9.1 (one-sided form, exact, by exponential moments), in
\texttt{\nolinkurl{Problems/Juggler/FateOneSided.lean}} &
\texttt{\nolinkurl{OneSided.cylinder_split}},
\texttt{\nolinkurl{OneSided.LBad_of_LBad_append}},
\texttt{\nolinkurl{OneSided.sum_allWords_succ}},
\texttt{\nolinkurl{OneSided.sum_pow_oddCount_le}},
\texttt{\nolinkurl{OneSided.badWeight}},
\texttt{\nolinkurl{OneSided.badWeight_nonneg}},
\texttt{\nolinkurl{OneSided.badWeight_le_card}},
\texttt{\nolinkurl{OneSided.badMass}},
\texttt{\nolinkurl{OneSided.OneSidedShare}},
\texttt{\nolinkurl{OneSided.badMass_succ_le}},
\texttt{\nolinkurl{OneSided.card_cylinder_zero_le}},
\texttt{\nolinkurl{OneSided.badMass_one_le}},
\texttt{\nolinkurl{OneSided.badMass_le}},
\texttt{\nolinkurl{OneSided.oddFailures_card_le_badMass}},
\texttt{\nolinkurl{OneSided.one_sided_bound}},
\texttt{\nolinkurl{OneSided.klDiv}}, \texttt{\nolinkurl{OneSided.tilt}},
\texttt{\nolinkurl{OneSided.tilt_ge_one}},
\texttt{\nolinkurl{OneSided.tilt_pow_ratio}},
\texttt{\nolinkurl{OneSided.one_sided_bound_kl}} \\
Theorem 9.1's consequence (the conjecture from the one-sided
hypothesis), in
\texttt{\nolinkurl{Problems/Juggler/FateOneSidedCorollary.lean}} &
\texttt{\nolinkurl{OneSided.OneSidedBound}},
\texttt{\nolinkurl{OneSided.OneSidedExact}},
\texttt{\nolinkurl{OneSided.oneSidedExponent}},
\texttt{\nolinkurl{OneSided.OneSidedShare.mono}},
\texttt{\nolinkurl{OneSided.oneSidedBound_of_exact}},
\texttt{\nolinkurl{OneSided.klDiv_nonneg}},
\texttt{\nolinkurl{OneSided.exp_le_rpow_scale}},
\texttt{\nolinkurl{OneSided.pow_le_rpow_scale}},
\texttt{\nolinkurl{OneSided.oddFailures_le_of_one_sided}},
\texttt{\nolinkurl{OneSided.implies_conjecture_of_contagion}},
\texttt{\nolinkurl{OneSided.one_sided_implies_conjecture}},
\texttt{\nolinkurl{OneSided.exact_share_implies_conjecture}} \\
Section 9.2's consequences (the conjecture from the pressure and
no-momentum hypotheses), in
\texttt{\nolinkurl{Problems/Juggler/FatePressureCorollary.lean}} &
\texttt{\nolinkurl{Pressure.oddFailures_subset_live}},
\texttt{\nolinkurl{Pressure.PressureBound}},
\texttt{\nolinkurl{Pressure.NoMomentumBound}},
\texttt{\nolinkurl{Pressure.momentumExponent}},
\texttt{\nolinkurl{Pressure.absorb}},
\texttt{\nolinkurl{Pressure.oddFailures_le_of_pressure}},
\texttt{\nolinkurl{Pressure.pressure_conj_of_contagion}},
\texttt{\nolinkurl{Pressure.pressure_implies_conjecture}},
\texttt{\nolinkurl{Pressure.oddFailures_le_of_noMomentum}},
\texttt{\nolinkurl{Pressure.noMomentum_conj_of_contagion}},
\texttt{\nolinkurl{Pressure.noMomentum_implies_conjecture}} \\
Section 10(d), Theorem 9.1 with exceptional atoms, in
\texttt{\nolinkurl{Problems/Juggler/FateOneSidedAtoms.lean}} &
\texttt{\nolinkurl{OneSided.OneSidedShareExc}},
\texttt{\nolinkurl{OneSided.oneSidedShareExc_of_share}},
\texttt{\nolinkurl{OneSided.OneSidedBoundExc}},
\texttt{\nolinkurl{OneSided.oneSidedBoundExc_of_bound}},
\texttt{\nolinkurl{OneSided.badMass_succ_le_exc}},
\texttt{\nolinkurl{OneSided.badMass_le_exc}},
\texttt{\nolinkurl{OneSided.one_sided_bound_exc}},
\texttt{\nolinkurl{OneSided.main_term_eq}},
\texttt{\nolinkurl{OneSided.one_sided_bound_kl_exc}},
\texttt{\nolinkurl{OneSided.pow_le_rpow_scale_gen}},
\texttt{\nolinkurl{OneSided.tail_le}},
\texttt{\nolinkurl{OneSided.oddFailures_le_of_exc}},
\texttt{\nolinkurl{OneSided.exc_conj_of_contagion}},
\texttt{\nolinkurl{OneSided.exc_implies_conjecture}} \\
Section 10(d), the bias energy supplying the exceptional atoms, in
\texttt{\nolinkurl{Problems/Juggler/FateEnergyAtoms.lean}} &
\texttt{\nolinkurl{OneSided.OneSidedShareExc.mono_err}},
\texttt{\nolinkurl{Energy.bias}}, \texttt{\nolinkurl{Energy.energyOn}},
\texttt{\nolinkurl{Energy.biasEnergy}},
\texttt{\nolinkurl{Energy.badEnergy}},
\texttt{\nolinkurl{Energy.violators}},
\texttt{\nolinkurl{Energy.badEnergy_le_biasEnergy}},
\texttt{\nolinkurl{Energy.card_allWords}},
\texttt{\nolinkurl{Energy.mass_violators_le}},
\texttt{\nolinkurl{Energy.oneSidedShareExc_of_energy}},
\texttt{\nolinkurl{Energy.wordCount_cylinder}},
\texttt{\nolinkurl{Energy.biasEnergy_eq}},
\texttt{\nolinkurl{Energy.EnergyBound}},
\texttt{\nolinkurl{Energy.oneSidedBoundExc_of_energy}},
\texttt{\nolinkurl{Energy.energy_conj_of_contagion}},
\texttt{\nolinkurl{Energy.energy_implies_conjecture}} \\
Section 10(d), the collapsed component, in
\texttt{\nolinkurl{Problems/Juggler/FateCollapse.lean}} &
\texttt{\nolinkurl{Collapse.fiber}},
\texttt{\nolinkurl{Collapse.window}},
\texttt{\nolinkurl{Collapse.windowBias}},
\texttt{\nolinkurl{Collapse.card_filter_window}},
\texttt{\nolinkurl{Collapse.card_window}},
\texttt{\nolinkurl{Collapse.windowBias_eq_sum}},
\texttt{\nolinkurl{Collapse.abs_alt_sum_le}},
\texttt{\nolinkurl{Collapse.Icc_eq_Ico}},
\texttt{\nolinkurl{Collapse.abs_windowBias_le}},
\texttt{\nolinkurl{Collapse.lt_sq_succ_of_floorPower_eq}},
\texttt{\nolinkurl{Collapse.pre}},
\texttt{\nolinkurl{Collapse.iterate_succ_eq}},
\texttt{\nolinkurl{Collapse.fiber_succ}},
\texttt{\nolinkurl{Collapse.pre_filter_even}},
\texttt{\nolinkurl{Collapse.blockSum}},
\texttt{\nolinkurl{Collapse.oddPart}},
\texttt{\nolinkurl{Collapse.oddPart_nonneg}},
\texttt{\nolinkurl{Collapse.fiber_succ_eq}},
\texttt{\nolinkurl{Collapse.evenCount}},
\texttt{\nolinkurl{Collapse.evenCount_le}},
\texttt{\nolinkurl{Collapse.le_evenCount}},
\texttt{\nolinkurl{Collapse.blockSum_sub_le}},
\texttt{\nolinkurl{Collapse.sum_abs_oddPart_sub_le}},
\texttt{\nolinkurl{Collapse.collapse_bias_le}},
\texttt{\nolinkurl{Collapse.fiber_succ_sandwich}},
\texttt{\nolinkurl{Collapse.collapse_bias_two_step}} \\
Section 8.4, the unconditional criteria at \texttt{C\ =\ 30} with every
constant a numeral, in
\texttt{\nolinkurl{Problems/Juggler/FateCertified.lean}} &
\texttt{\nolinkurl{Certified.e_ge}},
\texttt{\nolinkurl{Certified.e_le}},
\texttt{\nolinkurl{Certified.log_le_of_pow_le}},
\texttt{\nolinkurl{Certified.le_log_of_pow_le}},
\texttt{\nolinkurl{Certified.logb_two_le}},
\texttt{\nolinkurl{Certified.le_logb_two}},
\texttt{\nolinkurl{Certified.log_two_le}},
\texttt{\nolinkurl{Certified.le_log_two}},
\texttt{\nolinkurl{Certified.le_logb_three}},
\texttt{\nolinkurl{Certified.logb_three_le}},
\texttt{\nolinkurl{Certified.le_log_lo}},
\texttt{\nolinkurl{Certified.le_log_hi}},
\texttt{\nolinkurl{Certified.log_lo_le}},
\texttt{\nolinkurl{Certified.log_hi_le}},
\texttt{\nolinkurl{Certified.logb_three_pos}},
\texttt{\nolinkurl{Certified.pC_thirty_ge}},
\texttt{\nolinkurl{Certified.pC_thirty_le}},
\texttt{\nolinkurl{Certified.klHalf_ge_of}},
\texttt{\nolinkurl{Certified.klHalf_le_of}},
\texttt{\nolinkurl{Certified.klHalf_thirty_ge}},
\texttt{\nolinkurl{Certified.klHalf_thirty_le}},
\texttt{\nolinkurl{Certified.chernoffExponent_thirty_gt}},
\texttt{\nolinkurl{Certified.chernoffExponent_thirty_lt}},
\texttt{\nolinkurl{Certified.cylinder_bound_thirty}},
\texttt{\nolinkurl{Certified.pressure_thirty}},
\texttt{\nolinkurl{Certified.klDiv_half}},
\texttt{\nolinkurl{Certified.oneSidedExponent_half}},
\texttt{\nolinkurl{Certified.logb_tilt_thirty_le}},
\texttt{\nolinkurl{Certified.one_sided_thirty}} \\
Lemma 5.14 (block lock), in
\texttt{\nolinkurl{Problems/Juggler/FateBlockLock.lean}} &
\texttt{\nolinkurl{BlockLock.gridRes}},
\texttt{\nolinkurl{BlockLock.fract_grid}},
\texttt{\nolinkurl{BlockLock.gridRes_mul_inj}},
\texttt{\nolinkurl{BlockLock.grid_reindex}},
\texttt{\nolinkurl{BlockLock.grid_half_count}},
\texttt{\nolinkurl{BlockLock.grid_near_count}},
\texttt{\nolinkurl{BlockLock.span_bounds}},
\texttt{\nolinkurl{BlockLock.block_count}},
\texttt{\nolinkurl{BlockLock.block_chain}},
\texttt{\nolinkurl{BlockLock.block_lock}} \\
Lemma 5.15 (fiber lock), in
\texttt{\nolinkurl{Problems/Juggler/FateFiberLock.lean}} &
\texttt{\nolinkurl{FiberParity.evenImageCount_eq_fract}},
\texttt{\nolinkurl{FiberParity.eps_mul_Hlen_le}},
\texttt{\nolinkurl{FiberParity.exists_coprime_approx}},
\texttt{\nolinkurl{FiberParity.fiber_block_lock}},
\texttt{\nolinkurl{FiberParity.fiber_lock}} \\
The resonance count behind Theorem 5.16, in
\texttt{\nolinkurl{Problems/Juggler/FateResonanceCount.lean}} &
\texttt{\nolinkurl{FiberParity.Resonant}},
\texttt{\nolinkurl{FiberParity.resonant_mem_arc}},
\texttt{\nolinkurl{FiberParity.Am_window_le}},
\texttt{\nolinkurl{FiberParity.shifted_arc_count_le}},
\texttt{\nolinkurl{FiberParity.resonance_count_one}},
\texttt{\nolinkurl{FiberParity.resonance_count_le}},
\texttt{\nolinkurl{FiberParity.resonance_count_le'}} \\
Theorem 5.16 (poor-fiber tail) and the non-poor fiber bound of Corollary
5.17, in \texttt{\nolinkurl{Problems/Juggler/FatePoorTail.lean}} &
\texttt{\nolinkurl{FiberParity.Poor}},
\texttt{\nolinkurl{FiberParity.oeFiber_nonempty}},
\texttt{\nolinkurl{FiberParity.resonant_mono}},
\texttt{\nolinkurl{FiberParity.poor_resonant}},
\texttt{\nolinkurl{FiberParity.poor_count_le}},
\texttt{\nolinkurl{FiberParity.poor_count_le'}},
\texttt{\nolinkurl{FiberParity.poor_block_logMass_le}},
\texttt{\nolinkurl{FiberParity.poor_sum_dyadic_le}},
\texttt{\nolinkurl{FiberParity.poor_logMass_le}},
\texttt{\nolinkurl{FiberParity.nonpoor_fiber_logMass_ge}} \\
Theorem 5.18 and its consequences (Section 5.8), in
\texttt{\nolinkurl{Problems/Juggler/FatePoorProduction.lean}} &
\texttt{\nolinkurl{Production.family_OE_averaged}},
\texttt{\nolinkurl{Production.errOEavg}},
\texttt{\nolinkurl{Production.errAddAvg}},
\texttt{\nolinkurl{Production.production_two_averaged}},
\texttt{\nolinkurl{Production.coef2avg}},
\texttt{\nolinkurl{Production.err2avg}},
\texttt{\nolinkurl{Production.coef2avg_ge}},
\texttt{\nolinkurl{Production.coef2avg_nonneg}},
\texttt{\nolinkurl{Production.err2avg_nonneg}},
\texttt{\nolinkurl{Production.err2avg_le}},
\texttt{\nolinkurl{Production.production_two_averaged_sum}},
\texttt{\nolinkurl{Production.errorsAvg_le}},
\texttt{\nolinkurl{Production.errorsAvg_vanish}},
\texttt{\nolinkurl{Production.zeta2avg_antitone}},
\texttt{\nolinkurl{Production.contagion_averaged}},
\texttt{\nolinkurl{Production.zeta2avg_pos}},
\texttt{\nolinkurl{Production.logMass_contagion_averaged}},
\texttt{\nolinkurl{Production.failures_logMass_averaged}},
\texttt{\nolinkurl{Production.conjecture_of_tao_rate_averaged}},
\texttt{\nolinkurl{Production.conjecture_of_cylinder_averaged}} \\
Corollary 5.4 and Corollary 5.5 at the fate classes (Section 5.5), in
\texttt{\nolinkurl{Problems/Juggler/FateDyadicDensity.lean}} &
\texttt{\nolinkurl{Density.blockCount}},
\texttt{\nolinkurl{Density.block_logMass_le}},
\texttt{\nolinkurl{Density.block_index}},
\texttt{\nolinkurl{Density.halfLogMass_le_blocks}},
\texttt{\nolinkurl{Density.exists_block_of_shell}},
\texttt{\nolinkurl{Density.natDensity_of_shell_bound}},
\texttt{\nolinkurl{Density.natDensity_averaged}},
\texttt{\nolinkurl{Density.reachesOne_logMass_averaged}},
\texttt{\nolinkurl{Density.reachesOne_natDensity_averaged}},
\texttt{\nolinkurl{Density.failures_natDensity_averaged}},
\texttt{\nolinkurl{Density.basin_logMass_averaged}},
\texttt{\nolinkurl{Density.basin_natDensity_averaged}},
\texttt{\nolinkurl{Density.escapes_logMass_averaged}},
\texttt{\nolinkurl{Density.escapes_natDensity_averaged}} \\*
\end{longtable}

\endgroup

The supplementary modules of Theorems 5.19 and 5.20 lie outside this
barrel: \texttt{\nolinkurl{FateOEWeighted}},
\texttt{\nolinkurl{FateOOEEAssembly}},
\texttt{\nolinkurl{OOEEMixedModes}},
\texttt{\nolinkurl{OOEEResonanceTail}} and
\texttt{\nolinkurl{FateOOEEWeighted}} for Theorem 5.19, and
\texttt{\nolinkurl{DepthFiveFibreGeometry}},
\texttt{\nolinkurl{FateOOOEEAssembly}},
\texttt{\nolinkurl{FateDepthFiveAssembly}} and
\texttt{\nolinkurl{FateDepthFiveWeighted}} for Theorem 5.20. The axiom
checks \texttt{\nolinkurl{formal/AxiomCheckOOEEWeighted.lean}},
\texttt{\nolinkurl{formal/AxiomCheckOOEECountPoorTail.lean}},
\texttt{\nolinkurl{formal/AxiomCheckDepthFiveAssembly.lean}} and
\texttt{\nolinkurl{formal/AxiomCheckDepthFiveWeighted.lean}} print the
axioms of the cited theorems, whose dependencies include the other
modules; only the three axioms above occur. Each is run with
\texttt{lake\ env\ lean\ \textless{}file\textgreater{}} in
\texttt{\nolinkurl{formal}}. Their sources and audits are pinned in the
release manifest, \texttt{\nolinkurl{docs/theory/paper_c_release.json}}.
...
+ \(V_6=OEOEOEOEOEOEE\) (\(\lambda^{**}\), earlier finite-production
route; elementary, audited) &
\(\ldots,(\tfrac1{2187},\tfrac{729}{8192})\) & \(0.4926\) \\
...
two productions at the averaged coefficient, \(\eta_0\downarrow0\)
(\(\lambda_{\mathrm{ideal}}\), Theorem 5.18; Lean to \(100/203\)) &
\((\tfrac13-\tfrac23\eta_0,\tfrac34)\) & \(0.4927\) \\
pairing + \(OOEEE\) + \(OOOEE\), \(OOEOE\) on even blocks, localized
(closed: fibers \(P^{5/32}\); the averaged route is Theorem 5.20) &
pairing terms, \((\tfrac19,\tfrac9{32})\) and
\((\tfrac2{27},\tfrac{27}{64})\) & \(0.6066\) \\
three actual productions (Theorem 5.19; Lean) &
\((\tfrac{33}{100},\tfrac34),(\tfrac{11}{100},\tfrac9{16})\) &
\(0.6266\) \\
five actual productions (Theorem 5.20; Paper B's Theorem 6.3) &
\(\ldots,(\tfrac1{14},\tfrac{27}{32})\) & \(0.7406\) \\
...
\emph{Depth constants.} At the current thresholds, \(13/50\) from
Theorem 5.20 and \(3/8\) from Theorem 5.19, the least integer values are
these; Arb certifies each value and the failure of the integer below it.
...
criterion
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Theorem 5.20, rate \(13/50\)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Theorem 5.19, rate \(3/8\)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\endlastfoot
fair Chernoff & 14 & 16 \\
one-sided Azuma, \(q=0.5,0.55,0.60,0.62\) & 14, 28, 132, 866 & 16, 34,
175, 1201 \\
...
The historical least integer values below use the three explicitly named
rate thresholds; the threshold \(1-\lambda_{\mathrm{ideal}}=0.5073\) of
the second route gives the same integers as \(0.5074\) in every column;
intermediate \(V_2\) through \(V_5\) have complementary thresholds
\(0.5199\), \(0.5109\), \(0.5084\), and \(0.5076\), respectively.
...
\item
  ...
  SHA-256:
  \texttt{\nolinkurl{d322b8549e2239c5fc78279c7a579332766a3287811f73ee4d225d6eb9e5cc7b}}
\item
  \texttt{\nolinkurl{data/research/juggler/tao_reduction/summary.json}}

  SHA-256:
  \texttt{\nolinkurl{2316e5339e3670d283c1983a5b5b0c11bc92101ab327c477072b7897619a587f}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateContagion.lean}}

  SHA-256:
  \texttt{\nolinkurl{834198a6b0e8cc13c9fb91d9ad938f62ce5e783a3cc9009fb38334c913d5b7e2}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/CubeFiber.lean}}

  SHA-256:
  \texttt{\nolinkurl{2b0add4108651218c43fb741a207d3d6860525a2ad13ad1341ece9fc5c37f8e9}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/TiltedShare.lean}}

  SHA-256:
  \texttt{\nolinkurl{7dcafa45a2a0e9bca182a713b495d791cf812e4c70d6472c67bf85b0b736aef4}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateRecursion.lean}}

  SHA-256:
  \texttt{\nolinkurl{e6d050acc924867abcf5c03b4a3e59b62363f40bbba452e2ced83afe474dd435}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateFirstLetter.lean}}

  SHA-256:
  \texttt{\nolinkurl{a383c715010cb1e7323359a414aeaa99242550f59f7213049cc528232e37875a}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateBlockAverage.lean}}

  SHA-256:
  \texttt{\nolinkurl{5cef5c273577376b72799732865415a91c6b91f8bdb06e79bd398d9c478d0b07}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateProduction.lean}}

  SHA-256:
  \texttt{\nolinkurl{7cc24ff9f95692ead6b594aa703677910daf4936f24becce7b13da2c4105df2e}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateProductionWords.lean}}

  SHA-256:
  \texttt{\nolinkurl{a712db485e4c48029ce9a9afcad1cfea1240f4bb4dda1cb7537661ae1f3fdb3e}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateOneSided.lean}}

  SHA-256:
  \texttt{\nolinkurl{7ae0aa4cab061a24ef51eff381a0f4bbd4e4569ae2578f04e4c3677b5999bddc}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateOneSidedCorollary.lean}}

  SHA-256:
  \texttt{\nolinkurl{481b4cecbebd7291d551b814f733b9cfd78dec50a936a31fd9ab6544408c714e}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FatePressureCorollary.lean}}

  SHA-256:
  \texttt{\nolinkurl{cdd1ca9cbb59c4bb0a5fb24702f12bb8e27c21365f9b28e515819f874e9b437e}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateOneSidedAtoms.lean}}

  SHA-256:
  \texttt{\nolinkurl{db2601560b54b95a5ac5b2f5f16c0d83e3b4cf4182f1d20795c47db26c0a9ce3}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateEnergyAtoms.lean}}

  SHA-256:
  \texttt{\nolinkurl{e98ccccbb25140d41ede236466f7e0f522c433a4c80fcaa330abfb1388653b1a}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateCollapse.lean}}

  SHA-256:
  \texttt{\nolinkurl{56ba2f731fdd02ea1d19a6e6ef909534947c8c30b48828c72f8009901ed06e06}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateCertified.lean}}

  SHA-256:
  \texttt{\nolinkurl{ff47a23462fb591264125859b9d511bb2cec046da7a71fbad5d17b35a5581c90}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateShareLaw.lean}}

  SHA-256:
  \texttt{\nolinkurl{a229f4fa4cf48cb5b003c746672947eec10bf93d7287ad7eecaa3f84d5236e7c}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateCylinderEnergy.lean}}

  SHA-256:
  \texttt{\nolinkurl{274cd6b960bdd1de8c1b637bd2dee86c8cbe67e883e25937812dadcbe98d6a0b}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateLandingWindow.lean}}

  SHA-256:
  \texttt{\nolinkurl{ef28985363d3b855e0a9ec4b8fba687855d34ab8cf1ff9b355a50e9377e29545}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateWindowCount.lean}}

  SHA-256:
  \texttt{\nolinkurl{9d790163433899c73993a24b434ff20ae45aba3dbe47b717507dba840307b6a3}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateSweep.lean}}

  SHA-256:
  \texttt{\nolinkurl{a5ede4a6d25cec71d6b6c7fb99e577df3335366175dd22f361581456669a72cc}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateChernoff.lean}}

  SHA-256:
  \texttt{\nolinkurl{3b808649443805fa3dd41b89333afe886f3cbc7e36f1bdae23f6c93fe95a7032}}
\item
...
\item
  ...
  SHA-256:
  \texttt{\nolinkurl{e2ffc98f56268a31826b86f54115e0fed949e62651d05936d545cd829761b4d2}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateNumerics.lean}}

  SHA-256:
  \texttt{\nolinkurl{8340b3a8a4ff2ce35bd77b7697ca95b9d36e5d61ea4173782287d0839b381737}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateFiberParity.lean}}

  SHA-256:
  \texttt{\nolinkurl{bf2b1c6058c3d00f0f9cf24f11bb7bee7a17a75375c2ddc7223e065621e64131}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateThinFibers.lean}}

  SHA-256:
  \texttt{\nolinkurl{3fbcd3e9f7adf13967e7494691aa8f62779630e0446fe81e813486947478cb74}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateContagionBound.lean}}

  SHA-256:
  \texttt{\nolinkurl{d8991fd05c076875a0e8fd75692a10bb31930c289a3812de07212b620645acca}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateBlockLock.lean}}

  SHA-256:
  \texttt{\nolinkurl{b4fdc02e5b41b8759156f5aa1b3f208854d2e6c994a3963e24aa164df498bed7}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateFiberLock.lean}}

  SHA-256:
  \texttt{\nolinkurl{f82aceb5a06642f7f55ea1b1bbee317ea487c46e97cbf52aecbd920dc9d10007}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateResonanceCount.lean}}

  SHA-256:
  \texttt{\nolinkurl{f39eca082166b63d15989946e8ae54279bb9a4b779d5440d0c970212883f4aa1}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FatePoorTail.lean}}

  SHA-256:
  \texttt{\nolinkurl{773c579e2ba303dc49f1488020ea142b0b35ea26573707ebfd6afca7c4dd284b}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FatePoorProduction.lean}}

  SHA-256:
  \texttt{\nolinkurl{0fc332ef6b54bebbe4de215b03ac582a2d0cc38d82a391d23856624599940283}}
\item
  \texttt{\nolinkurl{formal/Problems/Juggler/FateDyadicDensity.lean}}

  SHA-256:
  \texttt{\nolinkurl{f5065c952302de36981e147f5ae95804eaa16d34a38459025a65e10b5a24e4fe}}
\item
  \texttt{\nolinkurl{formal/Problems/JugglerFatePaper.lean}}

  SHA-256:
  \texttt{\nolinkurl{ce57dde1ab5f47f98277f4b870641f864727a4136b617627773e531e2b6753c2}}
\item
  \texttt{\nolinkurl{formal/AxiomCheckPaperC.expected}}

  SHA-256:
  \texttt{\nolinkurl{b8569df4c76a94d8665940b04dba86739d773a796d8683e7801ede5c654ddc24}}
\item
  \texttt{\nolinkurl{src/research/juggler_sequence/fate_contagion.py}}

  SHA-256:
  \texttt{\nolinkurl{27b94f873d4cc9fc4f424443db058c8b3f7c6217e9bdbde7cc17c984829e9c46}}
\item
  \texttt{\nolinkurl{src/research/juggler_sequence/tao_reduction.py}}

  SHA-256:
  \texttt{\nolinkurl{7d4cc3e0dbf197232c084e0aa6fe89cff7deb77bdab2d52523f5bcfc58a9e14f}}
\item
  \texttt{\nolinkurl{docs/theory/figures/render_paper_c_figures.py}}

  SHA-256:
  \texttt{\nolinkurl{bf0e053dfa143c903900ca81410502a1ba3b29b9d79d8ba067eb93d0598d00a0}}
\end{itemize}
...
\section{Appendix C. An earlier conditional
strengthening}\label{appendix-c.-an-earlier-conditional-strengthening}
...
\emph{Status.} An earlier working draft of Paper B stated Hypothesis L
as a theorem. Its argument removed the twist by partial summation after
the Weyl differencing and ran a seven-step argument (exact linearization
...
what \emph{is} proved in this appendix is everything that follows from
it. (The derivation below followed that draft.)
...
now give stronger constants without Hypothesis L (Appendix B). Pairing
plus \(OOEEE\) sits at \(0.5392\), above the depth-two ceiling
\(0.4927\), because \(OOEEE\) is an extra production. A further
production from the words \(OOOEE\) and \(OOEOE\) on even blocks, at
coefficient \(\tfrac2{27}\) and scale \(\tfrac{27}{64}\), would give
\(\lambda\approx0.6066\) and \(C=17\) in this model (earlier versions
printed \(0.5561\), which no recursion of this form gives). Those fibers
have length \(P^{5/32}\), below the threshold of the localized triple,
and the trivial bound on the kernel proof of an earlier draft of Paper B
covers the whole interval, so no analogue of Hypothesis L for that
kernel estimate is available (fate-contagion note §7.4). That concerns
this appendix's localized route only. The averaged route of Section 5.10
needs no analogue of Hypothesis L: Paper B's Theorem 6.3 gives the fair
share on almost every such fiber, and Theorem 5.20 uses both words at
exponent \(37/50\). Hypothesis L concerns consecutive odd starts in an
interval and says nothing about the odd images \(S_{\rm odd}\); the free
term \(\psi_F\) is untouched by this appendix.
...
Lean: (D.1) is \texttt{\nolinkurl{LandingWindow.exact_endpoints}} and
\texttt{\nolinkurl{LandingWindow.setOf_cell34_mem_Ico}}, and the nested
window (D.2) is
\texttt{\nolinkurl{LandingWindow.exact_endpoints_iterate}}, in
\texttt{\nolinkurl{formal/Problems/Juggler/FateLandingWindow.lean}}.
There \(F\) is \texttt{\nolinkurl{cell34}} and
\(\Phi(a)=\lceil a^{4/3}\rceil\) is \texttt{\nolinkurl{windowStart}},
the least \(n\) with \(a^4\le n^3\), which is what the ceiling means in
exact arithmetic; the two are a Galois connection, and that is the whole
proof. \texttt{\nolinkurl{cell34_eq_floorPower_two}} is \(J^2=F\) on an
\(OE\) step. The smooth comparison (D.3), the endpoint error and the
multiplicities are not formalized.
...
\section{Appendix E. Actual OOEE poor-fiber
tail}\label{appendix-e.-actual-ooee-poor-fiber-tail}

This appendix supplies the written analytic input to Theorem 5.19. It
derives the short-interval estimate needed for the actual nested floors,
with all four parity guards. It does not invoke Paper B's global
estimate as an unproved localization.

\subsection{E.1 Statement and exact
objects}\label{e.1-statement-and-exact-objects}
...
\(O^2(n)\) and \(Q(O^2(n))\) are even, and \(Q^2(O^2(n))=m.\) Thus
\(F_m\) is the actual OOEE predecessor fibre, with all four parity
guards. Write
...
\textbf{Theorem E.1 (poor-fibre tail).} For every fixed \(0<\eta<1/9,\)
...
No numerical onset is asserted. The proof also applies to the set where
\(|R_m-1/9|>\eta.\) It does not claim uniform convergence at every
target. The exact empty fibres in the branch dossier are consistent with
(E.1).
...
Indeed \(O(n)\ge a\) iff \(n^3\ge a^2,\) and \(Q^2(v)=m\) iff
\(m^4\le v<(m+1)^4.\) The ceilings give

\[
 m^{16/9}\le A_m\le m^{16/9}+2.
\]

Consequently, with \(P=m^{16/9}\) and \(H_m\) the number of odd integers
in \(I_m,\)
...
of its real endpoints; integer counts differ by at most a constant. On
odd n in \(I_m,\) \(w(n)=2/P\) times 1+O(1/m), uniformly. Also
...
The larger error \(m^{-7/9},\) rather than \(m^{-1},\) retains the
integer rounding in \(H_m.\) All constants in (E.2)--(E.4) are absolute.

\subsection{E.2 Classical analytic
inputs}\label{e.2-classical-analytic-inputs}

Write \(e(t)=\exp(2\cdot \pi\cdot i\cdot t).\) We use the following
classical forms on arbitrary real intervals, retaining an absolute
endpoint term when necessary:
...
\item
  If \(\lambda\le |f''|\le C\) lambda, then the second-derivative bound
  on an interval of length L is \(O_C(1+L\)
  \(sqrt(\lambda)+\lambda^{-1/2}).\)
\item
  For N consecutive terms of modulus at most one and integer
  \(1\le D\le N,\) van der Corput differencing bounds the square of
  their sum by \(O(N^2/D+(N/D)\) \(sum_{1\le h<D}|T_h|),\) with overlap
  correlations \(T_h.\)
\item
  The one- and three-dimensional Erd\H{o}s--Tur\'{a}n--Koksma
  inequalities bound unnormalized interval or box discrepancy by the
  count divided by K, plus the finite Fourier sums weighted by the
  product of \(1/\max(1,|k_j|),\) for frequencies at most K.
\end{itemize}

These are the same tools as in
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/theory/juggler_parity_discrepancy_note.md}{Paper
B, Sections 3--4}. For accessible statements and a proof of the
differencing inequality, see
\href{https://math.univ-cotedazur.fr/~pjammes/publications/diviseurs95.pdf}{Jammes,
Section 2}. The second-derivative bound is also stated as Theorem E.10
in
\href{https://people.math.osu.edu/bergelson.1/BR_DensityCoprime.pdf}{Bergelson--Richter}.
The discrepancy reference is Kuipers--Niederreiter, \(\cdot Uniform\)
Distribution of \(Sequences\cdot ,\) Chapter 2, already indexed as
\texttt{\nolinkurl{kuipers-niederreiter-1974-uniform-distribution}}. No
qualitative Hardy-field or global-count localization is used.

The finite differencing bullet is now kernel-checked, with constants 2
and 4 and exact overlap length N-h, in
\href{../../formal/BTCalculus/WeylDifferencing.lean}{WeylDifferencing.lean}.
Its exponential-sum specialization works directly on the odd lattice.
The
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/theory/finite_weyl_differencing_note.md}{proof
mapping} distinguishes this classical input from its phase-specific
application. The
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/theory/first_derivative_power_cancellation_note.md}{first-derivative
estimate} and the
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/theory/second_derivative_cancellation_note.md}{quantitative
second-derivative test} are now also kernel-checked. The latter has
constants 4 and 8 on the unit lattice, and 8 and 4 on the odd lattice.
Uniform curvature of the actual carry-cell phases and their unweighted
\(O(P^{3/8})\) sums are now kernel-checked in
\texttt{\nolinkurl{OOEECurvature.lean}}, including the growing shift
range and explicit floor conditions. The
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/theory/juggler_ooee_curvature_note.md}{proof
mapping} retains the extra endpoint required by the derivative test. The
\href{https://github.com/sneakyweasel/btlab/blob/main/docs/theory/juggler_ooee_mixed_modes_note.md}{mixed-mode
proof} now supplies the carry partition, weighted sums, Fourier errors,
original phase comparison and differencing. Pure slow modes and
three-coordinate discrepancy are not formalized as such; the Lean route
to the production (\texttt{\nolinkurl{OOEEResonanceTail}},
\texttt{\nolinkurl{FateOOEEWeighted}}) proves a count form of the tail
by its own argument.

\subsection{E.3 The short mixed
estimate}\label{e.3-the-short-mixed-estimate}

For real \(x\ge 3\) define \(X(x)=x^{3/2},\) \(Z(x)=x^{9/8};\) for
integer n put \(M(n)=floor(X(n)),\) \(Y(n)=M(n)^{3/2}.\)

\textbf{Lemma E.2 (fixed mixed modes).} Fix constants \(K,L_0\ge 1.\)
For every integer triple (i,j,k) with \(\max(|i|,|j|,|k|)\le K\) and
\((i,j)\ne (0,0),\) and every interval I in {[}P,2P{]} of length at most
\(L_0\) \(P^{7/16},\)
...
\subsubsection{E.3.1 The carry approximation on a short
interval}\label{e.3.1-the-carry-approximation-on-a-short-interval}
...
with \(E_R=1\) at integers. The ordinary truncated Fourier series
satisfies \(b=b_R+O(E_R),\) including at integers; Paper B Lemma 4.3
proves this pointwise assertion. We re-estimate its total error at the
current length.
...
counts exact integer values of X. The same bound holds for X(n+2h) by
translation of the odd interval. No dyadic exceptional set has been
multiplied by a short-interval length ratio.

\subsubsection{E.3.2 Differencing, with a bounded number of carry
cells}\label{e.3.2-differencing-with-a-bounded-number-of-carry-cells}

Suppose \(j\ne 0.\) Conjugating if necessary, take \(u=j/2>0;\) then
\(1/2\le u\le K/2.\) Use odd-lattice shifts 2h with
\(1\le h\le D=floor(P^{1/16}).\) The overlap interval still has length
\(O_{L_0}(P^{7/16}).\) Write
...
The one-signed linearization, proved by factorization in Paper B Lemma
7.1, is
...
cells, including the partial endpoint cells. On each cell freeze
\(G=floor(\delta)\) and let \(z=\delta-G.\) Then \(0\le z<1\) is
monotone, G is of order h sqrt(P), and
...
This follows by substituting (E.11) into \((1-kappa)e(F_{G,0})+kappa\)
\(e(F_{G,1}).\) There is no independence assumption.

\subsubsection{E.3.3 Every local error
term}\label{e.3.3-every-local-error-term}

The identity \(A_h(x)=x^{9/4}\) a(2h/x), with
\(a(t)=(3/2)((1+t)^{3/4}-1)-(1/2)((1+t)^{9/4}-1),\) has
\(a(0)=a'(0)=0.\) Writing \(a(t)=t^2\) \(a_2(t),\) whose first two
derivatives are bounded near zero, shows \(A_h''=O(h^2\) \(P^{-7/4}).\)
Thus on each fixed cell
...
bounded ratio, uniformly in \(h\le P^{1/16}.\) The second-derivative
estimate and partial summation for z and 1-z bound the first two terms
of (E.12) by
...
The endpoint term \(P^{3/8}\) is retained. There are only \(O_{L_0}(1)\)
cells.

Replace the two b functions in (E.12) by \(b_R.\) By (E.7) and
\(|e(F_1)-e(F_0)|\le 2,\) their total remainder is \(O(P^{5/16}\) log
P), even after the partition. Each nonzero Fourier mode now has phase
\(F_{G,\epsilon}+rX(x)\) or \(F_{G,\epsilon}+rX(x+2h).\) Its curvature
is comparable to \(|r|P^{-1/2}:\) the competing curvature divided by
that quantity is \(O_K(h\) \(P^{-1/4}/|r|)=O_K(P^{-3/16}).\) Hence the
modes, summed with their O(1/\textbar r\textbar) coefficients over
\(1\le |r|\le R=P^{1/8},\) cost
...
Combining (E.9), (E.14), (E.7), and (E.15), every overlap correlation
obeys \(|T_h|=O_{K,L_0}(P^{3/8}).\) In particular, the carry error is
smaller than this bound; log P is absorbed by the gap \(3/8-5/16=1/16.\)
...
If \(N<D\) the trivial estimate is already stronger. This proves (E.5)
for \(j\ne 0.\) If \(j=0\) and \(i\ne 0,\) the undifferenced smooth
phase has curvature comparable to \(|i|P^{-1/2},\) since the k term has
curvature \(O_K(P^{-7/8}).\) Its sum is \(O_{K,L_0}(P^{1/4}),\) which
also proves (E.5). This completes Lemma E.2.

\subsection{E.4 The pure slow modes and the actual last
guard}\label{e.4-the-pure-slow-modes-and-the-actual-last-guard}
...
Indeed replacing sqrt(floor(Y)) by \(sqrt(Y)=M^{3/4}\) costs
\(O(P^{-9/8});\) replacing \(M=X-theta\) by X in the 3/4 power costs
\(O(P^{-3/8}).\) The mean value theorem applies on positive intervals
bounded below by constant multiples of the relevant powers. Thus
replacing Z by sqrt(V) in a fixed-frequency exponential sum costs
\(O_K(P^{1/16}),\) negligible in (E.5). We use (E.17) for exponential
sums, not to assert equality of floor parities.

Put \(\alpha_m=(9/8)m^{2/9}.\) In the odd coordinate \(n=2t+1,\) the
derivative of \((k/2)n^{9/8}\) is \((9k/8)n^{1/8}.\) On \(I_m\) it
differs from k \(\alpha_m\) by at most
...
for an absolute \(C_0.\) This follows from (E.3) and the derivative
bound for \(x^{1/8},\) since \(P^{-7/8}\) \(P^{7/16}=m^{-7/9}.\) The
derivative is monotone.
...
For sufficiently large m the derivative of each pure slow phase stays in
one interval between consecutive integers and at distance at least
...
\subsection{E.5 Joint parity, resonance inclusion, and the
tail}\label{e.5-joint-parity-resonance-inclusion-and-the-tail}
...
The box {[}1/2,1) times {[}0,1/2) times {[}0,1/2) specifies exactly the
three remaining OOEE guards. Its volume is 1/8. Half-open boundaries
encode integer-floor parity exactly, including square and cube
coincidences.

For fixed K and C as above and m satisfying (E.19), Lemma E.2 controls
every mode with \((i,j)\ne (0,0),\) and (E.20) controls the pure slow
modes. Therefore
...
the indicated constants. For any \(\epsilon>0,\) first choose K so the
first term is less than epsilon/3; then choose \(C>2\) \(C_0\) K so the
second is less than epsilon/3; finally choose m large so the remaining
terms total less than epsilon/3. This order keeps K and C independent of
m.
...
For example, take \(\epsilon=\eta/4\) and enlarge \(m_\eta\) until the
error in (E.4) is at most eta/4. Strict poor inequalities
\(R_m<1/9-\eta\) are contained in this absolute-deviation set.

For fixed q, the derivative of q \(\alpha_m\) is \((q/4)m^{-7/9}.\) On
{[}M,2M), each window within \(C_\eta\) \(M^{-7/9}\) of an integer has
preimage length at most 8 \(C_\eta\) \(2^{7/9}/q.\) There are O(q
\(M^{2/9}+C_\eta+1)\) possible integer levels. Counting integers in
...
The same bound for deficient targets follows beyond \(m_\eta;\)
increasing its constant absorbs the finite initial set. Division by M
and summation over dyadic blocks starting at U yields (E.1). This proves
Theorem E.1.

\subsection{E.6 What the coefficient now
supplies}\label{e.6-what-the-coefficient-now-supplies}
...
The physical source cutoffs and three-production recursion are proved in
Section 5.9. This appendix is the written argument, pending independent
review. The production it feeds is also kernel-checked, by its own
formal route: \texttt{\nolinkurl{OOEEMixedModes}} for the mixed modes,
\texttt{\nolinkurl{OOEEResonanceTail}} for the summable count-poor tail
and \texttt{\nolinkurl{FateOOEEWeighted}} for the production at
coefficient \(11/100\).
...
the prose, proposed proof arguments, the Lean formalizations, and the
...
Appendix A. The author is responsible for the statements, proofs, code,
and the decision to make this version public.
...
of 21 September 2026, which added Section 5.8 with its Lean layer, the
corresponding rows of Section 1.4 and Appendices A and B, the remarks of
Sections 5.7, 6.3 and 7.1, and the exponent \(\lambda_{\mathrm{ideal}}\)
in the statements, and 1.0.0
...
resolves to the latest version; the record of version 1.1.0 is at
\url{https://zenodo.org/records/22865705}. The author's ORCID is
...
keeps the concept DOI, rather than as a new deposit. The changes between
versions are listed in the build guide,
\texttt{\nolinkurl{PAPER_C_BUILD.md}}, in the repository. The
accompanying source package contains the Markdown and LaTeX sources,
figures, the local Lean dependency closure with its pinned toolchain,
numerical audit code, archived experiment records, and a manifest of
file hashes. The repository is
...
\item
  ...
  for all versions; revision of 21 September 2026 at
...
\item
  ...
  Parity Statistics of Nested Floor Powers'' (Paper B), Zenodo version
  ...
  same repository. Its Theorem 6.3 and Appendix D are the input of
  Section 5.10.
\item
...
\item
  P. Cochin, research notes on fate contagion, the Tao-type reduction
  and the poor-fiber tail for the Juggler map, 2026;
  \texttt{\nolinkurl{docs/theory/juggler_fate_contagion_note.md}},
  \texttt{\nolinkurl{docs/theory/juggler_tao_reduction_note.md}} and
  \texttt{\nolinkurl{docs/theory/juggler_oe_poor_fiber_tail_note.md}},
  same repository.
\item
