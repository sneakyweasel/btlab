...
2. [juggler_parity_discrepancy_note.md](../../docs/theory/juggler_parity_discrepancy_note.md) — Paper B: parity statistics of nested floor powers (9 September proof repair). Theorem 5.2 proves unconditional four-step certificate density 13/16 through the restricted mixed-sum estimate in Section 4.2. Theorem 5.3 adds the OOEOE split and a five-step certificate subfamily of density 27/32. Full five-step density 7/8 requires only OOOEE in Theorem 5.4. The general decorated kernel and short-interval bounds remain unproved; see docs/theory/paper_b_ooeoe_report.md. The D2 follow-up repairs its error expansion (docs/theory/paper_b_d2_report.md); the combined signed zero-offset family now has bound O_epsilon(P^(31/32+epsilon)) in docs/theory/paper_b_signed_waves_report.md. The wave-bearing family with widened signed D1 terms and specified D2 factors now has the same exponent in docs/theory/paper_b_wave_bearing_report.md. The nonzero-offset anchor family at zero total Y frequency now has exponent 23/24 with corrected composite coefficient 81/64 in docs/theory/paper_b_offset_anchor_report.md. The weaker undecorated dyadic kernel is assembled with exponent 127/128 in docs/theory/paper_b_kernel_assembly_report.md. The subsequent [OOOEE transfer](../../docs/theory/paper_b_oooee_transfer_report.md) proves its exact four-coordinate modes with exponent 127/128, the OOOEE count N/32, and full five-step certificate density 7/8 as AI-assisted written research results. Independent review and manuscript consolidation remain outstanding; the released 27/32 files are unchanged. The historical 95/96 target, arbitrary decorations, and short-interval localization remain unproved. Audit ledger: [paper_b_audit_ledger.md](../../docs/theory/paper_b_audit_ledger.md) + `paper_b_audit`.
...
- **Termination.** The OOOEE research supplement proves full five-step
  power-envelope certificate density \(7/8\), with count error
  \(O_\varepsilon(N^{127/128+\varepsilon})\)
  (J-paper-b-five-step-density-127). This is an AI-assisted written
  result, pending independent mathematical review. The existing
  Paper B manuscript still prints the unconditional \(27/32\)
  subfamily and conditional full count; consolidation is outstanding.
  No termination theorem follows.
