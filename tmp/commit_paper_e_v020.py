...
ROW_ID = 'J-paper-e-formal-notation-bridges'
FILES = [
...
def scoped(path, before):
    ...
    if path == 'docs/research_journal.md':
        start = '## 2026-09-22 -- Paper E closes its four smaller Lean coverage gaps\n'
    ...
    if path == 'formal/Problems/Juggler.lean':
        line = 'import Problems.Juggler.PaperECompletion\n'
        anchor = 'import Problems.Juggler.FateContagionBound\n'
    elif path == 'src/research/juggler_sequence/lean_paths.py':
        line = next(x for x in live.splitlines(True) if '"PaperECompletion":' in x)
...
expected_file = ROOT/'tmp/paper_e_v020_expected_commit.json'
patch_file = ROOT/'tmp/paper_e_v020_scoped.patch'
if '--normalize-ledger' in sys.argv:
    ...
    print('Prepared a scoped Paper E 0.2.0 commit with',len(expected),'files.')
else:
    ...
    run('commit','-m','Close Paper E notation gaps in Lean and release version 0.2.0')
