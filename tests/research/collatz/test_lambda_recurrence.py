...
def test_the_landing_set_is_oeis_a190812() -> None:
    """The values trajectories land on are exactly one OEIS sequence.

    A190812 is "increasing sequence generated by a(1)=1 and the rules
    `x -> 2x+1`, `x -> 3x+2`". In the coordinate `u = n+1` those rules are
    `u -> 2u` and `u -> 3u` starting from `u = 2`, so the sequence is the odd
    `n` with `n + 1` 3-smooth -- Williams's `lambda(n) = 1` together with her
    `a >= 1`.

    Verified both ways over odd starts below 2e5: every landing value is in
    the set, and every member of the set below the largest landing value IS
    landed on. An identity, not an inclusion.

    TWO COMPARISONS FAILED FIRST and both were the domain, not the
    mathematics. Including even `n` broke it, because `a >= 1` excludes them.
    Then testing membership against the entry's 60 stored terms broke it,
    because those stop near 1.7e4 while landing values reach 1.96e5 -- the
    check was against a truncated prefix. Generate the reference set to the
    bound you will actually test.
    """
    def three_smooth(m: int) -> bool:
        while m % 2 == 0:
            m //= 2
        while m % 3 == 0:
            m //= 3
    ...
    landings = sorted({landing_value(n) for n in range(3, 200001, 2)})
    assert len(landings) == 101
    ceiling = max(landings)
    a190812 = {n for n in range(1, ceiling + 1, 2) if three_smooth(n + 1)}

    assert set(landings) <= a190812
    assert not a190812 - set(landings)

    # the entry's own head, as published
    assert sorted(a190812)[:14] == [1, 3, 5, 7, 11, 15, 17, 23, 31, 35, 47, 53, 63, 71]
