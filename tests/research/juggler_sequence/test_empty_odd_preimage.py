...
from research.juggler_sequence.empty_odd_preimage import (
    CLASS_PARK,
    EXISTING_LEAN,
    FORBIDDEN_NEW_API,
    FORBIDDEN_THEOREMS,
...
def test_types_on_named_landings():
    assert odd_preimage_kind(763) == 0
    assert odd_preimage_integers(763) == []
    assert odd_preimage_empty(763) is True
    assert odd_pred_empty(763) is True
    assert odd_preimage_kind(117) == 1
    assert odd_preimage_integers(117) == [24]
    assert odd_pred_empty(117) is True
    assert odd_preimage_kind(70) == 2
    assert odd_preimage_integers(70) == [17]
...
def test_odd_step_makes_type2_regardless_of_emptiness():
    ...
    nxt = floor_power(763)
    assert nxt == 21075
    assert odd_preimage_kind(nxt) == 2
    assert odd_preimage_integers(nxt) == [763]
    assert odd_preimage_integers(floor_power(365)) == [365]
    assert odd_preimage_kind(floor_power(365)) == 2
...
def test_classify_render_and_artifacts():
    ...
    assert data["experiment"] == "juggler_empty_odd_preimage"
...
def test_dossier_boundary():
    ...
    dossier = (repo / "docs" / "problems" / "juggler_empty_odd_preimage.md").read_text(
        encoding="utf-8"
    )
