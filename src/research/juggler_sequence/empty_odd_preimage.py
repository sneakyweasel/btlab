...
CLASS_INCOMPLETE = "EMPTY_ODD_PREIMAGE_INCOMPLETE"
...
NEW_LEAN_FILES = (
    ...
    JUGGLER_DIR / "EmptyOddPreimage.lean",
)
...
def odd_preimage_kind(x: int) -> int:
    """Type 0 empty, Type 1 even occupant, Type 2 odd occupant."""
    if x < 0:
        raise ValueError("odd_preimage_kind requires a nonnegative integer")
    k = ceil_cbrt(x * x)
    if k * k * k >= (x + 1) * (x + 1):
        return 0
            ...
            rows.append(
                {
                    ...
                    "kind": odd_preimage_kind(landing),
                    "odd_pred_empty": odd_pred_empty(landing),
                    "offset": offset,
                    "width": width,
                    "next": nxt,
                    "next_odd": nxt % 2 == 1,
                    "next_kind": odd_preimage_kind(nxt),
...
def leftover_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "763_kind": odd_preimage_kind(763),
        "763_next_kind": odd_preimage_kind(floor_power(763)),
    }
...
def probe_payload() -> dict[str, Any]:
    ...
    return {
        "experiment": "juggler_empty_odd_preimage",
